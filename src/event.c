...
/* GfsEventSpatialSum: Object */

static void gfs_event_spatialsum_destroy (GtsObject * o)
{
  GfsEventSpatialSum * s = GFS_EVENT_SPATIALSUM (o);

  gts_object_destroy (GTS_OBJECT (s->v));

  (* GTS_OBJECT_CLASS (gfs_event_spatialsum_class ())->parent_class->destroy) (o);
}

static void gfs_event_spatialsum_write (GtsObject * o, FILE * fp)
{
  GfsEventSpatialSum * s = GFS_EVENT_SPATIALSUM (o);

  (* GTS_OBJECT_CLASS (gfs_event_spatialsum_class ())->parent_class->write) (o, fp);

  gfs_function_write (s->v, fp);
  fprintf (fp, " %s", s->mv->derived->name);
}

/* Had to copy this from variable.c; see comment in _read */
...
static void gfs_event_spatialsum_read (GtsObject ** o, GtsFile * fp)
{
  GfsEventSpatialSum * s = GFS_EVENT_SPATIALSUM (*o);
  GfsDomain * domain =  GFS_DOMAIN (gfs_object_simulation (s));

  (* GTS_OBJECT_CLASS (gfs_event_spatialsum_class ())->parent_class->read) (o, fp);
  if (fp->type == GTS_ERROR) 
    return;
  ...
  if (fp->type != GTS_STRING) {
    gts_file_error (fp, "expecting a string (mv)");
    return;
  }

  /* ==========
     (* GTS_OBJECT(s->mv)->klass->read) ( (GtsObject **) &s->mv, fp);
     ==========
     This results in an "unknown class" error from gfs_event_read

     1) Read method of GfsConstant calls read method of GfsEvent. 

     2) Read method of GfsEvent calls gfs_object_class_from_name (fp->token->str)

     3 ) fp->token->str = Name of GfsConstant supplied in .gfs file. Not a class.

         ---> gfs_object_class_from_name returns NULL 

     I'm stumped as of now. The below, copied from GfsConstant's read method, works. 
     I also had to copy constant_func from variable.c to make this work.
  */
...
#include <mpi.h>
/*
 #include <gerris/utils.h> does not make gfs_all_reduce visible
   so I had to do this for now. Copying is wonderful. This is 
   similar to the trouble I'm having with including variable.h 
   in event.c so that it understands GfsConstant. 
*/
...
static gboolean gfs_event_spatialsum_event (GfsEvent * event, GfsSimulation * sim)
{
  if ((* GFS_EVENT_CLASS (GTS_OBJECT_CLASS (gfs_event_spatialsum_class ())->parent_class)->event) 
      (event, sim)) {
    GfsEventSpatialSum * s = GFS_EVENT_SPATIALSUM (event);
    GfsDomain * domain = GFS_DOMAIN (sim);

    s->vol = 0.; s->sum = 0.;
    gfs_catch_floating_point_exceptions ();
    gfs_domain_traverse_leaves (domain, s->add, s);
    gfs_restore_fpe_for_function (s->v);
    gfs_all_reduce (domain, s->sum, MPI_DOUBLE, MPI_SUM);
    gfs_all_reduce (domain, s->vol, MPI_DOUBLE, MPI_SUM);
    s->mv->val = ( s->vol > 0 ) ? s->sum / s->vol : 0.;
    return TRUE;
  }
  return FALSE;
}

static void gfs_event_spatialsum_class_init (GfsEventClass * klass)
{
  GTS_OBJECT_CLASS (klass)->read = gfs_event_spatialsum_read;
  GTS_OBJECT_CLASS (klass)->write = gfs_event_spatialsum_write;
  GTS_OBJECT_CLASS (klass)->destroy = gfs_event_spatialsum_destroy;
  GFS_EVENT_CLASS (klass)->event = gfs_event_spatialsum_event;
}

static void add (FttCell * cell, GfsEventSpatialSum * s) 
{
  gdouble vol = gfs_cell_volume (cell, GFS_DOMAIN (gfs_object_simulation (s)));
  s->sum += vol*gfs_function_value (s->v, cell);
  s->vol += vol;
}

static void gfs_event_spatialsum_init (GfsEventSpatialSum * object)
{

  if (object->mv == NULL) { 
    object->mv = GFS_CONSTANT (gts_object_new (GTS_OBJECT_CLASS(gfs_constant_class ())));
  }
  object->v = gfs_function_new (gfs_function_class (), 0.);
  object->add = (FttCellTraverseFunc) add;
}
...
GfsEventClass * gfs_event_spatialsum_class (void)
{
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_event_spatialsum_info = {
      "GfsEventSpatialSum",
      sizeof (GfsEventSpatialSum),
      sizeof (GfsEventClass),
      (GtsObjectClassInitFunc) gfs_event_spatialsum_class_init,
      (GtsObjectInitFunc) gfs_event_spatialsum_init,
      (GtsArgSetFunc) NULL,
      (GtsArgGetFunc) NULL
    };
    klass = gts_object_class_new (GTS_OBJECT_CLASS (gfs_event_class ()),
				  &gfs_event_spatialsum_info);
  }
