...
#define GFS_SURFACE_BC_ODE(obj)                                                \
  GTS_OBJECT_CAST(obj, GfsSurfaceBcODE, gfs_surface_bc_ode_class())
#define GFS_IS_SURFACE_BC_ODE(obj)                                             \
  (gts_object_is_from_class(obj, gfs_surface_bc_ode_class()))
...
#define GFS_SOLID_MOVING_ODE(obj)                                              \
  GTS_OBJECT_CAST(obj, GfsSolidMovingODE, gfs_solid_moving_ode_class())
#define GFS_IS_SOLID_MOVING_ODE(obj)                                           \
  (gts_object_is_from_class(obj, gfs_solid_moving_ode_class()))
...
static void solid_moving_ode_destroy(GtsObject *object) {
  ...
  (*GTS_OBJECT_CLASS(gfs_solid_moving_ode_class())->parent_class->destroy)(
      object);
}
...
static void solid_moving_ode_read(GtsObject **o, GtsFile *fp) {
  ...
  GfsVariable **v =
      gfs_domain_velocity(GFS_DOMAIN(gfs_object_simulation(solid)));
  FttComponent c;
  for (c = 0; c < FTT_DIMENSION; c++)
    if (v[c]->surface_bc) {
      gts_file_error(fp,
                     "variable `%s' already has a surface boundary condition",
                     v[c]->name);
      return;
    } else {
      GfsSurfaceGenericBc *bc = GFS_SURFACE_GENERIC_BC(
          gts_object_new(GTS_OBJECT_CLASS(gfs_surface_bc_ode_class())));
      bc->v                        = v[c];
      bc->v->surface_bc            = bc;
  ...
  if (fp->type == '{') {
    ...
    GtsFileVariable var[] = {{GTS_DOUBLE, "vx", TRUE, &vx},
...
static gboolean solid_moving_ode_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_solid_moving_ode_class())->parent_class)
            ->event)(event, sim)) {
