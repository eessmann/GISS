...
static GtsSurface *triangulate(GSList *vertices, GSList *constraints) {
  GtsVertex * v1, *v2, *v3;
  GtsSurface *s;
  GSList *    i;
...
static GSList *contour(GSList *i, GtsSurface *s, gdouble z, gboolean closed) {
  GSList *   edges = NULL;
  ...
  while (i) {
    ...
    if (vold)
      edges = g_slist_prepend(
          edges, gts_edge_new(GTS_EDGE_CLASS(gts_constraint_class()), v, vold));
    else
      vin = v;
    vold = v;
    i    = i->next;
  ...
  if (closed)
    edges = g_slist_prepend(
        edges, gts_edge_new(GTS_EDGE_CLASS(gts_constraint_class()), vin, vold));
  return edges;
}

static void surface_add_shape(GtsSurface *s, GSList *shape, gdouble z1,
                              gdouble z2, guint nz, gboolean closed,
                              gboolean ends) {
  ...
  for (i = 0, z = z1; i <= nz; i++, z += dz) {
    if (e1 == NULL)
      ...
      while (i && j) {
        GtsEdge *e;
        GtsEdge *ee = gts_edge_new(gts_edge_class(), GTS_SEGMENT(i->data)->v2,
                                   GTS_SEGMENT(j->data)->v1);

        if (!closed || i->next != NULL)
          e = gts_edge_new(gts_edge_class(), GTS_SEGMENT(i->data)->v2,
                           GTS_SEGMENT(j->data)->v2);
        else
          e = ein;
        if (eold == NULL)
          eold = ein = gts_edge_new(gts_edge_class(), GTS_SEGMENT(i->data)->v1,
                                    GTS_SEGMENT(j->data)->v1);
        gts_surface_add_face(s, gts_face_new(s->face_class, i->data, eold, ee));
        gts_surface_add_face(s, gts_face_new(s->face_class, e, ee, j->data));
        eold = e;
        i    = i->next;
        j    = j->next;
      }
...
static void surface_add_ellipse_shape(GtsSurface *s, gdouble x, gdouble y,
                                      gdouble radius, gdouble theta,
                                      gdouble thetamax, gdouble e, gdouble z1,
                                      gdouble z2, guint np, gboolean closed) {
  GSList *shape = NULL;
  guint   i, npm = np;
  ...
  for (i = 0; i < npm; i++) {
    ...
    shape          = g_slist_prepend(
        shape, gts_point_new(gts_point_class(), x + radius * e * cos(theta1),
                             y + radius / e * sin(theta1), 0.));
  }
  surface_add_shape(s, shape, z1, z2, 10, TRUE, closed);
  g_slist_free(shape);
}

static void surface_add_star_shape(GtsSurface *s, gdouble dr, gdouble z1,
                                   gdouble z2, guint np, gboolean closed) {
  GSList *shape = NULL;
  guint   i;
  ...
  for (i = 0; i < np; i++) {
    ...
    shape = g_slist_prepend(shape, gts_point_new(gts_point_class(),
                                                 radius * cos(theta),
                                                 radius * sin(theta), 0.));
  }
  surface_add_shape(s, shape, z1, z2, 10, TRUE, closed);
...
static void surface_add_channel_shape(GtsSurface *s, gdouble z1, gdouble z2,
                                      guint np, gboolean closed) {
  GSList *shape = NULL;
  gint    i;
  ...
  for (i = np - 1; i >= 0; i--) {
    ...
    shape = g_slist_prepend(shape, gts_point_new(gts_point_class(), x, y, 0.));
  }
  for (i = 0; i < np; i++) {
    gdouble x = -0.501 + 1.002 * (gdouble)i / (gdouble)(np - 1);
...
static void surface_add_rectangular_channel_shape(GtsSurface *s, gdouble z1,
                                                  gdouble z2, gboolean closed) {
  GSList *shape = NULL;

  g_return_if_fail(s != NULL);

  shape = g_slist_prepend(
      shape, gts_point_new(gts_point_class(), -0.5001, -0.125001, 0.));
  shape = g_slist_prepend(
      shape, gts_point_new(gts_point_class(), -0.5001, 0.125001, 0.));
  shape = g_slist_prepend(
      shape, gts_point_new(gts_point_class(), 0.5001, 0.125001, 0.));
...
static void surface_add_witch_shape(GtsSurface *s, gdouble xo, gdouble h,
                                    gdouble lh, gdouble z1, gdouble z2,
                                    guint np) {
  GSList *shape = NULL;
  guint   i;
  ...
  shape = g_slist_prepend(
      shape, gts_point_new(gts_point_class(), -0.5001, -0.5002, 0.));
  for (i = 0; i < np; i++) {
...
static void surface_add_rayleigh_taylor_shape(GtsSurface *s, gdouble yo,
                                              gdouble a, gdouble z1, gdouble z2,
                                              guint np) {
  GSList *shape = NULL;
  guint   i;
  ...
  shape = g_slist_prepend(shape,
                          gts_point_new(gts_point_class(), -0.5001, -10., 0.));
  for (i = 0; i < np; i++) {
    gdouble x = -0.5001 + 1.0002 * i / (gdouble)(np - 1);

    shape = g_slist_prepend(
        shape, gts_point_new(gts_point_class(), x,
                             yo + a * cos(i * 2. * M_PI / (np - 1)), 0.));
  }
  shape = g_slist_prepend(shape,
                          gts_point_new(gts_point_class(), 0.5001, -10., 0.));
  shape = g_slist_reverse(shape);
...
static gboolean surface_add_file_shape(GtsSurface *s, gdouble z1, gdouble z2,
                                       gboolean zextrude, FILE *fp) {
  ...
  s1 = gts_surface_new(gts_surface_class(), s->face_class, s->edge_class,
                       s->vertex_class);
  surface_add_shape(s1, shape, z1, z2, zextrude ? 1 : 10, TRUE, TRUE);
  if (!gts_surface_is_orientable(s1) || !gts_surface_is_closed(s1) ||
      (self = gts_surface_is_self_intersecting(s1))) {
    GSList *i = shape;

    while (i) {
      fprintf(stderr, "%g %g\n", GTS_POINT(i->data)->x, GTS_POINT(i->data)->y);
      i = i->next;
    }
    fprintf(stderr, "\n");
    if (self)
...
int main(int argc, char *argv[]) {
  ...
  while (c != EOF) {
    ...
    static struct option long_options[] = {
        ...
        {NULL}};
    int option_index = 0;
    switch ((c = getopt_long(argc, argv, "hvn:d:r:oz", long_options,
                             &option_index))) {
    ...
    case 'h': /* help */
      fprintf(stderr,
              ...
              "  ellipse, star, 4ellipses, square, almgren, channel, "
              "half-cylinder,\n"
              "  rayleigh-taylor, FILE\n"
              "\n"
              "  -z    --zextrude    shape file contains z coordinate\n"
              "  -n N  --number=N    set number of points for polar surfaces "
              "(default is 100)\n"
              "  -o    --open        generate open surfaces\n"
              "  -d R  --dr=R        set inner radius for star to R (default "
              "is 0.15)\n"
  ...
  if (optind >= argc) { /* missing SHAPE */
    fprintf(stderr, "shapes: missing SHAPE\n"
  ...
  if (!strcmp(shape, "ellipse"))
    ...
    surface_add_ellipse_shape(s, 0.25, 0.25, 5. / 32. / sqrt(2.), 0.001,
                              2. * M_PI, sqrt(2.), -1., 1., number, closed);
    surface_add_ellipse_shape(s, -0.25, 0.25, 5. / 32. / sqrt(2.), 0.001,
                              2. * M_PI, sqrt(2.), -1., 1., number, closed);
    surface_add_ellipse_shape(s, 0.25, -0.25, 5. / 32. / sqrt(2.), 0.001,
                              2. * M_PI, sqrt(2.), -1., 1., number, closed);
    ...
    surface_add_ellipse_shape(s, 0., 0., 0.25 * sqrt(2.), M_PI / 4., 2. * M_PI,
                              1., -1., 1., 4, closed);
  else if (!strcmp(shape, "almgren")) {
    surface_add_ellipse_shape(s, 0.25, 0.25, 0.1, 0.001, 2. * M_PI, 1., -1., 1.,
                              number, closed);
    surface_add_ellipse_shape(s, -0.25, 0.125, sqrt(0.15 * 0.1), 0.001,
                              2. * M_PI, 0.15 / sqrt(0.15 * 0.1), -1., 1.,
                              number, closed);
    surface_add_ellipse_shape(s, 0., -0.25, sqrt(0.2 * 0.1), 0.001, 2. * M_PI,
                              0.2 / sqrt(0.2 * 0.1), -1., 1., number, closed);
    ...
    surface_add_ellipse_shape(s, -0.375001, 0., 0.03125001, M_PI / 2., M_PI, 1.,
                              -1., 1., number, closed);
    ...
    surface_add_ellipse_shape(s, 0., 0., 0.5, 0.001, 2. * M_PI, 1., -2., 2.,
                              number, closed);
    gts_surface_foreach_face(s, (GtsFunc)gts_triangle_revert, NULL);
    surface_add_ellipse_shape(s, 0., 0., 0.25, 0.001, 2. * M_PI, 1., -1., 1.,
                              number, closed);
