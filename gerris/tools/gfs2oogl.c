...
static void draw_vector(FttCell *cell, gpointer *data) {
  ...
  fprintf(fp, "VECT 1 3 0 3 0 %g %g %g %g %g %g %g %g %g\n",
          ...
          pos.y + f.y - (f.x / 2. + f.y) / 5., pos.z + f.z, pos.x + f.x,
          pos.y + f.y, pos.z + f.z, pos.x + f.x - (f.x + f.y / 2.) / 5.,
          pos.y + f.y + (f.x / 2. - f.y) / 5., pos.z + f.z);
...
static void output_mixed_vorticity(FttCell *cell, GfsVariable *v) {
  ...
  printf("%g %g %g %g\n", s->ca.x, s->ca.y, s->ca.z, GFS_VALUE(cell, v) / size);
}
...
#define S_VERTEX(obj)    GTS_OBJECT_CAST(obj, SVertex, s_vertex_class())
#define IS_S_VERTEX(obj) (gts_object_is_from_class(obj, s_vertex_class()))
...
static GtsVertexClass *s_vertex_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo s_vertex_info = {"SVertex",
                                        sizeof(SVertex),
      ...
      if (i1 >= 0 && i1 < g->nx && j1 >= 0 && j1 < g->ny) {
        ...
        while (k) {
          ...
          if (IS_S_VERTEX(p) && IS_S_VERTEX(p1) && S_VERTEX(p1)->s >= 0.) {
            ...
            if (S_VERTEX(p)->s - S_VERTEX(p1)->s >= 3. * ds &&
                d < 0.9 * ds * ds)
...
static InsertStatus closest_grid_add(ClosestGrid *g, GtsPoint *p, gdouble dmin,
                                     gdouble ds) {
  InsertStatus status;
...
static InsertStatus insert(FttVector p, ClosestGrid *g, gdouble dmin, gdouble s,
                           gdouble ds, GtsVertex **vertex) {
...
static gboolean advect(GfsDomain *domain, FttCell *cell, FttVector *p,
                       gdouble ds, gint direction) {
  FttComponent  c;
  FttVector     u, ph;
  gdouble       nu = 0.;
  guint         n  = 10;
...
static InsertStatus grow_streamline(GfsDomain *domain, ClosestGrid *grid,
                                    FttVector p, gdouble dmin, gdouble rds,
                                    gint direction, GSList **stream) {
...
static GSList *streamline(GfsDomain *domain, ClosestGrid *grid, FttVector p,
                          gdouble dmin, gdouble rds, gboolean closed) {
  GSList *     stream = NULL;
  InsertStatus status = grow_streamline(domain, grid, p, dmin, rds, 1, &stream);
...
static gboolean seed(GSList *i, GfsDomain *domain, ClosestGrid *grid,
                     gdouble dsep, gdouble dmin, GList **streams,
                     gboolean closed) {
...
static GList *even_streamlines(GfsDomain *domain, ClosestGrid *grid,
                               gdouble dsep, gdouble dmin, gboolean closed) {
  ...
  current = streams =
      g_list_prepend(streams, streamline(domain, grid, p, dmin, 0.25, closed));
  do {
...
static GSList *simplify_stream(GSList *stream, gdouble maxcost) {
  GSList *  i  = stream;
  GSList *  s  = NULL;
...
int main(int argc, char *argv[]) {
  ...
  while (c != EOF) {
    ...
    case 'S': /* squares */
      ...
      break;
    case 'x': /* sx */
      box = gts_bbox_new(gts_bbox_class(), NULL, atof(optarg),
                         -G_MAXDOUBLE / 2., -G_MAXDOUBLE / 2., atof(optarg),
                         G_MAXDOUBLE / 2., G_MAXDOUBLE / 2.);
      break;
    case 'y': /* sy */
      box = gts_bbox_new(gts_bbox_class(), NULL, -G_MAXDOUBLE / 2.,
                         atof(optarg), -G_MAXDOUBLE / 2., G_MAXDOUBLE / 2.,
                         atof(optarg), G_MAXDOUBLE / 2.);
      break;
    case 'z': /* sz */
      box = gts_bbox_new(gts_bbox_class(), NULL, -G_MAXDOUBLE / 2.,
                         -G_MAXDOUBLE / 2., atof(optarg), G_MAXDOUBLE / 2.,
                         G_MAXDOUBLE / 2., atof(optarg));
      break;
    case 's': /* surface */
      draw_surface = TRUE;
    ...
    case 'h': /* help */
      fprintf(
          ...
          "                        the variable needs to be specified using "
          "-c\n"
          "  -f D    --stream=D    draw evenly-spaced streamlines (D is the "
          "spacing)\n"
          "  -I M    --dmin=M      controls length of evenly-spaced "
          "streamlines\n"
          "                        default is 0.5\n"
          "  -O M    --maxcost=M   controls compression of streamlines "
          "(default is 2e-7)\n"
          "  -b x,.. --box=x,y,..  specify bounding box for streamline "
          "calculation\n"
          "  -j      --closed      outputs only closed streamlines\n"
          "  -p F    --profile=F   output list of values for coordinates "
          "defined in F\n"
          "  -o      --mixed       output text values in mixed cells only\n"
          "  -L L    --level=L     use cells at level L only\n"
          "  -i      --reinit      reinitializes refinement and solid "
          "fractions\n"
          "  -e      --merged      draw boundaries of merged cells\n"
          "  -S      --squares     draw (colored) squares\n"
          "  -g      --gnuplot     output gnuplot data\n"
          "  -x VAL  --sx=VAL      outputs a GTS surface, cross section for x "
          "= VAL\n"
          "                        of the scalar variable\n"
          "  -y VAL  --sy=VAL      outputs a GTS surface, cross section for y "
          "= VAL\n"
          "                        of the scalar variable\n"
          "  -z VAL  --sz=VAL      outputs a GTS surface, cross section for z "
          "= VAL\n"
          "                        of the scalar variable\n"
          "  -s S    --surface=S   outputs the surface defined by file S (or "
          "the solid\n"
          "                        surface is S is equal to `solid')\n"
          "  -V S    --vector=S    output an OOGL representation of the "
          "velocity vector\n"
          "                        field in the mixed cells\n"
          "  -l F    --streamlines=F  draw streamlines starting from each "
          "point defined\n"
          "                        in file F\n"
          "  -C F    --cylinder=F  draw stream cylinders starting from each "
          "point defined\n"
          "                        in file F\n"
          "  -R F    --ribbon=F    draw stream ribbons starting from each "
          "point defined\n"
          "                        in file F\n"
          "  -r                    refines the solid surface according to the "
          "local\n"
          "                        resolution\n"
          "  -c V    --color=V     color surfaces, streamlines etc... "
          "according to the\n"
    ...
    if (var != NULL) {
      if (min == max) {
        stats = gfs_domain_stats_variable(domain, var, FTT_TRAVERSE_ALL, -1,
                                          NULL, NULL);
        if (verbose)
          fprintf(stderr, "min: %g avg: %g| %g max: %g n: %7d\n", stats.min,
                  stats.mean, stats.stddev, stats.max, stats.n);
      } else {
        stats.min = min;
        stats.max = max;
      }
    ...
    if (var != NULL && gnuplot) {
      if (mixed)
        gfs_domain_traverse_mixed(domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                                  (FttCellTraverseFunc)output_mixed_variable,
                                  var);
      else {
        if (level < 0)
          gfs_write_gnuplot(domain, var, FTT_TRAVERSE_LEAFS, -1, box, stdout);
        else
          gfs_write_gnuplot(domain, var, FTT_TRAVERSE_LEVEL, level, box,
                            stdout);
      }
      ...
      while (fscanf(stream, "%lf %lf %lf", &p.x, &p.y, &p.z) == 3) {
        ...
        if (lines)
          ...
          gfs_draw_stream_ribbon(domain, p, 2e-3, var, stats.min, stats.max,
                                 stdout);
        else
          gfs_draw_stream_cylinder(domain, p, 5e-4, var, stats.min, stats.max,
                                   stdout);
#endif /* 3D */
      }
      printf("})\n");
      ...
      if (var)
        while (fscanf(profile, "%lf %lf %lf", &p.x, &p.y, &p.z) == 3) {
          ...
          if (cell)
            printf("%g %g %g %g\n", p.x, p.y, p.z,
                   gfs_dimensional_value(var, gfs_interpolate(cell, pm, var)));
        }
      else {
        GSList *j;
        guint   i = 4;
        ...
        while (fscanf(profile, "%lf %lf %lf", &p.x, &p.y, &p.z) == 3) {
          ...
          if (cell) {
            ...
            while (j) {
              GfsVariable *v = j->data;
              printf("%g ",
                     gfs_dimensional_value(v, gfs_interpolate(cell, pm, v)));
              j = j->next;
            }
            printf("\n");
          }
...
#if FTT_2D
      if (box == NULL)
        box = gts_bbox_new(gts_bbox_class(), NULL, 0., -G_MAXDOUBLE / 2.,
                           -G_MAXDOUBLE / 2., 0., G_MAXDOUBLE / 2.,
                           G_MAXDOUBLE / 2.);
#else  /* 3D */
      if (box == NULL)
        gfs_domain_traverse_mixed(domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
...
#endif /* 3D */
      gfs_domain_cell_traverse_box(domain, box, FTT_PRE_ORDER,
      ...
      while (i) {
        GtsSurface *s = GFS_IS_SURFACE(GFS_SOLID(i->data)->s)
                            ? GFS_SURFACE(GFS_SOLID(i->data)->s)->s
                            : NULL;

        if (s) {
          if (refine)
            gts_surface_refine(s, (GtsKeyFunc)local_size_ratio, domain, NULL,
                               NULL, (GtsStopFunc)stop, NULL);
          gfs_draw_surface(domain, s, var, stats.min, stats.max, stdout);
        }
        i = i->next;
      }
      g_slist_free(l);
      ...
      puts("LIST {\n");
      gfs_domain_traverse_merged(domain, (GfsMergedTraverseFunc)merged_draw,
                                 stdout);
      puts("}\n");
      ...
      for (c = 0; c < FTT_DIMENSION; c++) {
        ...
        data[1] = u[c];
        gfs_domain_traverse_mixed(domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                                  (FttCellTraverseFunc)compute_mixed_vorticity,
                                  data);
      }
      gfs_domain_traverse_mixed(domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
      ...
      i = s = even_streamlines(domain, grid, even_stream, even_stream * rdmin,
                               closed);
