...
static void relativepos(FttCell *cell, GfsSimulation *s1, GfsSimulation *s2,
                        FttVector *pos) {
...
static void combine(FttCell *cell, CombineData *p) {
  ...
  if (matching) {
    ...
    GFS_VALUE(cell, p->v1) =
        MAX(GFS_VALUE(cell, p->v1), GFS_VALUE(matching, p->v2));
  }
}
...
int main(int argc, char *argv[]) {
  ...
  int            c    = 0;
  gchar *        name = NULL;
  GfsVariable *  var1 = NULL;
  GfsVariable *  var2 = NULL;
  GfsSimulation *s1   = NULL;
  GfsSimulation *s2   = NULL;
  ...
  while (c != EOF) {
    ...
    static struct option long_options[] = {{"help", no_argument, NULL, 'h'},
                                           ...
                                           {NULL}};
    int                  option_index   = 0;
    switch ((c = getopt_long(argc, argv, "hv", long_options, &option_index))) {
    ...
    case 'h': /* help */
      fprintf(stderr,
              ...
              "Computes the maximum of VAR between the solutions in FILE1 and "
              "FILE2\n"
  ...
  if (optind >= argc) { /* missing FILE1 */
    fprintf(stderr, "gfscombine: missing FILE1\n"
  ...
  if (optind >= argc) { /* missing FILE2 */
    fprintf(stderr, "gfscombine: missing FILE2\n"
                    "Try `gfscombine --help' for more information.\n");
    return 1; /* failure */
  }
  fname2 = argv[optind++];
  ...
  if (optind >= argc) { /* missing VAR */
    fprintf(stderr, "gfscombine: missing VAR\n"
                    "Try `gfscombine --help' for more information.\n");
    return 1; /* failure */
  }
  name = argv[optind++];
  ...
  if (verbose) {
    GfsNorm  norm = gfs_domain_norm_variable(GFS_DOMAIN(s1), var1, NULL,
                                            FTT_TRAVERSE_LEAFS, -1, NULL, NULL);
    GtsRange s    = gfs_domain_stats_variable(GFS_DOMAIN(s1), var1,
                                           FTT_TRAVERSE_LEAFS, -1, NULL, NULL);
  ...
  if (verbose) {
    ...
    fprintf(stderr,
            ...
            norm.w, s.min * f, s.mean * f, s.stddev * f, s.max * f);
  }
