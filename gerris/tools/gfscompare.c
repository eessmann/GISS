...
#define IS_GFS_VERTEX(obj) (gts_object_is_from_class(obj, gfs_vertex_class()))
...
GtsVertexClass *gfs_vertex_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_vertex_info = {"GfsVertex",
                                          sizeof(GfsVertex),
...
static void difference_triangulated(GfsVertex *v, gpointer *data) {
  ...
  if (f != NULL && gts_triangle_quality(GTS_TRIANGLE(f)) > 0.8) {
    ...
    GTS_POINT(v)->z = GFS_VALUE(v->cell, e) =
        GFS_VALUE(v->cell, var1) - (fv1 + a * (fv2 - fv1) + b * (fv3 - fv1));
  }
}
...
static gboolean difference_tree(FttCell *cell, GfsDomain *ref, GfsVariable *v1,
                                GfsVariable *v2, GfsVariable *e,
                                gdouble period) {
...
static void difference_constant(FttCell *cell, gpointer *data) {
  ...
  if ((full == -2 || (full == -1 && !GFS_IS_MIXED(cell)) ||
...
static void difference(FttCell *cell, gpointer *data) {
  ...
  if ((!(*mixed) || a < 1.) &&
      (full == -2 || (full == -1 && !GFS_IS_MIXED(cell)) ||
       (full >= 0 && !is_mixed(cell, full))) &&
      (!(*centered) || a >= 0.5)) {
...
int main(int argc, char *argv[]) {
  ...
  while (c != EOF) {
    ...
    case 'g': /* gradient */
      ...
      if (gradient >= FTT_DIMENSION) {
        fprintf(stderr, "gfscompare: invalid argument for option `gradient'.\n"
    ...
    case 'h': /* help */
      fprintf(
          ...
          "  -m V  --min=V       set minimum of color scale to V (used with "
          "-S)\n"
          "  -M V  --max=V       set maximum of color scale to V\n"
          "  -a    --abs         output the absolute value of the error field\n"
          "  -C    --constant    apply a constant shift to one of the field, "
          "minimizing\n"
          "                      the error between the two fields (useful for "
          "pressure)\n"
          "  -w    --not-weighted do not use area-weighted norm estimation\n"
          "  -c    --centered    use error estimation for cell-centered "
          "variables\n"
          "  -p P  --period=P    shifts FILE1 by P along the x axis\n"
          "  -H    --histogram   output (error,volume) pairs for each cell "
          "used\n"
          "                      to compute the error norms\n"
          "  -o    --output      output a GTS representation of the error "
          "field\n"
          "  -S    --squares     output an OOGL representation of the error "
          "field\n"
#if FTT_2D
          "  -G    --gnuplot     output a gnuplot representation of the error "
          "field\n"
          "  -t    --triangulate use center of mass triangulation\n"
#endif /* FTT_2D */
          "  -l    --log         output the log10 of the absolute value of the "
          "error field\n"
          "  -f L  --full=L      compare only leaf cells descendants of a cell "
          "full at level L\n"
  ...
  if (optind >= argc) { /* missing FILE1 */
    fprintf(stderr, "gfscompare: missing FILE1\n"
                    "Try `gfscompare --help' for more information.\n");
    return 1; /* failure */
  }
  fname1 = argv[optind++];
  ...
  if (optind >= argc) { /* missing FILE2 */
    fprintf(stderr, "gfscompare: missing FILE2\n"
                    "Try `gfscompare --help' for more information.\n");
    return 1; /* failure */
  }
  fname2 = argv[optind++];
  ...
  if (optind >= argc) { /* missing VAR */
    fprintf(stderr, "gfscompare: missing VAR\n"
                    "Try `gfscompare --help' for more information.\n");
    return 1; /* failure */
  }
  name = argv[optind++];
  ...
  if (verbose) {
    ...
    norm = gfs_domain_norm_variable(GFS_DOMAIN(s1), var1, NULL,
                                    FTT_TRAVERSE_LEAFS, -1, NULL, NULL);
    s = gfs_domain_stats_variable(GFS_DOMAIN(s1), var1, FTT_TRAVERSE_LEAFS, -1,
                                  NULL, NULL);
    ...
    fprintf(stderr,
            ...
            "  min: %g avg: %g | %g max: %g\n",
            fname1, norm.first * f, norm.second * f, norm.infty * f, norm.w,
            s.min * f, s.mean * f, s.stddev * f, s.max * f);
    norm = gfs_domain_norm_variable(GFS_DOMAIN(s2), var2, NULL,
                                    FTT_TRAVERSE_LEAFS, -1, NULL, NULL);
    s = gfs_domain_stats_variable(GFS_DOMAIN(s2), var2, FTT_TRAVERSE_LEAFS, -1,
                                  NULL, NULL);
    fprintf(stderr,
            "%s:\n"
            "  first: %g second: %g infty: %g w: %g\n"
            "  min: %g avg: %g | %g max: %g\n",
            fname2, norm.first * f, norm.second * f, norm.infty * f, norm.w,
            s.min * f, s.mean * f, s.stddev * f, s.max * f);
  }
  ...
  if (gradient < FTT_DIMENSION) {
    ...
    data[1] = &gradient;
    data[2] = g1;
    gfs_domain_cell_traverse(GFS_DOMAIN(s1), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             -1, (FttCellTraverseFunc)compute_gradient, data);
    data[0] = var2;
    data[2] = g2;
    gfs_domain_cell_traverse(GFS_DOMAIN(s2), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             -1, (FttCellTraverseFunc)compute_gradient, data);
    var1 = g1;
    var2 = g2;
  }
  ...
  if (centered)
    gfs_domain_cell_traverse(GFS_DOMAIN(s1), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             -1, (FttCellTraverseFunc)difference_centered,
                             data);
#if FTT_2D
  else if (triangulate) {
    GtsSurface *ss1, *ss2;
    ...
    gfs_domain_cell_traverse(
        GFS_DOMAIN(s1), FTT_POST_ORDER, FTT_TRAVERSE_NON_LEAFS, -1,
        (FttCellTraverseFunc)gfs_get_from_below_intensive, var1);
    gfs_domain_cell_traverse(
        GFS_DOMAIN(s2), FTT_POST_ORDER, FTT_TRAVERSE_NON_LEAFS, -1,
  ...
  if (constant) {
    ...
    data[6] = e;
    gfs_domain_cell_traverse(GFS_DOMAIN(s1), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             -1, (FttCellTraverseFunc)difference_constant,
                             data);
  ...
  data[7] = e;
  gfs_domain_cell_traverse(GFS_DOMAIN(s1), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                           -1, (FttCellTraverseFunc)difference, data);
  ...
  if (verbose) {
    ...
    if (refined_error) {
      norm = gfs_domain_norm_variable(
          GFS_DOMAIN(s1), e, NULL, FTT_TRAVERSE_LEVEL,
          gfs_domain_depth(GFS_DOMAIN(s1)), NULL, NULL);
      fprintf(stderr,
  ...
  if (output ||
      ...
      squares) {
    if (take_log)
      gfs_domain_cell_traverse(GFS_DOMAIN(s1), FTT_PRE_ORDER,
    ...
    if (squares) {
      GtsRange stats = gfs_domain_stats_variable(
          GFS_DOMAIN(s1), e, FTT_TRAVERSE_LEAFS, -1, NULL, NULL);

      gfs_write_squares(GFS_DOMAIN(s1), e, min < G_MAXDOUBLE ? min : stats.min,
                        max > -G_MAXDOUBLE ? max : stats.max,
                        FTT_TRAVERSE_LEAFS, -1, NULL, stdout);
    }
#if FTT_2D
    else if (gnuplot)
      gfs_write_gnuplot(GFS_DOMAIN(s1), e, FTT_TRAVERSE_LEAFS, -1, NULL,
                        stdout);
#endif /* FTT_2D */
    else
