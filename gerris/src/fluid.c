 ...
 */
static gdouble average_neighbor_value(const FttCellFace *face, guint v,
                                      gdouble *x) {
  ...
  if (FTT_CELL_IS_LEAF(face->neighbor))
    ...
    for (i = 0; i < n; i++)
      if (children.c[i] && GFS_VALUEI(children.c[i], v) != GFS_NODATA) {
        gdouble w = GFS_IS_MIXED(children.c[i])
                        ? GFS_STATE(children.c[i])->solid->s[od]
                        : 1.;
        a += w;
...
static void average_neighbor_value_stencil(const FttCellFace *face, guint v) {
  ...
  if (FTT_CELL_IS_LEAF(face->neighbor))
    ...
    n = ftt_cell_children_direction(face->neighbor,
                                    FTT_OPPOSITE_DIRECTION(face->d), &children);
    for (i = 0; i < n; i++)
      if (children.c[i]) {
        a += 1.;
 ...
 */
static GfsGradient interpolate_1D2(FttCell *cell, FttDirection d, gdouble x,
                                   guint v) {
  GfsGradient p;
  FttCellFace f1, f2;
  gdouble     p1 = 0., p2 = 0.;
 ...
 * this function is modified
 */
static GfsGradient interpolate_1D1(FttCell *cell, FttDirection d, gdouble x,
                                   guint v) {
  GfsGradient p = {1., 0.};
  FttCellFace f;
...
static void interpolate_1D1_stencil(FttCell *cell, FttDirection d, guint v) {
  FttCellFace f;
 ...
 * this function is modified
 */
static GfsGradient interpolate_2D1(FttCell *cell, FttDirection d1,
                                   FttDirection d2, gdouble x, gdouble y,
                                   guint v) {
  GfsGradient p = {1., 0.};
  FttCellFace f1, f2;
...
static void interpolate_2D1_stencil(FttCell *cell, FttDirection d1,
                                    FttDirection d2, guint v) {
  FttCellFace f1, f2;
...
static gint perpendicular[FTT_NEIGHBORS][FTT_CELLS] = {
    {-1, 2, -1, 3}, {2, -1, 3, -1}, {1, 0, -1, -1}, {-1, -1, 1, 0}};
#else  /* FTT_3D */
...
#define REFINE_CORNER(cell)                                                    \
  {                                                                            \
    if (cell && FTT_CELL_IS_LEAF(cell) && ftt_cell_level(cell) < level - 1)    \
      ftt_cell_refine_single(cell, init, data);                                \
  }

void ftt_cell_refine_corners(FttCell *cell, FttCellInitFunc init,
                             gpointer data) {
  FttDirection     d;
...
gdouble gfs_center_gradient(FttCell *cell, FttComponent c, guint v) {
  ...
  if (f1.neighbor) {
    ...
    if (f2.neighbor) {
      ...
      return (x1 * x1 * (v2 - v0) + x2 * x2 * (v0 - v1)) /
             (x1 * x2 * (x2 + x1));
    } else
...
+ van Leer limiter. See http://en.wikipedia.org/wiki/Flux_limiter */
      ...
      if (s1 * s2 <= 0.)
        return 0.;
      gdouble s0 =
          (x1 * x1 * (v2 - v0) + x2 * x2 * (v0 - v1)) / (x1 * x2 * (x2 + x1));
      if (ABS(s2) < ABS(s1))
        s1 = s2;
      if (ABS(s0) < ABS(s1))
        return s0;
 ...
 * at the center of the cell.
 */
gdouble gfs_center_regular_gradient(FttCell *cell, FttComponent c,
                                    GfsVariable *v) {
  g_return_val_if_fail(cell != NULL, 0.);
  g_return_val_if_fail(c < FTT_DIMENSION, 0.);
 ...
 */
gdouble gfs_center_regular_2nd_derivative(FttCell *cell, FttComponent c,
                                          GfsVariable *v) {
  g_return_val_if_fail(cell != NULL, 0.);
  g_return_val_if_fail(c < FTT_DIMENSION, 0.);
  g_return_val_if_fail(v != NULL, 0.);
  ...
  if (n1 && n2) {
    ...
    if (ftt_cell_level(n1) < level || ftt_cell_level(n2) < level)
      return gfs_center_regular_2nd_derivative(ftt_cell_parent(cell), c, v) /
             4.;
 ...
 */
void gfs_face_gradient(const FttCellFace *face, GfsGradient *g, guint v,
                       gint max_level) {
  guint level;
...
/* face_weighted_gradient_stencil() needs to be updated whenever
   this function is modified                                   */
static void face_weighted_gradient(const FttCellFace *face, GfsGradient *g,
                                   guint v, gint max_level, guint dimension) {
  guint level;
...
void gfs_face_weighted_gradient(const FttCellFace *face, GfsGradient *g,
                                guint v, gint max_level) {
  face_weighted_gradient(face, g, v, max_level, FTT_DIMENSION);
}

void gfs_face_weighted_gradient_2D(const FttCellFace *face, GfsGradient *g,
                                   guint v, gint max_level) {
...
static FttCell *cell_corner_neighbor1(FttCell *cell, FttDirection *d,
                                      gint max_level) {
  if (!cell)
    return NULL;
...
static void draw_cell(FttCell *cell, gdouble r, gdouble g, gdouble b,
  ...
  fprintf(stderr,
          ...
          "4 1 2 6 5 %g %g %g\n"
          ")\n",
          name, p.x - size, p.y - size, p.z - size, p.x + size, p.y - size,
          ...
          p.z + size, p.x - size, p.y + size, p.z + size, r, g, b, r, g, b, r,
...
static gboolean face_bilinear(const FttCellFace *face, FttCell **n,
                              FttVector *o,
  ...
  d1[0] = d[2];
  d1[1] = d[0];
  d1[2] = FTT_OPPOSITE_DIRECTION(d[1]);
...
static gboolean mixed_face_gradient(const FttCellFace *face, Gradient *g,
                                    guint v, gint max_level) {
  FttCell *    n[N_CELLS];
  gdouble      m[N_CELLS - 1][N_CELLS - 1];
  FttVector    o, cm;
...
#if FTT_2D
  {
    ...
    vp   = ((&cm.x)[cp] - (&o.x)[cp]) / h;
    g->a = ((m[c][0] + m[2][0] * vp) + (m[c][1] + m[2][1] * vp) +
            (m[c][2] + m[2][2] * vp));
    g->b = m[c][0] + m[2][0] * vp;
...
#else  /* 3D */
  {
    ...
    case FTT_X:
      g->a = g->b =
          m[0][0] + cm.y * m[3][0] + cm.z * m[4][0] + cm.y * cm.z * m[6][0];
      for (j = 1; j < N_CELLS - 1; j++) {
        gdouble a =
            m[0][j] + cm.y * m[3][j] + cm.z * m[4][j] + cm.y * cm.z * m[6][j];
        g->a += a;
    ...
    case FTT_Y:
      g->a = g->b =
          m[1][0] + cm.x * m[3][0] + cm.z * m[5][0] + cm.x * cm.z * m[6][0];
      for (j = 1; j < N_CELLS - 1; j++) {
        gdouble a =
            m[1][j] + cm.x * m[3][j] + cm.z * m[5][j] + cm.x * cm.z * m[6][j];
        g->a += a;
        g->c += a * GFS_VALUEI(n[j + 1], v);
      }
      break;
    case FTT_Z:
      g->a = g->b =
          m[2][0] + cm.x * m[4][0] + cm.y * m[5][0] + cm.x * cm.y * m[6][0];
      for (j = 1; j < N_CELLS - 1; j++) {
        gdouble a =
            m[2][j] + cm.x * m[4][j] + cm.y * m[5][j] + cm.x * cm.y * m[6][j];
        g->a += a;
        g->c += a * GFS_VALUEI(n[j + 1], v);
      }
      break;
 ...
 * this function is modified
 */
static void face_cm_gradient(const FttCellFace *face, GfsGradient *g, guint v,
                             gint max_level, gboolean weighted) {
  guint    level;
  Gradient gcf;
 ...
 */
void gfs_face_cm_gradient(const FttCellFace *face, GfsGradient *g, guint v,
                          gint max_level) {
  g_return_if_fail(face != NULL);
 ...
 * this function is modified
 */
void gfs_face_cm_weighted_gradient(const FttCellFace *face, GfsGradient *g,
                                   guint v, gint max_level) {
  g_return_if_fail(face != NULL);
  g_return_if_fail(g != NULL);
...
static gboolean cell_bilinear(FttCell *cell, FttCell **n, FttVector *o,
                              void (*cell_pos)(const FttCell *, FttVector *),
                              gint    max_level,
                              gdouble m[N_CELLS - 1][N_CELLS - 1]) {
...
/* gfs_cell_dirichlet_gradient_stencil needs to be updated whenever
   this function is modified  */
void gfs_cell_dirichlet_gradient(FttCell *cell, guint v, gint max_level,
                                 gdouble v0, FttVector *grad) {
  g_return_if_fail(cell != NULL);
  ...
  if (!GFS_IS_MIXED(cell))
    ...
    if (!cell_bilinear(cell, n, &GFS_STATE(cell)->solid->ca, gfs_cell_cm,
                       max_level, m))
      return;
...
void gfs_mixed_cell_gradient(FttCell *cell, GfsVariable *v, FttVector *g) {
  ...
  if (v->surface_bc) {
    (*GFS_SURFACE_GENERIC_BC_CLASS(GTS_OBJECT(v->surface_bc)->klass)->bc)(
        cell, v->surface_bc);
...
#else  /* 3D */
    g->x +=
        (m[0][i] + m[3][i] * cm.y + m[4][i] * cm.z + m[6][i] * cm.y * cm.z) *
        val;
    g->y +=
        (m[1][i] + m[3][i] * cm.x + m[5][i] * cm.z + m[6][i] * cm.x * cm.z) *
        val;
    g->z +=
        (m[2][i] + m[4][i] * cm.x + m[5][i] * cm.y + m[6][i] * cm.x * cm.y) *
        val;
#endif /* 3D */
  }
}
...
#else  /* 3D */
  p.z = (p.z - o->z) / h;
  return (a[0] * p.x + a[1] * p.y + a[2] * p.z + a[3] * p.x * p.y +
          a[4] * p.x * p.z + a[5] * p.y * p.z + a[6] * p.x * p.y * p.z + v0);
#endif /* 3D */
}
...
/* gfs_cell_dirichlet_gradient_flux_stencil needs to be updated whenever
   this function is modified  */
gdouble gfs_cell_dirichlet_gradient_flux(FttCell *cell, guint v, gint max_level,
                                         gdouble v0) {
  g_return_val_if_fail(cell != NULL, 0.);
...
static void gfs_cell_dirichlet_gradient_stencil(FttCell *cell, gint max_level,
                                                gdouble v0, FttVector *grad,
                                                GfsLinearProblem *lp,
                                                GfsStencil *      stencil,
                                                FttVector *       weight) {
  if (GFS_IS_MIXED(cell)) {
    ...
    for (i = 0; i < N_CELLS - 1; i++)
      for (c = 0; c < FTT_DIMENSION; c++) {
        gfs_stencil_add_element(stencil, n[i + 1], lp,
                                m[c][i] * (&weight->x)[c]);
 ...
 * boundary contained in @cell.
 */
gdouble gfs_cell_dirichlet_gradient_flux_stencil(FttCell *cell, gint max_level,
                                                 gdouble           v0,
                                                 GfsLinearProblem *lp,
                                                 GfsStencil *      stencil) {
  g_return_val_if_fail(cell != NULL, 0.);
  ...
  if (!GFS_IS_MIXED(cell))
    ...
    GfsSolidVector *s = GFS_STATE(cell)->solid;
    FttVector       g;
    gfs_cell_dirichlet_gradient_stencil(cell, max_level, v0, &g, lp, stencil,
                                        &s->v);
    return g.x * s->v.x + g.y * s->v.y + g.z * s->v.z;
  }
}
 ...
 */
gdouble gfs_cell_dirichlet_value(FttCell *cell, GfsVariable *v,
                                 gint max_level) {
  g_return_val_if_fail(cell != NULL, 0.);
  g_return_val_if_fail(v != NULL, 0.);
  ...
  if (!GFS_IS_MIXED(cell))
    ...
    void (*cell_pos)(const FttCell *, FttVector *) =
        v->centered ? ftt_cell_pos : gfs_cell_cm;
    guint i, j;
 ...
 */
void gfs_shear_strain_rate_tensor(FttCell *cell, GfsVariable **u,
 ...
 */
GtsRange gfs_stats_variable(FttCell *root, GfsVariable *v,
 ...
 */
GfsNorm gfs_norm_variable(FttCell *root, GfsVariable *v, FttTraverseFlags flags,
                          gint max_depth) {
  GfsNorm  n;
  gpointer data[2];
...
gdouble gfs_face_interpolated_value(const FttCellFace *face, guint v) {
  ...
  if (f2.neighbor) {
    ...
    return v0 +
           (x2 * (v1 - v0) * (1. + 2. * x2) - x1 * (v0 - v2) * (1. - 2. * x1)) /
               (4. * x1 * x2 * (x1 + x2));
  } else
...
gdouble gfs_face_interpolated_value_generic(const FttCellFace *face,
  ...
  if (!face->neighbor || FTT_CELL_IS_LEAF(face->neighbor) ||
    ...
    for (i = 0; i < n; i++)
      if (child.c[i]) {
        f.cell = child.c[i];
        avg += gfs_face_interpolated_value(&f, v->i) *
               gfs_domain_face_fraction(v->domain, &f);
      }
    return avg == 0. ? 0.
                     : avg / (gfs_domain_face_fraction(v->domain, face) * n);
  }
}
...
void gfs_normal_divergence(FttCell *cell, GfsVariable *v) {
  ...
  for (face.d = 0; face.d < FTT_NEIGHBORS; face.d++)
    div += (FTT_FACE_DIRECT(&face) ? 1. : -1.) * GFS_STATE(cell)->f[face.d].un *
           gfs_domain_face_fraction(v->domain, &face);
  ...
  for (f.d = 0; f.d < FTT_NEIGHBORS; f.d++)
    if (n.c[f.d]) {
      ...
      div += gfs_domain_face_fraction(domain, &f) *
             ...
             gfs_face_interpolated_value_generic(&f, v[f.d / 2]);
    }
...
static void cell_traverse_mixed(FttCell *cell, FttTraverseType order,
 ...
 */
void gfs_cell_traverse_mixed(FttCell *root, FttTraverseType order,
...
#if FTT_2D
static FttDirection corner[4][FTT_DIMENSION] = {{FTT_LEFT, FTT_BOTTOM},
 ...
 */
void gfs_cell_corner_values(FttCell *cell, GfsVariable *v, gint max_level,
...
gdouble gfs_interpolate_from_corners(FttCell *cell, FttVector p, gdouble *f) {
  ...
  return (c[0] * p.x + c[1] * p.y + c[2] * p.z + c[3] * p.x * p.y +
          c[4] * p.x * p.z + c[5] * p.y * p.z + c[6] * p.x * p.y * p.z + c[7]) /
         8.;
#endif /* 3D */
}
...
static FttCell *cell_corner_neighbor(FttCell *cell, FttDirection *d,
                                     gint max_level, gboolean *t_junction) {
  FttCell *neighbor = ftt_cell_neighbor(cell, d[0]);
  if (!neighbor)
    return NULL;
  else {
...
static void t_junction_interpolator(FttCell *cell, FttDirection *d, FttCell *n,
                                    gint max_level, gboolean centered,
                                    GfsInterpolator *inter) {
  FttDirection    d1[FTT_DIMENSION];
  GfsInterpolator a;
...
static gboolean do_path(FttCell *cell, gint i, FttCell *n[N_CELLS],
                        FttDirection *d, gint max_level, gboolean centered,
                        GfsInterpolator *inter) {
  ...
  for (j = 0; j < FTT_DIMENSION; j++) {
    ...
    if (n[k] == NULL) {
      ...
      for (l = 0; l < FTT_DIMENSION; l++)
        d1[l] = path[i][j][l] < 0
                    ? FTT_OPPOSITE_DIRECTION(d[-path[i][j][l] - 1])
                    : d[path[i][j][l] - 1];
 ...
 */
void gfs_cell_corner_interpolator(FttCell *cell, FttDirection d[FTT_DIMENSION],
                                  gint max_level, gboolean centered,
                                  GfsInterpolator *inter) {
  FttCell *n[N_CELLS];
  guint    i;
  ...
  while (!FTT_CELL_IS_LEAF(cell) && ftt_cell_level(cell) != max_level &&
 ...
 * @cell defined by @d.
 */
gdouble gfs_cell_corner_value(FttCell *cell, FttDirection d[FTT_DIMENSION],
                              GfsVariable *v, gint max_level) {
  GfsInterpolator inter;
  gdouble         val = 0.;
  guint           i;
 ...
 */
GfsStencil *gfs_stencil_new(FttCell *cell, GfsLinearProblem *lp,
                            gdouble coeff) {
 ...
 */
void gfs_stencil_add_element(GfsStencil *stencil, FttCell *cell,
                             GfsLinearProblem *lp, gdouble coeff) {
  gint i;
      ...
      *x = 3. / 4.;
      for (i = 0; i < n; i++)
        if (children.c[i]) {
          ...
          gfs_stencil_add_element(stencil, children.c[i], lp,
                                  weight * w / a / (*x));
        }
    } else
 ...
 * First order 1D interpolation.
 */
static GfsGradient get_interpolate_1D1_stencil(FttCell *cell, FttDirection d,
                                               gdouble x, GfsLinearProblem *lp,
                                               GfsStencil *stencil,
                                               gdouble     weight) {
  GfsGradient p;
 ...
 * First order 2D interpolation.
 */
static GfsGradient get_interpolate_2D1_stencil(FttCell *cell, FttDirection d1,
                                               FttDirection d2, gdouble x,
                                               gdouble y, GfsLinearProblem *lp,
                                               GfsStencil *stencil,
                                               gdouble     weight) {
  GfsGradient p;
  gdouble     y1 = 1.;
...
#if (FTT_2D || FTT_2D3)
  g_assert(dp >= 0);
  p = get_interpolate_1D1_stencil(face->neighbor, dp, 1. / 4., lp, stencil,
                                  2. / 3. * weight);
#else  /* FTT_3D */
  g_assert(dp[0] >= 0 && dp[1] >= 0);
  p = get_interpolate_2D1_stencil(face->neighbor, dp[0], dp[1], 1. / 4.,
                                  1. / 4., lp, stencil, 2. / 3. * weight);
#endif /* FTT_3D */
 ...
 */
void gfs_face_weighted_gradient_stencil(const FttCellFace *face, GfsGradient *g,
                                        gint max_level, GfsLinearProblem *lp,
                                        GfsStencil *stencil) {
  face_weighted_gradient_stencil(face, g, max_level, FTT_DIMENSION, lp,
                                 stencil);
}

static gboolean mixed_face_gradient_stencil(const FttCellFace *face,
                                            Gradient *g, gint max_level,
                                            GfsLinearProblem *lp,
                                            GfsStencil *      stencil,
                                            gdouble           weight) {
  FttCell *    n[N_CELLS];
...
static void face_cm_gradient_stencil(const FttCellFace *face, GfsGradient *g,
                                     gint max_level, gboolean weighted,
                                     GfsLinearProblem *lp,
                                     GfsStencil *      stencil) {
  guint    level;
  ...
  if (ftt_cell_level(face->neighbor) < level) {
    ...
    if (level == max_level || FTT_CELL_IS_LEAF(face->neighbor)) {
      ...
      if (!GFS_IS_MIXED(face->cell) && !GFS_IS_MIXED(face->neighbor)) {
        g->a = w;
        gfs_stencil_add_element(stencil, face->neighbor, lp, w);
      } else if (mixed_face_gradient_stencil(face, &gcf, max_level, lp, stencil,
                                             w)) {
        g->a = w * gcf.a;
        gfs_stencil_add_element(stencil, face->neighbor, lp, w * gcf.b);
      } else {
      ...
      for (i = 0; i < n; i++)
        if ((f.cell = children.c[i])) {
          w = weighted ? GFS_STATE(f.cell)->f[f.d].v : 1.;
          if (GFS_IS_MIXED(f.cell) || GFS_IS_MIXED(f.neighbor)) {
            if (!mixed_face_gradient_stencil(&f, &gcf, max_level, lp, stencil,
                                             w))
              gcf = gradient_fine_coarse_stencil(&f, lp, stencil, -w);
          } else
            gcf = gradient_fine_coarse_stencil(&f, lp, stencil, -w);
