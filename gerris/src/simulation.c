...
static void simulation_destroy(GtsObject *object) {
  ...
  gts_container_foreach(GTS_CONTAINER(sim->maps), (GtsFunc)gts_object_destroy,
                        NULL);
...
static void simulation_write(GtsObject *object, FILE *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_simulation_class())->parent_class->write)(object, fp);
  ...
  while (i) {
    ...
    fprintf(fp, "  GModule %s",
            g_module_symbol(i->data, "gfs_module_name", (gpointer)&name)
                ? name
  ...
  if (!g_module_supported()) {
    ...
    if (module == NULL) {
      ...
      if (name[0] != G_DIR_SEPARATOR)
        path = g_strconcat(GFS_MODULES_DIR, G_DIR_SEPARATOR_S, "lib", name,
                           MODULES_SUFFIX, NULL);
      else
        path = g_strdup(name);
#else
...
static void simulation_read(GtsObject **object, GtsFile *fp) {
  ...
  while (fp->type != GTS_ERROR && fp->type != '}') {
    ...
    /* ------------ GfsAdvectionParams ------------ */
      ...
      if (sim->advection_params.linear) {
        sim->u0[0] = gfs_domain_get_or_add_variable(
            domain, "U0", "x-component of the base velocity");
        sim->u0[0]->units = 1.;
        sim->u0[1]        = gfs_domain_get_or_add_variable(
            domain, "V0", "y-component of the base velocity");
        sim->u0[1]->units = 1.;
#if (!FTT_2D)
        sim->u0[2] = gfs_domain_get_or_add_variable(
            domain, "W0", "z-component of the base velocity");
        sim->u0[2]->units = 1.;
#endif /* FTT_3D */
  ...
  if (sim->advection_params.scheme == GFS_NONE)
    ...
    /* we need to add a pseudo-Coriolis term to include the metric advection
     * terms */
  }
}
...
void gfs_advance_tracers(GfsSimulation *sim, gdouble dt) {
  ...
  while (i) {
    if (GFS_IS_VARIABLE_TRACER_VOF(i->data)) {
      ...
      gfs_domain_variable_centered_sources(domain, i->data, i->data,
                                           t->advection.dt);
...
static void simulation_run(GfsSimulation *sim) {
  ...
  if (sim->time.i == 0) {
    gfs_approximate_projection(domain, &sim->approx_projection_params,
                               sim->advection_params.dt, p,
                               sim->physical_params.alpha, res, g, NULL);
    gfs_simulation_set_timestep(sim);
  ...
  while (sim->time.t < sim->time.end && sim->time.i < sim->time.iend) {
    ...
    if (sim->advection_params.linear) {
      ...
      gfs_domain_face_traverse(
          domain, FTT_XYZ, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
          (FttFaceTraverseFunc)gfs_face_reset_normal_velocity, NULL);
      gfs_domain_face_traverse(
          domain, FTT_XYZ, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
          (FttFaceTraverseFunc)gfs_face_interpolated_normal_velocity, sim->u0);
    } else
      gfs_predicted_face_velocities(domain, FTT_DIMENSION,
                                    &sim->advection_params);

    gfs_variables_swap(p, pmac);
    gfs_mac_projection(domain, &sim->projection_params,
                       sim->advection_params.dt / 2., p,
                       sim->physical_params.alpha, gmac, NULL);
    gfs_variables_swap(p, pmac);

    gts_container_foreach(GTS_CONTAINER(sim->events),
                          (GtsFunc)gfs_event_half_do, sim);

    gfs_centered_velocity_advection_diffusion(
        domain, FTT_DIMENSION, &sim->advection_params, gmac,
        sim->time.i > 0 || !gc ? gc : gmac, sim->physical_params.alpha);
    if (gc) {
      gfs_source_coriolis_implicit(domain, sim->advection_params.dt);
      gfs_correct_centered_velocities(domain, FTT_DIMENSION,
                                      sim->time.i > 0 ? gc : gmac,
                                      -sim->advection_params.dt);
    } else if (gfs_has_source_coriolis(domain)) {
      gfs_correct_centered_velocities(domain, FTT_DIMENSION, gmac,
                                      sim->advection_params.dt);
      gfs_source_coriolis_implicit(domain, sim->advection_params.dt);
      gfs_correct_centered_velocities(domain, FTT_DIMENSION, gmac,
                                      -sim->advection_params.dt);
...
static gdouble simulation_cfl(GfsSimulation *sim) {
  ...
  while (i) {
    ...
    if (GFS_IS_VARIABLE_TRACER(v) &&
      ...
      gfs_add_sinking_velocity(GFS_DOMAIN(sim),
                               &GFS_VARIABLE_TRACER(v)->advection);
      gdouble cfl = gfs_domain_cfl(GFS_DOMAIN(sim), FTT_TRAVERSE_LEAFS, -1);
      gfs_remove_sinking_velocity(GFS_DOMAIN(sim),
                                  &GFS_VARIABLE_TRACER(v)->advection);
      if (cfl < cflmin)
        cflmin = cfl;
    }
  ...
  return cflmin < G_MAXDOUBLE
             ? cflmin
             : gfs_domain_cfl(GFS_DOMAIN(sim), FTT_TRAVERSE_LEAFS, -1);
}
...
static gdouble cell_vorticity(FttCell *cell, FttCellFace *face,
                              GfsDomain *domain) {
  return gfs_vorticity(cell, gfs_domain_velocity(domain));
}

static gdouble cell_divergence(FttCell *cell, FttCellFace *face,
                               GfsDomain *domain) {
  return gfs_divergence(cell, gfs_domain_velocity(domain));
}

static gdouble cell_velocity_norm(FttCell *cell, FttCellFace *face,
                                  GfsDomain *domain) {
  gdouble L = GFS_SIMULATION(domain)->physical_params.L;
  return L * gfs_vector_norm(cell, gfs_domain_velocity(domain));
}

static gdouble cell_velocity_norm2(FttCell *cell, FttCellFace *face,
                                   GfsDomain *domain) {
  gdouble L = GFS_SIMULATION(domain)->physical_params.L;
  return L * L * gfs_vector_norm2(cell, gfs_domain_velocity(domain));
}

static gdouble cell_level(FttCell *cell) { return ftt_cell_level(cell); }
...
static gdouble cell_solid_area(FttCell *cell, FttCellFace *face,
                               GfsDomain *domain) {
  FttVector n;
  gfs_solid_normal(cell, &n);
  return ftt_vector_norm(&n) *
         pow(ftt_cell_size(cell) * GFS_SIMULATION(domain)->physical_params.L,
             FTT_DIMENSION - 1);
}
...
static gdouble cell_velocity_lambda2(FttCell *cell, FttCellFace *face,
                                     GfsDomain *domain) {
  return gfs_vector_lambda2(cell, gfs_domain_velocity(domain));
}

static gdouble cell_streamline_curvature(FttCell *cell, FttCellFace *face,
                                         GfsDomain *domain) {
  gdouble L = GFS_SIMULATION(domain)->physical_params.L;
  return gfs_streamline_curvature(cell, gfs_domain_velocity(domain)) / L;
}

static gdouble cell_2nd_principal_invariant(FttCell *cell, FttCellFace *face,
                                            GfsDomain *domain) {
  return gfs_2nd_principal_invariant(cell, gfs_domain_velocity(domain)) /
         ftt_cell_size(cell);
}
...
static gdouble cell_orthogonality(FttCell *cell, FttCellFace *face,
                                  GfsDomain *domain) {
  gdouble   h;
  FttVector p;
  if (cell) {
  ...
  return fabs(acos(gts_vector_scalar(p1p2, p3p4) /
                   (gts_vector_norm(p1p2) * gts_vector_norm(p3p4))) -
              M_PI / 2.) *
         180. / M_PI;
}
...
static void simulation_init(GfsSimulation *object) {
  ...
  static GfsDerivedVariableInfo derived_variable[] = {
    ...
    {"dt", "Timestep", cell_dt},
    {"Vorticity", "Norm of the vorticity vector of the velocity field",
     cell_vorticity},
...
#endif /* 3D */
    {"Lambda2", "Vortex-detection criterion of Jeong & Hussein",
     cell_velocity_lambda2},
    {"Curvature", "Curvature of the local streamline",
     cell_streamline_curvature},
    {"D2", "Second principal invariant of the deformation tensor",
     cell_2nd_principal_invariant},
  ...
  object->solids = GTS_SLIST_CONTAINER(
      gts_container_new(GTS_CONTAINER_CLASS(gts_slist_container_class())));
  object->output_solid = TRUE;

  object->refines = GTS_SLIST_CONTAINER(
      gts_container_new(GTS_CONTAINER_CLASS(gts_slist_container_class())));
  object->maps = GTS_SLIST_CONTAINER(
      gts_container_new(GTS_CONTAINER_CLASS(gts_slist_container_class())));
  object->adapts = GTS_SLIST_CONTAINER(
      gts_container_new(GTS_CONTAINER_CLASS(gts_slist_container_class())));
  gfs_adapt_stats_init(&object->adapts_stats);
  object->events = GTS_SLIST_CONTAINER(
      gts_container_new(GTS_CONTAINER_CLASS(gts_slist_container_class())));
...
void gfs_simulation_init(GfsSimulation *sim) {
  ...
  gts_container_foreach(GTS_CONTAINER(sim->events), (GtsFunc)init_non_variable,
                        sim);
  ...
  if (domain->timer->start >=
      0.) /* only if timing is on (i.e. within gfs_simulation_run()) */
    while (gfs_simulation_adapt(sim)) {
      gts_container_foreach(GTS_CONTAINER(sim->events),
                            (GtsFunc)redo_non_variable, sim);
      i = domain->variables;
      while (i) {
      ...
      gfs_domain_cell_traverse(domain, FTT_POST_ORDER, FTT_TRAVERSE_NON_LEAFS,
                               -1, (FttCellTraverseFunc)gfs_cell_coarse_init,
                               domain);
...
static void check_solid_fractions(GfsBox *box, guint *nf) {
  ...
  for (d = 0; d < FTT_NEIGHBORS; d++)
    ftt_face_traverse_boundary(box->root, d, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                               -1, (FttFaceTraverseFunc)check_face, nf);
}
...
guint gfs_check_solid_fractions(GfsDomain *domain) {
  ...
  guint nf = 0;
  gts_container_foreach(GTS_CONTAINER(domain), (GtsFunc)check_solid_fractions,
                        &nf);
  return nf;
}
...
static void set_permanent(FttCell *cell) { cell->flags |= GFS_FLAG_PERMANENT; }
...
static gboolean coarsen_cell(void) { return TRUE; }
...
static void refine_leaf_boxes(GfsBox *box, GfsDomain *domain) {
  ...
  if (FTT_CELL_IS_LEAF(box->root)) {
    ftt_cell_refine_single(box->root, domain->cell_init,
                           domain->cell_init_data);
    gfs_cell_coarse_init(box->root, domain);
    ftt_cell_coarsen(box->root, (FttCellCoarsenFunc)coarsen_cell, NULL,
...
void gfs_simulation_refine(GfsSimulation *sim) {
  ...
  if (solids) {
    ...
    sim->thin = gfs_domain_init_solid_fractions(
        domain, solids, TRUE, (FttCellCleanupFunc)gfs_cell_cleanup, domain,
        NULL);
    g_slist_free(solids);
    gfs_domain_match(domain);
...
GfsSimulation *gfs_simulation_read(GtsFile *fp) {
  ...
  while (fp->type == GTS_STRING &&
         !strcmp(fp->token->str, "GModule")) { /* preloaded module */
...
static void count_edges(GtsGEdge *e, guint *nedge) { (*nedge)++; }
...
static void write_edge(GtsGEdge *edge, FILE *fp) {
  fprintf(fp, "%u %u", GPOINTER_TO_UINT(GTS_OBJECT(edge->n1)->reserved),
...
void gfs_simulation_union_write(GfsSimulation *sim, gint max_depth, FILE *fp) {
  ...
  if (domain->pid < 0)
    ...
    /* see gts/src/graph.c:gts_graph_write() for the original (serial)
     * implementation */
    ...
    gts_container_foreach(GTS_CONTAINER(g), (GtsFunc)gts_object_reset_reserved,
                          NULL);
#endif /* HAVE_MPI */
  }
}
...
static gdouble min_cfl(GfsSimulation *sim) {
  gdouble cfl =
      (sim->advection_params.scheme == GFS_NONE ? G_MAXDOUBLE
...
void gfs_simulation_set_timestep(GfsSimulation *sim) {
  ...
  if ((cfl = min_cfl(sim)) < G_MAXDOUBLE)
    sim->advection_params.dt =
        cfl * (*GFS_SIMULATION_CLASS(GTS_OBJECT(sim)->klass)->cfl)(sim);
  else
    ...
    if (v->sources) {
      ...
      while (j) {
        ...
        if (GFS_SOURCE_GENERIC_CLASS(GTS_OBJECT(s)->klass)->stability) {
          gdouble dt =
              (*GFS_SOURCE_GENERIC_CLASS(GTS_OBJECT(s)->klass)->stability)(s,
                                                                           sim);
  ...
  gfs_all_reduce(GFS_DOMAIN(sim), sim->advection_params.dt, MPI_DOUBLE,
                 MPI_MIN);
...
void gfs_time_read(GfsTime *t, GtsFile *fp) {
  GtsFileVariable var[] = {
      {GTS_DOUBLE, "t", TRUE},     {GTS_DOUBLE, "start", TRUE},
      {GTS_DOUBLE, "end", TRUE},   {GTS_UINT, "i", TRUE},
      {GTS_UINT, "istart", TRUE},  {GTS_UINT, "iend", TRUE},
      {GTS_DOUBLE, "dtmax", TRUE}, {GTS_NONE}};
 ...
 */
void gfs_physical_params_read(GfsPhysicalParams *p, GfsDomain *domain,
                              GtsFile *fp) {
  g_return_if_fail(p != NULL);
...
void gfs_simulation_run(GfsSimulation *sim) {
  ...
  guint id = g_log_set_handler(
      "Gfs", G_LOG_LEVEL_ERROR | G_LOG_FLAG_FATAL | G_LOG_FLAG_RECURSION,
      error_handler, sim);
  GfsDomain *domain = GFS_DOMAIN(sim);
 ...
 */
void gfs_simulation_map_vector(GfsSimulation *sim, const FttVector *p,
                               FttVector *v) {
  g_return_if_fail(sim != NULL);
  g_return_if_fail(p != NULL);
 ...
 * location @p, to vector @v.
 */
void gfs_simulation_map_inverse_vector(GfsSimulation *sim, const FttVector *p,
                                       FttVector *v) {
  g_return_if_fail(sim != NULL);
  g_return_if_fail(p != NULL);
  g_return_if_fail(v != NULL);
...
static void advection_run(GfsSimulation *sim) {
  ...
  while (sim->time.t < sim->time.end && sim->time.i < sim->time.iend) {
    ...
    gts_container_foreach(GTS_CONTAINER(sim->events), (GtsFunc)event_do_adapt,
                          sim);
    ...
    gts_container_foreach(GTS_CONTAINER(sim->events),
                          (GtsFunc)event_do_not_adapt, sim);
...
static void add_ddiv(FttCell *cell, DivData *p) {
  ...
  GFS_VALUE(cell, p->div) +=
      size * size * p->ddiv * gfs_domain_cell_fraction(p->div->domain, cell);
}
...
static void correct_div(GfsDomain *domain, GfsVariable *divu, GfsVariable *div,
  ...
  if (!dirichlet) {
    ...
    p.ddiv = -gfs_domain_stats_variable(domain, div, FTT_TRAVERSE_LEAFS, -1,
                                        NULL, NULL)
                  .mean /
             p.vol.mean;
    gfs_domain_cell_traverse(domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
...
static void poisson_run(GfsSimulation *sim) {
  ...
  if (!dirichlet) {
    ...
    CompatPar q = {p, FALSE};
    gts_container_foreach(GTS_CONTAINER(domain), (GtsFunc)check_box_dirichlet,
                          &q);
  ...
  while (sim->time.i < sim->time.iend && sim->time.t < sim->time.end) {
    ...
    correct_div(domain, gfs_variable_from_name(domain->variables, "Div"), div,
                dirichlet);
    gfs_poisson_coefficients(domain, sim->physical_params.alpha, FALSE,
                             p->centered, TRUE);
...
static void axi_read(GtsObject **object, GtsFile *fp) {
  ...
  if (w) {
    ...
    w->component =
        FTT_Y; /* to include the appropriate diffusion metric terms */
  }
}

static gdouble axi_face_metric(const GfsDomain *  domain,
...
static void axi_solid_metric(const GfsDomain *domain, const FttCell *cell,
                             FttVector *m) {
  g_assert(GFS_IS_MIXED(cell));
  m->x = m->y = GFS_STATE(cell)->solid->ca.y;
}

static gdouble axi_scale_metric(const GfsDomain *domain, const FttCell *cell,
                                FttComponent c) {
  if (c < 2)
    return 1.;
  FttVector p;
...
GfsSimulationClass *gfs_axi_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_axi_info = {"GfsAxi",
                                       sizeof(GfsSimulation),
                                       sizeof(GfsSimulationClass),
