...
static void gfs_source_tension_generic_destroy(GtsObject *o) {
  ...
  (*GTS_OBJECT_CLASS(gfs_source_tension_generic_class())
...
static void gfs_source_tension_generic_read(GtsObject **o, GtsFile *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_source_tension_generic_class())->parent_class->read)(
      o, fp);
  ...
  if ((s->c = gfs_variable_from_name(domain->variables, fp->token->str)) ==
      NULL) {
...
static void gfs_source_tension_generic_write(GtsObject *o, FILE *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_source_tension_generic_class())->parent_class->write)(
      o, fp);
...
static void interface_level(FttCell *cell, StabilityParams *p) {
  ...
  if (level > p->depth && GFS_VALUE(cell, p->c) > 1e-3 &&
...
static gdouble gfs_source_tension_generic_stability(GfsSourceGeneric *s,
  ...
  gfs_domain_cell_traverse(GFS_DOMAIN(sim), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                           -1, (FttCellTraverseFunc)min_max_alpha, &p);
...
static void
gfs_source_tension_generic_class_init(GfsSourceGenericClass *klass) {
...
static void gfs_source_tension_css_read(GtsObject **o, GtsFile *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_source_tension_css_class())->parent_class->read)(o,
                                                                          fp);
  if (fp->type == GTS_ERROR)
    return;
...
static void foreach_cell_normal(FttCell *cell, GfsSourceTensionCSS *s) {
  ...
  gdouble   nn = 0.;
  gdouble   sigh =
      gfs_function_value(GFS_SOURCE_TENSION_GENERIC(s)->sigma, cell) /
      ftt_cell_size(cell);
  FttComponent c;
...
static void foreach_cell_tension_css(FttCell *cell, GfsSourceTensionCSS *s) {
  ...
  gdouble        alpha = sim->physical_params.alpha
                      ? gfs_function_value(sim->physical_params.alpha, cell)
                      : 1.;
...
static gboolean gfs_source_tension_css_event(GfsEvent *     event,
                                             GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_source_tension_css_class())->parent_class)
            ->event)(event, sim)) {
    ...
    gfs_domain_cell_traverse(GFS_DOMAIN(sim), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             -1, (FttCellTraverseFunc)foreach_cell_normal,
                             event);
    /* fixme: boundary conditions for normal */
    gfs_domain_cell_traverse(GFS_DOMAIN(sim), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             -1, (FttCellTraverseFunc)foreach_cell_tension_css,
                             event);
    for (i = 0; i < 3; i++)
...
static gdouble gfs_source_tension_css_value(GfsSourceGeneric *s, FttCell *cell,
                                            GfsVariable *v) {
...
static void gfs_source_tension_read(GtsObject **o, GtsFile *fp) {
  ...
  if ((s->k = gfs_variable_from_name(domain->variables, fp->token->str)) ==
      NULL) {
    gts_file_error(fp, "unknown variable `%s'", fp->token->str);
    return;
  }
...
static gdouble gfs_source_tension_stability(GfsSourceGeneric *s,
  ...
  if (GFS_IS_VARIABLE_POSITION(GFS_SOURCE_TENSION(s)->k)) {
    ...
    p.sigma           = 0.;
    gfs_domain_cell_traverse(GFS_DOMAIN(sim), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             -1, (FttCellTraverseFunc)interface_level, &p);
    return p.sigma > 0. ? sqrt(ftt_level_size(p.depth) / fabs(p.sigma))
                        : G_MAXDOUBLE;
  } else
    /* surface tension */
...
static void variable_curvature_read(GtsObject **o, GtsFile *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_variable_curvature_class())->parent_class->read)(o,
                                                                          fp);
  if (fp->type == GTS_ERROR)
    return;
  ...
  if (GFS_IS_VARIABLE_TRACER(v->f)) {
    ...
    GFS_VARIABLE(v)->description = g_strjoin(
        " ", "Curvature of the interface defined by tracer", v->f->name, NULL);
    gts_file_next_token(fp);
    if (fp->type == GTS_STRING) {
      v->kmax = gfs_domain_get_or_add_variable(domain, fp->token->str,
                                               "Maximum curvature");
      if (v->kmax) {
    ...
    GFS_VARIABLE(v)->description =
        g_strjoin(" ", "Curvature of the interface defined by distance",
                  v->f->name, NULL);
    gts_file_next_token(fp);
  } else {
    gts_file_error(fp, "variable `%s' is neither a tracer nor a distance",
                   fp->token->str);
    return;
  }
...
static void variable_curvature_write(GtsObject *o, FILE *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_variable_curvature_class())->parent_class->write)(o,
                                                                           fp);
...
static void height_curvature(FttCell *cell, GfsVariable *v) {
  ...
  if (GFS_IS_FULL(f)) {
    ...
    if (kmax) {
      gdouble k;
      GFS_VALUE(cell, v) =
          gfs_height_curvature(cell, GFS_VARIABLE_TRACER_VOF(t), &k);
      GFS_VALUE(cell, kmax) = k;
    } else
      GFS_VALUE(cell, v) =
          gfs_height_curvature(cell, GFS_VARIABLE_TRACER_VOF(t), NULL);
  }
}
...
static void fit_curvature(FttCell *cell, GfsVariable *v) {
  ...
  if (!GFS_IS_FULL(f) && !GFS_HAS_DATA(cell, v)) {
    ...
    if (kmax) {
      gdouble k;
      GFS_VALUE(cell, v) =
          gfs_fit_curvature(cell, GFS_VARIABLE_TRACER_VOF(t), &k);
      GFS_VALUE(cell, kmax) = k;
    } else
      GFS_VALUE(cell, v) =
          gfs_fit_curvature(cell, GFS_VARIABLE_TRACER_VOF(t), NULL);
  }
}
...
static void variable_curvature_diffuse(GfsVariable *v, GfsVariable *f,
  ...
  FttCellTraverseFunc diff =
      f ? (FttCellTraverseFunc)diffuse_kmax : (FttCellTraverseFunc)diffuse;
  DiffuseParms p;
  p.v   = v;
  p.f   = f;
...
static void variable_curvature_from_fraction(GfsEvent *     event,
                                             GfsSimulation *sim) {
  GfsDomain *  domain = GFS_DOMAIN(sim);
...
static void variable_curvature_from_distance(GfsEvent *     event,
                                             GfsSimulation *sim) {
  ...
  gfs_domain_copy_bc(domain, FTT_TRAVERSE_LEAFS, -1, GFS_VARIABLE(event),
                     n[FTT_DIMENSION]);
  gfs_domain_cell_traverse(domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
...
static void set_curvature(FttCell *cell, gdouble kappa, gdouble kmax,
                          CurvatureData *p) {
...
static void propagate_curvature(FttCell *cell, gdouble kappa, gdouble kmax,
                                CurvatureData *p) {
  ...
  for (d = 2 * p->c; d <= 2 * p->c + 1; d++) {
    ...
    while (n && ftt_cell_level(n) == level && is_interfacial(n, p->k->f) &&
...
static void remaining_curvatures(FttCell *cell, GfsVariable *v) {
  if (!GFS_HAS_DATA(cell, v)) {
    GfsVariableTracerVOFHeight *t =
        GFS_VARIABLE_TRACER_VOF_HEIGHT(GFS_VARIABLE_CURVATURE(v)->f);
    GfsVariable *kmax = GFS_VARIABLE_CURVATURE(v)->kmax;
    if (kmax) {
      gdouble k;
...
static gboolean variable_curvature_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_variable_curvature_class())->parent_class)
            ->event)(event, sim)) {
    if (GFS_IS_VARIABLE_TRACER_VOF_HEIGHT(GFS_VARIABLE_CURVATURE(event)->f)) {
      ...
      gfs_domain_traverse_leaves(domain, (FttCellTraverseFunc)set_undefined,
                                 &p);
      for (p.c = 0; p.c < FTT_DIMENSION; p.c++)
        gfs_domain_cell_traverse_condition(
            domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
            (FttCellTraverseFunc)height_curvature_max, &p, is_interfacial,
            GFS_VARIABLE_CURVATURE(event)->f);
      gts_object_destroy(GTS_OBJECT(p.max));
      gfs_domain_cell_traverse_condition(
          domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
          (FttCellTraverseFunc)remaining_curvatures, event, is_interfacial,
          GFS_VARIABLE_CURVATURE(event)->f);
      ...
      if (kmax) {
        gfs_domain_cell_traverse(domain, FTT_POST_ORDER, FTT_TRAVERSE_NON_LEAFS,
                                 -1, (FttCellTraverseFunc)kmax->fine_coarse,
                                 kmax);
        gfs_domain_bc(domain, FTT_TRAVERSE_LEAFS, -1, kmax);
        variable_curvature_diffuse(kmax, GFS_VARIABLE_CURVATURE(event)->f, sim,
                                   1);
      }
      variable_curvature_diffuse(GFS_VARIABLE(event), NULL, sim, 1);
...
static void variable_position_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GFS_VARIABLE(v)->description)
    g_free(GFS_VARIABLE(v)->description);
  GFS_VARIABLE(v)->description = g_strjoin(
      " ", fp->token->str, "coordinate of the interface defined by tracer",
...
static void variable_position_write(GtsObject *o, FILE *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_variable_position_class())->parent_class->write)(o,
                                                                          fp);
...
static void position(FttCell *cell, GfsVariable *v) {
  ...
  if (gfs_vof_center(cell,
                     GFS_VARIABLE_TRACER_VOF(GFS_VARIABLE_CURVATURE(v)->f), &p))
    GFS_VALUE(cell, v) =
        (&p.x)[GFS_VARIABLE_POSITION(v)->c] - GFS_VARIABLE_POSITION(v)->ref;
  else
    GFS_VALUE(cell, v) = GFS_NODATA;
}

static gboolean variable_position_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_variable_class())->parent_class)
            ->event)(event, sim)) {
    GfsDomain *domain = GFS_DOMAIN(sim);
...
static void variable_position_init(GfsVariable *v) { v->units = 1.; }
...
GfsVariableClass *gfs_variable_position_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_variable_position_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass =
        gts_object_class_new(GTS_OBJECT_CLASS(gfs_variable_curvature_class()),
