...
static void face_fractions(CellFace *f, GfsSolidVector *solid, FttVector *h) {
  ...
  for (k = m; k < m + 4; k++) {
    ...
    gdouble x1 = f->p[i].x - x0, y1 = f->p[i].y - y0, x2 = f->p[i1].x - x0,
            y2 = f->p[i1].y - y0;
    if (f->s[i].n > 0) {
      ...
      if (o == 2) {
        ...
        solid->a += (x1 + x2) * (y2 - y1);
        solid->cm.x +=
            (x1 - x2) * (2. * (x1 * y1 + x2 * y2) + x1 * y2 + x2 * y1);
        solid->cm.y +=
            (y2 - y1) * (2. * (x1 * y1 + x2 * y2) + x1 * y2 + x2 * y1);
        solid->ca.x = (x1 + x2) / 2.;
        solid->ca.y = (y1 + y2) / 2.;
      }
      ins = !ins;
...
static void face_new(CellFace *f, FttCell *cell, GfsGenericSurface *s,
                     FttVector *h) {
  FttVector p;
  guint     i;
...
static void cube_new(CellCube *cube, FttCell *cell, GfsGenericSurface *s,
                     FttVector *o, FttVector *h) {
  guint i;
...
static void set_solid_fractions_from_surface(FttCell *          cell,
  ...
  for (i = 0; i < 12; i++) { /* for each edge of the cube */
    ...
    if (cube.s[i].n % 2 != 0) { /* only for odd number of intersections */
      ...
      /* intersection vertex position is the average of all the n[i]
       * intersections */
      s->x /= s->n;
...
static void push_leaf(GtsFifo *fifo, FttCell *cell, FttDirection d, gdouble a,
  ...
  if (FTT_CELL_IS_LEAF(cell)) {
    if (!GFS_IS_MIXED(cell) &&
        GFS_VALUE(cell, status) == GFS_STATUS_UNDEFINED) {
    ...
    for (i = 0; i < FTT_NEIGHBORS; i++)
      if ((n = ftt_cell_neighbor(cell, i)) && !GFS_CELL_IS_BOUNDARY(n)) {
        if (solid->s[i] == 0. || solid->s[i] == 1.) {
          ...
          for (j = 0; j < k; j++)
            if (child.c[j])
              w += GFS_IS_FLUID(child.c[j])
                       ? 1.
                       : GFS_STATE(child.c[j])
                             ->solid->s[FTT_OPPOSITE_DIRECTION(i)];
          if (w / k <= 0. || w / k >= 1.)
            g_warning("file %s: line %d (%s): w/k=%g solid->s[%d]=%g", __FILE__,
                      __LINE__, G_STRFUNC, w / k, i, solid->s[i]);
          solid->s[i] = w / k;
        }
      }
        ...
        for (i = 0; i < FTT_CELLS; i++)
          if (child.c[i]) {
            if (GFS_VALUE(cell, p->status) == GFS_STATUS_UNDEFINED)
              ...
              g_assert(GFS_VALUE(cell, p->status) ==
                       GFS_VALUE(child.c[i], p->status));
          }
      }
    }
  }
        ...
        if (!FTT_CELL_IS_LEAF(neighbor.c[d])) {
          ...
          for (i = 0; i < n; i++)
            if (child.c[i] && GFS_VALUE(child.c[i], status) != 1.)
              s += GFS_IS_MIXED(child.c[i])
                       ? GFS_STATE(child.c[i])->solid->s[od]
                       : 1.;
          solid->s[d] = s / n;
          ...
          if (!GFS_IS_MIXED(neighbor.c[d]))
            ...
            solid->s[d] =
                GFS_STATE(neighbor.c[d])->solid->s[FTT_OPPOSITE_DIRECTION(d)];
 ...
 */
guint gfs_init_solid_fractions_leaves(GfsDomain *domain, GSList *i,
                                      GfsVariable *status) {
  InitSolidParams p;
  ...
  while (i) {
    gfs_domain_traverse_cut(
        domain, GFS_SOLID(i->data)->s, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
 ...
 * Returns: the number of thin cells.
 */
guint gfs_domain_init_solid_fractions(GfsDomain *domain, GSList *i,
                                      gboolean           destroy_solid,
  ...
  gfs_init_solid_fractions_from_children(domain, destroy_solid, cleanup, data,
                                         status1);
  if (status == NULL)
...
static gboolean check_area_fractions(const FttCell *root) {
  ...
  if (solid) {
    ...
    if (!gts_bbox_point_is_inside(&bb, &solid->cm)) {
      g_warning("file %s: line %d (%s): cm (%g,%g,%g)/%d is not inside cell "
                "[(%g,%g,%g),(%g,%g,%g)]",
                __FILE__, __LINE__, G_STRFUNC, solid->cm.x, solid->cm.y,
                solid->cm.z, ftt_cell_level(root), bb.x1, bb.y1, bb.z1, bb.x2,
                bb.y2, bb.z2);
      ret = FALSE;
      g_assert_not_reached();
    }
    if (!gts_bbox_point_is_inside(&bb, &solid->ca)) {
      g_warning("file %s: line %d (%s): ca (%g,%g,%g)/%d is not inside cell "
                "[(%g,%g,%g),(%g,%g,%g)]",
                __FILE__, __LINE__, G_STRFUNC, solid->ca.x, solid->ca.y,
                solid->ca.z, ftt_cell_level(root), bb.x1, bb.y1, bb.z1, bb.x2,
                bb.y2, bb.z2);
      ret = FALSE;
      g_assert_not_reached();
    }
        ...
        if (GFS_IS_FLUID(root)) {
          if (!GFS_IS_FLUID(neighbor.c[i])) {
            if (1. - nsolid->s[oi] >= 1e-10) {
              ...
              g_warning("file %s: line %d (%s): (%g,%g,%g)/%d: s[%d]: %g",
                        __FILE__, __LINE__, G_STRFUNC, p.x, p.y, p.z,
                        ftt_cell_level(root), oi, nsolid->s[oi]);
              ret = FALSE;
            }
            nsolid->s[oi] = 1.;
          }
          ...
          if (fabs(solid->s[i] - nsolid->s[oi]) >= 1e-10) {
            ...
            g_warning("file %s: line %d (%s): (%g,%g,%g)/%d: s[%d]: %g "
                      "neighbor->s[%d]: %g",
                      __FILE__, __LINE__, G_STRFUNC, p.x, p.y, p.z,
                      ftt_cell_level(root), i, solid->s[i], oi, nsolid->s[oi]);
            ret = FALSE;
          }
          ...
          if (1. - solid->s[i] >= 1e-10) {
            ...
            g_warning("file %s: line %d (%s): (%g,%g,%g)/%d: s[%d]: %g",
                      __FILE__, __LINE__, G_STRFUNC, p.x, p.y, p.z,
                      ftt_cell_level(root), i, solid->s[i]);
            ret = FALSE;
          }
          solid->s[i] = 1.;
        }
  ...
  if (!GFS_IS_MIXED(cell)) {
    for (n = 0; n < FTT_CELLS; n++)
      if (children.c[n] && GFS_IS_MIXED(children.c[n])) {
        g_warning("file %s: line %d (%s): children[%d] is mixed (%g)"
                  " parent is not",
                  __FILE__, __LINE__, G_STRFUNC, n,
                  GFS_STATE(children.c[n])->solid->a);
        *ret = FALSE;
      }
  } else {
    gdouble a = 0.;
    ...
    if (fabs(GFS_STATE(cell)->solid->a - a) >= 1e-10) {
      g_warning("file %s: line %d (%s): children->a: %g parent->a: %g",
                __FILE__, __LINE__, G_STRFUNC, a, GFS_STATE(cell)->solid->a);
      *ret = FALSE;
    }
  }
}
...
static void check_status(GfsBox *box, gpointer *data) {
  GfsVariable *status = data[0];
  if (!GFS_IS_MIXED(box->root) &&
      GFS_VALUE(box->root, status) == GFS_STATUS_UNDEFINED) {
    GfsGenericSurface *s = data[1];
    FttVector          pos;
    ftt_cell_pos(box->root, &pos);
    gdouble val = gfs_surface_point_is_inside(s, &pos) > 0 ? GFS_STATUS_FLUID
                                                           : GFS_STATUS_SOLID;
    data[2] = &val;
 ...
 * the cells of @domain.
 */
void gfs_domain_init_fraction(GfsDomain *domain, GfsGenericSurface *s,
                              GfsVariable *c) {
  gpointer     data[3];
  GfsVariable *status;
...
void gfs_face_ca(const FttCellFace *face, FttVector *ca) {
  ...
  if ((f = GFS_FACE_FRACTION(face)) < 1.) {
    ...
    (&ca->x)[cp] += (s->s[2 * cp] > s->s[2 * cp + 1]) ? (1. - f) / 2. * h
                                                      : (f - 1.) / 2. * h;
#else  /* 3D */
...
static void outer_fractions_coarse_fine(FttCell *parent, FttDirection d) {
  ...
  m.x = solid->s[2 * c1 + 1] - solid->s[2 * c1];
  nm  = fabs(m.x);
  m.y = solid->s[2 * c2 + 1] - solid->s[2 * c2];
  nm += fabs(m.y);
  if (nm > 0.) {
    m.x /= nm;
    m.y /= nm;
...
void gfs_solid_coarse_fine(FttCell *parent, GfsDomain *domain) {
  ...
  for (i = 0; i < FTT_CELLS; i++) {
    ...
    if (a > 0. && a < 1.) {
      GfsSolidVector *s = GFS_STATE(child.c[i])->solid =
          g_malloc(sizeof(GfsSolidVector));
      s->a = a;
      ...
      for (d = 0; d < FTT_NEIGHBORS; d++)
        if (!n.c[d])
        ...
        else if (!ftt_cell_neighbor_is_brother(child.c[i], d) &&
                 GFS_IS_FLUID(n.c[d]))
          s->s[d] = 1.;
        else {
#if FTT_2D
          ...
          if (ftt_cell_neighbor_is_brother(child.c[i], d)) {
            ...
            mp.x    = (&m.x)[c1];
            mp.y    = (&m.x)[c2];
            s->s[d] = gfs_line_area(&mp, d % 2 ? 2. * alpha1
                                               : 2. * alpha1 - (&m.x)[d / 2]);
          }
#endif /* 3D */
        }
  ...
  for (d = 0; d < FTT_NEIGHBORS; d++)
    if (neighbor.c[d] && FTT_CELL_IS_LEAF(neighbor.c[d]) &&
