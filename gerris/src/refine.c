...
static void refine_box(GfsBox *box, GfsFunction *maxlevel) {
  ftt_cell_refine(box->root, (FttCellRefineFunc)refine_maxlevel, maxlevel,
...
static void gfs_refine_refine(GfsRefine *refine, GfsSimulation *sim) {
  ...
  gts_container_foreach(GTS_CONTAINER(sim), (GtsFunc)refine_box,
                        refine->maxlevel);
...
#define GFS_REFINE_SOLID(obj)                                                  \
  GTS_OBJECT_CAST(obj, GfsRefineSolid, gfs_refine_solid_class())
...
static void refine_solid_read(GtsObject **o, GtsFile *fp) {
  ...
  GfsDerivedVariableInfo v      = {
      "SolidCurvature", "curvature of the solid boundary", solid_curvature};
  refine->v =
...
static void refine_cut_cell(FttCell *cell, GfsGenericSurface *s, RefineCut *p) {
  ...
  if (ftt_cell_level(cell) < gfs_function_value(p->refine->maxlevel, cell))
    ftt_cell_refine_single(cell, p->domain->cell_init,
                           p->domain->cell_init_data);
...
static void refine_implicit_cell(FttCell *cell, RefineCut *p) {
  ...
  if (ftt_cell_level(cell) < maxlevel &&
      gfs_cell_is_cut(cell, p->surface, FALSE, maxlevel))
    ftt_cell_refine_single(cell, p->domain->cell_init,
                           p->domain->cell_init_data);
}
  ...
  if (sim->solids) {
    ...
    while (i) {
      ...
      if (GFS_SURFACE(p.surface)->s)
        gfs_domain_traverse_cut(GFS_DOMAIN(sim), p.surface, FTT_PRE_ORDER,
                                FTT_TRAVERSE_LEAFS,
...
static void refine_surface_destroy(GtsObject *object) {
  ...
  (*GTS_OBJECT_CLASS(gfs_refine_surface_class())->parent_class->destroy)(
      object);
}

static void refine_surface_write(GtsObject *o, FILE *fp) {
  (*GTS_OBJECT_CLASS(gfs_refine_surface_class())->parent_class->write)(o, fp);
  gfs_generic_surface_write(GFS_REFINE_SURFACE(o)->surface,
                            gfs_object_simulation(o), fp);
}
...
static void refine_surface_read(GtsObject **o, GtsFile *fp) {
  ...
  gfs_generic_surface_read(GFS_REFINE_SURFACE(*o)->surface,
                           gfs_object_simulation(*o), fp);
}
  ...
  if (GFS_SURFACE(p.surface)->twod) {
    if (GFS_SURFACE(p.surface)->s)
      gfs_domain_traverse_cut_2D(
          GFS_DOMAIN(sim), GFS_REFINE_SURFACE(refine)->surface, FTT_PRE_ORDER,
          FTT_TRAVERSE_LEAFS, (FttCellTraverseCutFunc)refine_cut_cell, &p);
    else
...
static gdouble cell_distance(FttCell *cell, FttCellFace *face,
                             GfsSimulation *sim, GfsRefineDistance *refine) {
  FttVector pos;
  ...
  p.y = pos.y;
  p.z = pos.z;
  d   = gts_bb_tree_point_distance(
      refine->stree, &p, (GtsBBoxDistFunc)gts_point_triangle_distance, NULL);
  return d > h ? d - h : 0.;
}
...
static void refine_distance_read(GtsObject **o, GtsFile *fp) {
  ...
  v.data = *o;
  if (!gfs_domain_add_derived_variable(GFS_DOMAIN(gfs_object_simulation(*o)),
                                       v)) {
...
static gdouble cell_height(FttCell *cell, FttCellFace *face, GfsSimulation *sim,
