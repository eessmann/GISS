...
static int cell_is_corner(FttCell *cell) {
  ...
  for (d = 0; d < FTT_NEIGHBORS; d++)
    if (GFS_STATE(cell)->solid->s[d] != 1. &&
        GFS_STATE(cell)->solid->s[d] != 0. && d1 == -1 && d2 == -1)
      d1 = d;
    else if (GFS_STATE(cell)->solid->s[d] != 1. &&
             GFS_STATE(cell)->solid->s[d] != 0 && d2 == -1)
      d2 = d;
    else if (GFS_STATE(cell)->solid->s[d] != 1. &&
             GFS_STATE(cell)->solid->s[d] != 0.)
      ...
      if (neighbors.c[d1])
        if (GFS_IS_MIXED(neighbors.c[d2]) && GFS_IS_MIXED(neighbors.c[d1]))
          if (fabs(n1.x * n2.x + n1.y * n2.y) < 0.70) {
            if (GFS_STATE(neighbors.c[d1])->solid->s[d1] > 0 &&
                GFS_STATE(neighbors.c[d1])->solid->s[d1] < 1)
              return 1;
            if (GFS_STATE(neighbors.c[d2])->solid->s[d2] > 0 &&
                GFS_STATE(neighbors.c[d2])->solid->s[d2] < 1)
              return 1;
          }
    return 0;
  }
...
static int cell_was_corner(FttCell *cell, GfsVariable *old_solid_v,
                           GfsVariable **sold2) {
  FttDirection d, d1, d2;
  ...
  if (d1 / 2 == d2 / 2)
    ...
    if (neighbors.c[d1] && neighbors.c[d2]) {
      ...
      for (c = 0; c < FTT_DIMENSION; c++) {
        (&n1.x)[c] =
            (SOLD2(neighbors.c[d1], 2 * c + 1) - SOLD2(neighbors.c[d1], 2 * c));
        (&n2.x)[c] =
            (SOLD2(neighbors.c[d2], 2 * c + 1) - SOLD2(neighbors.c[d2], 2 * c));
      }
      norm = sqrt(n1.x * n1.x + n1.y * n1.y);
      if (norm != 0.) {
        n1.x /= norm;
      ...
      if (fabs(n1.x * n2.x + n1.y * n2.y) < 0.70) {
        if (SOLD2(neighbors.c[d1], d1) > 0 && SOLD2(neighbors.c[d1], d1) < 1)
          return 1.;
        else if (SOLD2(neighbors.c[d2], d2) > 0 &&
                 SOLD2(neighbors.c[d2], d2) < 1)
          return 1;
      }
    }
    return 0;
...
static double new_fluid_old_solid(FttCell *cell, FttDirection d1,
                                  GfsVariable *old_solid, GfsVariable **sold2) {
  FttDirection     d;
  FttCellNeighbors neighbors;
  double           s1, s2;
      ...
      if (neighbors.c[d])
        if (!cell_is_corner(neighbors.c[d]) &&
          ...
          if (GFS_STATE(neighbors.c[d])->solid)
            if (GFS_STATE(neighbors.c[d])->solid->s[d1] == 0. &&
                SOLD2(neighbors.c[d], d1) != 1.) {
...
static double new_solid_old_solid(FttCell *cell, FttDirection d1,
  ...
  for (d = 0; d < 2 * FTT_DIMENSION; d++)
    if (d != 2 * (d1 / 2) && d != 2 * (d1 / 2) + 1)
      if (neighbors.c[d] && !cell_is_corner(neighbors.c[d]) &&
          !cell_was_corner(neighbors.c[d], old_solid, sold2)) {
        if ((GFS_IS_MIXED(neighbors.c[d]) &&
             GFS_STATE(neighbors.c[d])->solid->s[d1] == 1.) ||
            !GFS_IS_MIXED(neighbors.c[d])) {
          if (SOLD2(neighbors.c[d], d1) != 1.) {
            s2 = 1. - SOLD2(neighbors.c[d], d1);
            return s1 / (s1 + s2);
          }
        } else if ((GFS_STATE(cell)->solid->s[d1] == 0. &&
                    GFS_IS_MIXED(neighbors.c[d]))) {
          s1 = SOLD2(cell, d1);
...
static void second_order_face_fractions(FttCell *            cell,
                                        GfsSimulationMoving *sim) {
#ifndef FTT_2D /* 3D */
  ...
  if (dto1 != -2 && dto2 != -2 && dto1 != dto2 && do1 / 2 != do2 / 2 &&
  ...
  if (d1 > -1) {
    if (SOLD2(cell, d1) == 0.)
      ...
      OLD_SOLID(cell)->s[d1] =
          (dt1 - 1.) * GFS_STATE(cell)->solid->s[d1] + 2. - dt1;
  }
  ...
  if (d2 > -1) {
    if (SOLD2(cell, d2) == 0.)
      ...
      OLD_SOLID(cell)->s[d2] =
          (dt2 - 1.) * GFS_STATE(cell)->solid->s[d2] + 2. - dt2;
  }

  if (d1 / 2 == d2 / 2 && do1 == -1 &&
      do2 == -1) /* third face has to be treated for
...
static void redistribute_old_face_in_merged(FttCell *cell, FttCell *merged,
                                            FttDirection d,
  ...
  for (i = 0; i < FTT_DIMENSION; i++)
    if (i != d / 2) {
      ...
      if (!old_solid_merged) {
        FttDirection c;
        OLD_SOLID(merged) = old_solid_merged =
      ...
      if (neighbors.c[2 * i]) {
        ...
        if (!old_solid) {
          FttDirection c;
          OLD_SOLID(neighbors.c[2 * i]) = old_solid =
              g_malloc0(sizeof(GfsSolidVector));
          old_solid->a = 1.;
          for (c = 0; c < FTT_NEIGHBORS; c++)
            old_solid->s[c] = 1.;
        }
        old_solid->s[ftt_opposite_direction[2 * i]] +=
            OLD_SOLID(cell)->s[2 * i];
      }
      ...
      if (neighbors.c[2 * i + 1]) {
        ...
        if (!old_solid) {
          FttDirection c;
          OLD_SOLID(neighbors.c[2 * i + 1]) = old_solid =
              g_malloc0(sizeof(GfsSolidVector));
          old_solid->a = 1.;
          for (c = 0; c < FTT_NEIGHBORS; c++)
            old_solid->s[c] = 1.;
        }
        old_solid->s[ftt_opposite_direction[2 * i + 1]] +=
            OLD_SOLID(cell)->s[2 * i + 1];
      }
    }
}

static void redistribute_old_face(FttCell *cell, FttCell *merged,
...
static double face_fraction_half(const FttCellFace *       face,
...
/* see gfs_face_advection_flux() for the initial implementation with static
 * boundaries */
static void moving_face_advection_flux(const FttCellFace *       face,
  ...
  flux = face_fraction_half(face, par) * GFS_FACE_NORMAL_VELOCITY(face) *
         par->dt * gfs_face_upwinded_value(face, GFS_FACE_UPWINDING, NULL) /
...
/* see gfs_face_velocity_advection_flux() for the initial implementation with
 * static boundaries */
...
static void moving_divergence_distribution_second_order(GSList *        merged,
                                                        DivergenceData *p) {
...
static void update_neighbors(FttCell *cell) {
  ...
  for (i = 0; i < FTT_NEIGHBORS; i++)
    if (neighbor.c[i] && neighbor.c[i]->children) {
      ftt_cell_neighbors_not_cached(neighbor.c[i],
                                    &(neighbor.c[i]->children->neighbors));
    }
}
...
static void moving_cell_coarse_fine(FttCell *cell, GfsVariable *v) {
  ...
  if (!GFS_CELL_IS_BOUNDARY(parent)) {
    ...
    for (c = 0; c < FTT_DIMENSION; c++)
      GFS_VALUE(cell, v) +=
          (&p.x)[c] * gfs_center_van_leer_gradient(parent, c, v->i);
  }
}
...
static void create_new_cells(FttCell *cell, GfsSurface *s,
  ...
  if (FTT_CELL_IS_DESTROYED(cell) && ftt_cell_level(cell) <= maxlevel) {
    ...
    if (ftt_cell_level(cell) < maxlevel)
      ftt_cell_refine(cell, (FttCellRefineFunc)refine_maxlevel, &maxlevel,
...
static void boundary_match(GfsBoundary *boundary) {
  ...
  while (l <= boundary->depth) {
    ftt_cell_traverse_boundary(boundary->root, boundary->d, FTT_PRE_ORDER,
                               FTT_TRAVERSE_LEVEL, l,
                               (FttCellTraverseFunc)match, boundary);
    l++;
  }
  if (boundary->changed)
    ftt_cell_flatten(boundary->root, boundary->d,
...
static void remesh_surface_moving(GfsSimulation *sim, GfsSolidMoving *s) {
  ...
  gfs_domain_traverse_cut(domain, GFS_SOLID(s)->s, FTT_POST_ORDER,
                          FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_DESTROYED,
                          (FttCellTraverseCutFunc)create_new_cells,
                          &solid_info);
...
static void solid_moving_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_solid_moving_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_solid_moving_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR)
    return;

  solid->nvertex =
      gts_surface_vertex_number(GFS_SURFACE(GFS_SOLID(solid)->s)->s);

  if (!GFS_IS_SURFACE(GFS_SOLID(solid)->s) ||
      !GFS_SURFACE(GFS_SOLID(solid)->s)->s) {
  ...
  if (!GFS_IS_SIMULATION_MOVING(gfs_object_simulation(*o))) {
    gts_file_error(fp,
                   "GfsSolidMoving only makes sense with GfsSimulationMoving");
    return;
  }
...
static void solid_moving_write(GtsObject *object, FILE *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_solid_moving_class())->parent_class->write)
    (*GTS_OBJECT_CLASS(gfs_solid_moving_class())->parent_class->write)(object,
                                                                       fp);
  fputs(" { level =", fp);
...
static void redistribute_destroyed_cells_content(FttCell *     cell,
                                                 ReInitParams *p) {
  ...
  if (GFS_SIMULATION(domain)->advection_params.moving_order == 2)
    redistribute_old_face(cell, merged,
                          GFS_SIMULATION_MOVING(domain)->old_solid);
}
 ...
 */
static guint domain_reinit_solid_fractions(GfsSimulation *sim, GSList *i) {
  GfsDomain *  domain = GFS_DOMAIN(sim);
  GfsVariable *status;
  ...
  if (sim->time.t != 0.) {
    ...
    gfs_domain_cell_traverse(
        domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
        (FttCellTraverseFunc)redistribute_destroyed_cells_content, &rp);
  }

  gfs_init_solid_fractions_from_children(
      domain, TRUE, (FttCellCleanupFunc)gfs_cell_cleanup, domain, status);
...
static void reinit_solid_fractions(GfsSimulation *sim) {
  ...
  gts_container_foreach(GTS_CONTAINER(sim), (GtsFunc)check_solid_fractions,
                        &nf);
  if (nf > 0) {
...
static void moving_advection_update(GSList *                  merged,
                                    const GfsAdvectionParams *par) {
  GfsVariable *old_solid_v = GFS_SIMULATION_MOVING(par->v->domain)->old_solid;
  ...
  if (merged->next == NULL) { /* cell is not merged */
    ...
    GFS_VALUE(cell, par->v) =
        (olda * GFS_VALUE(cell, par->v) + GFS_VALUE(cell, par->fv)) / a;
...
static gboolean solid_moving_event(GfsEvent *event, GfsSimulation *sim) {
  return (GFS_SOLID_MOVING(event)->active =
              (*GFS_EVENT_CLASS(
                    GTS_OBJECT_CLASS(gfs_solid_moving_class())->parent_class)
                    ->event)(event, sim));
}
...
static void solid_moving_init(GfsSolidMoving *solid) {
  gfs_event_set(GFS_EVENT(solid), 0., G_MAXDOUBLE / 2., -1., 0, G_MAXINT / 2,
                1);
...
static void move_vertex_mpi(GfsNumberedVertex *v, SolidInfo *par) {
  ...
  if (cell) {
    ...
    for (c = 0; c < FTT_DIMENSION; c++)
      g_array_index(par->stmp, double, FTT_DIMENSION * v->num + c) =
          -G_MAXDOUBLE;
  }
}
...
static void solid_move_remesh(GfsSolidMoving *solid, GfsSimulation *sim) {
  ...
  if (surface->s) {
    ...
    if (GFS_DOMAIN(sim)->pid >= 0) { /* Parallel simulation */
      p.stmp = g_array_set_size(g_array_new(FALSE, FALSE, sizeof(double)),
                                FTT_DIMENSION * solid->nvertex);
      p.sall = g_array_set_size(g_array_new(FALSE, FALSE, sizeof(double)),
                                FTT_DIMENSION * solid->nvertex);

      gts_surface_foreach_vertex(surface->s, (GtsFunc)move_vertex_mpi, &p);

      MPI_Allreduce(p.stmp->data, p.sall->data, p.stmp->len, MPI_DOUBLE,
...
static void move_solids(GfsSimulation *sim) {
  ...
  if (sim->advection_params.moving_order == 2) {
    gfs_domain_cell_traverse(domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
                             (FttCellTraverseFunc)second_order_face_fractions,
                             sim);
    FttDirection d;
    for (d = 0; d < FTT_NEIGHBORS; d++)
...
static void moving_divergence_approx(FttCell *cell, DivergenceData *p) {
  GFS_VALUE(cell, p->div) += GFS_STATE(cell)->solid->fv *
                             (GFS_STATE(cell)->solid->s[2 * p->c + 1] -
                              GFS_STATE(cell)->solid->s[2 * p->c]) *
                             ftt_cell_size(cell);
}
...
static void divergence_approx_hook(GfsDomain *domain, gdouble dt,
                                   GfsVariable *div) {
  DivergenceData q;
  ...
  for (q.c = 0; q.c < FTT_DIMENSION; q.c++) {
    ...
    gfs_domain_traverse_mixed(domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                              (FttCellTraverseFunc)moving_divergence_approx,
                              &q);
  }
  gfs_domain_traverse_merged(
      domain, (GfsMergedTraverseFunc)moving_divergence_distribution, &q);
}
...
static void divergence_mac_hook_order_1(GfsDomain *domain, gdouble dt,
                                        GfsVariable *div) {
  DivergenceData q;
...
static void divergence_mac_hook_order_2(GfsDomain *domain, gdouble dt,
                                        GfsVariable *div) {
  DivergenceData q;
  ...
  gfs_domain_traverse_merged(
      domain,
      (GfsMergedTraverseFunc)moving_divergence_distribution_second_order, &q);
}

static void moving_mac_projection(GfsSimulation *sim, GfsMultilevelParams *par,
  ...
  gfs_mac_projection(GFS_DOMAIN(sim), par, apar->dt / 2., p, alpha, g,
                     (apar->moving_order == 2 ? divergence_mac_hook_order_2
                                              : divergence_mac_hook_order_1));
  if (apar->moving_order == 2)
...
static void simulation_moving_run(GfsSimulation *sim) {
  ...
  if (sim->time.i == 0)
    gfs_approximate_projection(
        domain, &sim->approx_projection_params, sim->advection_params.dt, p,
        sim->physical_params.alpha, res, g, divergence_approx_hook);
  ...
  while (sim->time.t < sim->time.end && sim->time.i < sim->time.iend) {
    ...
    moving_mac_projection(sim, &sim->projection_params, &sim->advection_params,
