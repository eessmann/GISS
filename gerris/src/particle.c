...
static gboolean gfs_particle_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_particle_class())->parent_class)
            ->event)(event, sim)) {
...
GfsEventClass *gfs_particle_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_particle_info = {
        "GfsParticle",         sizeof(GfsParticle),
        sizeof(GfsEventClass), (GtsObjectClassInitFunc)gfs_particle_class_init,
        (GtsArgSetFunc)NULL,   (GtsArgGetFunc)NULL};
