...
void gfs_multilevel_params_write(GfsMultilevelParams *par, FILE *fp) {
  ...
  fprintf(fp,
          ...
          "  weighted  = %d\n"
          "  beta      = %g\n",
          par->tolerance, par->nrelax, par->erelax, par->minlevel,
          par->nitermax, par->nitermin, par->weighted, par->beta);
  if (par->omega != 1.)
    fprintf(fp, "  omega     = %g\n", par->omega);
  if (par->function)
    fputs("  function  = 1\n", fp);
...
void gfs_multilevel_params_read(GfsMultilevelParams *par, GtsFile *fp) {
  ...
  GtsFileVariable var[] = {{GTS_DOUBLE, "tolerance", TRUE, &par->tolerance},
...
void gfs_multilevel_params_stats_write(GfsMultilevelParams *par, FILE *fp) {
  ...
  fprintf(fp,
          ...
          par->residual_before.first, par->residual.first,
          rate(par->residual.first, par->residual_before.first, par->niter),
          par->residual_before.second, par->residual.second,
          rate(par->residual.second, par->residual_before.second, par->niter),
          par->residual_before.infty, par->residual.infty,
          rate(par->residual.infty, par->residual_before.infty, par->niter));
}
...
static void relax_dirichlet_stencil(FttCell *cell, RelaxStencilParams *p) {
  ...
  if (GFS_IS_MIXED(cell) && ((cell)->flags & GFS_FLAG_DIRICHLET) != 0) {
    g.b = gfs_cell_dirichlet_gradient_flux_stencil(cell, p->maxlevel, 0., p->lp,
                                                   stencil);
...
static void set_mpi_domain_index(GfsDomain *domain, GfsLinearProblem *lp) {
  ...
  MPI_Allgather(&lp->rhs->len, 1, MPI_UNSIGNED, mpi_domain_index, 1,
...
static void cell_numbering(GfsDomain *domain, GfsLinearProblem *lp,
                           GfsVariable *rhs, GfsVariable *lhs, gint maxlevel) {
  /* fixme: should it be FTT_TRAVERSE_LEVEL | FTT_TRAVERSE_LEAFS */
  NumberingParams np = {lp, lhs,      rhs,
                        0,  maxlevel, (FttFaceTraverseFunc)bc_number};
  ...
  if (domain->pid >= 0) {
    ...
    np.nleafs = lp->istart;
    gfs_domain_cell_traverse(domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             maxlevel, (FttCellTraverseFunc)leaves_renumbering,
                             &np);
  }
#endif /* HAVE_MPI */
 ...
 */
GfsLinearProblem *gfs_get_poisson_problem(GfsDomain *domain, GfsVariable *rhs,
                                          GfsVariable *lhs, GfsVariable *dia,
                                          gint maxlevel, GfsVariable *v) {
  ...
  gfs_domain_homogeneous_bc_stencil(
      domain, FTT_TRAVERSE_LEVEL | FTT_TRAVERSE_LEAFS, maxlevel, lhs, v, lp);
  gfs_domain_cell_traverse(
      domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEVEL | FTT_TRAVERSE_LEAFS, maxlevel,
      (FttCellTraverseFunc)(v->centered ? relax_stencil
                                        : relax_dirichlet_stencil),
      &p);
  ...
  /* If Neumann conditions are applied everywhere, the solution is
     ...
     simply connected... */
  if (v->centered &&
      /* fixme: this seems to reduce the convergence speed for pure Poisson
         problems */
...
static void relax2D(FttCell *cell, RelaxParams *p) {
  ...
  if (g.a != 0.)
    GFS_VALUEI(cell, p->u) = (1. - p->omega) * GFS_VALUEI(cell, p->u) +
 ...
 */
void gfs_relax(GfsDomain *domain, guint d, gint max_depth, gdouble omega,
  ...
  gfs_domain_cell_traverse(
      ...
      (FttCellTraverseFunc)(u->centered ? (d == 2 ? relax2D : relax)
                                        : relax_dirichlet),
      &p);
}
...
static void residual_set(FttCell *cell, RelaxParams *p) {
  ...
  GFS_VALUEI(cell, p->res) =
      GFS_VALUEI(cell, p->rhs) - (g.b - GFS_VALUEI(cell, p->u) * g.a);
}
...
static void residual_set_dirichlet(FttCell *cell, RelaxParams *p) {
  ...
  g.a = GFS_VALUEI(cell, p->dia);
  if (GFS_IS_MIXED(cell) && ((cell)->flags & GFS_FLAG_DIRICHLET) != 0)
    g.b = gfs_cell_dirichlet_gradient_flux(cell, p->u, p->maxlevel,
 ...
 * @res.
 */
void gfs_residual(GfsDomain *domain, guint d, FttTraverseFlags flags,
                  gint max_depth, GfsVariable *u, GfsVariable *rhs,
                  GfsVariable *dia, GfsVariable *res) {
  RelaxParams p;
  ...
  p.maxlevel = max_depth;
  gfs_domain_cell_traverse(
      domain, FTT_PRE_ORDER, flags, max_depth,
...
static void poisson_coeff(FttCellFace *face, PoissonCoeff *p) {
  gdouble alpha = p->alpha ? gfs_function_face_value(p->alpha, face) : 1.;
  gdouble v     = p->lambda2[face->d / 2] * alpha *
              gfs_domain_face_fraction(p->domain, face) /
...
static void face_coeff_from_below(FttCell *cell) {
  ...
  for (d = 0; d < FTT_NEIGHBORS; d++) {
    ...
    FttCell *neighbor;
    if (f[d].v != 0. && (neighbor = ftt_cell_neighbor(cell, d)) &&
        !GFS_CELL_IS_BOUNDARY(neighbor))
      neighbors++;
  }
 ...
 */
void gfs_poisson_coefficients(GfsDomain *domain, GfsFunction *alpha,
                              gboolean positive, gboolean centered,
                              gboolean reset) {
  PoissonCoeff p;
  FttComponent i;
  ...
  gfs_domain_face_traverse(domain, FTT_XYZ, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                           -1, (FttFaceTraverseFunc)poisson_coeff, &p);
...
static void tension_coeff(FttCellFace *face, gpointer *data) {
  ...
  gdouble v     = lambda2[face->d / 2] * alpha *
              gfs_domain_face_fraction(kappa->domain, face) *
 ...
 * If @alpha is %NULL, it is taken to be unity.
 */
void gfs_source_tension_coefficients(GfsSourceTension *s, GfsDomain *domain,
                                     GfsFunction *alpha) {
  gdouble      lambda2[FTT_DIMENSION];
  gpointer     data[3];
  FttComponent i;
  ...
  data[2] = alpha;
  gfs_domain_face_traverse(domain, FTT_XYZ, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                           -1, (FttFaceTraverseFunc)tension_coeff, data);
}
...
static void relax_loop(GfsDomain *domain, GfsVariable *dp, GfsVariable *u,
                       RelaxParams *q, guint nrelax,
                       FttCellTraverseFunc relaxfunc) {
  guint n;

  gfs_domain_homogeneous_bc(domain, FTT_TRAVERSE_LEVEL | FTT_TRAVERSE_LEAFS,
                            q->maxlevel, dp, u);
  for (n = 0; n < nrelax - 1; n++)
    gfs_traverse_and_homogeneous_bc(domain, FTT_PRE_ORDER,
                                    FTT_TRAVERSE_LEVEL | FTT_TRAVERSE_LEAFS,
                                    q->maxlevel, relaxfunc, q, dp, u);
 ...
 */
void gfs_poisson_cycle(GfsDomain *domain, GfsMultilevelParams *p,
                       GfsVariable *u, GfsVariable *rhs, GfsVariable *dia,
                       GfsVariable *res) {
  ...
  for (q.maxlevel = minlevel + 1; q.maxlevel <= p->depth;
       q.maxlevel++, nrelax /= p->erelax) {
    /* get initial guess from coarser grid */
    gfs_domain_cell_traverse(
        domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEVEL | FTT_TRAVERSE_NON_LEAFS,
        q.maxlevel - 1, (FttCellTraverseFunc)get_from_above, dp);
    relax_loop(domain, dp, u, &q, nrelax, relaxfunc);
  }
  ...
  gfs_traverse_and_bc(domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
                      (FttCellTraverseFunc)correct, data, u, u);
 ...
 * right-hand-side.
 */
gdouble gfs_poisson_compatibility(GfsDomain *domain, GfsVariable *lhs,
                                  GfsVariable *rhs, gdouble dt) {
  ...
  gfs_all_reduce(domain, p.dirichlet, MPI_INT, MPI_MAX);
  if (p.dirichlet)
    /* Assumes that the domain is simply connected in which case compatibility
     * is guaranteed */
    return 0.;
  /* compute volume integral of right-hand-side (again we assume that
     the domain is simply connected) */
  return fabs(
      gfs_domain_norm_residual(domain, FTT_TRAVERSE_LEAFS, -1, dt, rhs).bias);
}
...
void gfs_poisson_solve(GfsDomain *domain, GfsMultilevelParams *par,
  ...
  gfs_residual(domain, par->dimension, FTT_TRAVERSE_LEAFS, -1, lhs, rhs, dia,
               res);
  ...
  while (par->niter < par->nitermin ||
    ...
    if (par->residual.infty == res_max_before) /* convergence has stopped!! */
      break;
    if (par->residual.infty > res_max_before / 1.1 &&
        par->minlevel < par->depth)
      par->minlevel++;
...
static void diffusion_coef(FttCellFace *face, DiffusionCoeff *c) {
  gdouble v = c->lambda2[face->d / 2] * c->dt *
...
static void viscous_metric_coeff(FttCell *cell, DiffusionCoeff *c) {
  GFS_VALUE(cell, c->metric) =
      c->dt * gfs_source_diffusion_cell(c->d, cell) *
      (*c->domain->viscous_metric_implicit)(c->domain, cell,
                                            c->metric->component) *
      gfs_domain_cell_fraction(c->domain, cell) / GFS_VALUE(cell, c->rhoc);
}
 ...
 */
void gfs_diffusion_coefficients(GfsDomain *domain, GfsSourceDiffusion *d,
                                gdouble dt, GfsVariable *rhoc,
                                GfsVariable *metric, GfsFunction *alpha,
                                gdouble beta) {
  DiffusionCoeff coef;
  FttComponent   i;
  ...
  gfs_domain_face_traverse(domain, FTT_XYZ, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                           -1, (FttFaceTraverseFunc)diffusion_coef, &coef);
  gfs_domain_cell_traverse(domain, FTT_POST_ORDER, FTT_TRAVERSE_NON_LEAFS, -1,
                           (FttCellTraverseFunc)face_coeff_from_below, NULL);
}
 ...
 * gfs_diffusion_coefficients().
 */
void gfs_diffusion_rhs(GfsDomain *domain, GfsVariable *v, GfsVariable *rhs,
                       GfsVariable *rhoc, GfsVariable *metric, gdouble beta) {
  RelaxParams p;
...
static void diffusion_relax_stencil(FttCell *cell, RelaxStencilParams *p) {
  ...
  for (face.d = 0; face.d < FTT_NEIGHBORS; face.d++) {
    GfsGradient ng;
    face.neighbor = neighbor.c[face.d];
    gfs_face_cm_weighted_gradient_stencil(&face, &ng, p->maxlevel, p->lp,
                                          stencil);
    g.a += ng.a;
  }
 ...
 * gfs_diffusion_rhs().
 */
void gfs_diffusion_residual(GfsDomain *domain, GfsVariable *u, GfsVariable *rhs,
 ...
 * of @res (i.e. the cell tree is ready for another iteration).
 */
void gfs_diffusion_cycle(GfsDomain *domain, guint levelmin, guint depth,
                         guint nrelax, GfsVariable *u, GfsVariable *rhs,
                         GfsVariable *rhoc, GfsVariable *metric,
                         GfsVariable *res) {
  GfsVariable *dp;
  RelaxParams  p;
  ...
  gfs_domain_cell_traverse(domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEVEL, levelmin,
                           (FttCellTraverseFunc)gfs_cell_reset, dp);
  relax_loop(domain, dp, u, &p, 10 * nrelax,
             (FttCellTraverseFunc)diffusion_relax);
  /* relax from top to bottom */
  for (p.maxlevel = levelmin + 1; p.maxlevel <= depth; p.maxlevel++) {
    /* get initial guess from coarser grid */
    gfs_domain_cell_traverse(
        domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEVEL | FTT_TRAVERSE_NON_LEAFS,
        p.maxlevel - 1, (FttCellTraverseFunc)get_from_above, dp);
 ...
 * Returns: a #GfsLinearProblem.
 */
GfsLinearProblem *gfs_get_diffusion_problem(GfsDomain *domain, GfsVariable *rhs,
                                            GfsVariable *lhs, GfsVariable *rhoc,
                                            GfsVariable *metric, gint maxlevel,
                                            GfsVariable *v) {
  gfs_domain_timer_start(domain, "get_diffusion_problem");

  GfsLinearProblem *lp = gfs_linear_problem_new(domain);

  RelaxStencilParams p = {lp, rhoc, maxlevel, metric, rhs};

  gfs_domain_cell_traverse(domain, FTT_PRE_ORDER,
                           FTT_TRAVERSE_LEVEL | FTT_TRAVERSE_LEAFS, maxlevel,
                           (FttCellTraverseFunc)scale_rhs, &p);
  cell_numbering(domain, lp, rhs, lhs, maxlevel);
  ...
  gfs_domain_cell_traverse(domain, FTT_PRE_ORDER,
                           FTT_TRAVERSE_LEVEL | FTT_TRAVERSE_LEAFS, maxlevel,
                           (FttCellTraverseFunc)diffusion_relax_stencil, &p);
