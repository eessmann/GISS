...
static void gfs_output_destroy(GtsObject *object) {
  ...
  (*GTS_OBJECT_CLASS(gfs_output_class())->parent_class->destroy)(object);
}
    ...
    if (!output->dynamic) {
      if (output->file)
        ...
        if (output->format[0] == '{') { /* script */
          ...
          fname = gfs_format_string(output->formats, GFS_DOMAIN(sim)->pid,
                                    sim->time.i, sim->time.t);
          output->file = gfs_output_file_open(
              fname,
              sim->time.i > 0 && gfs_event_is_repetitive(event) ? "a" : "w");
          if (output->file == NULL)
...
static void gfs_output_read(GtsObject **o, GtsFile *fp) {
  ...
  if (fp->type == '{') {
    ...
    output->formats =
        gfs_format_new(output->format, fp, &output->dynamic, &output->parallel);
    if (fp->type == GTS_ERROR) {
      output_free(output);
      return;
    }
...
static gboolean time_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(gfs_output_class())->event)(event, sim)) {
    ...
    fprintf(GFS_OUTPUT(event)->file->fp,
            ...
            sim->time.i, sim->time.t, sim->advection_params.dt, cpu,
...
static gboolean progress_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(gfs_output_class())->event)(event, sim)) {
    ...
    if (idone > tdone)
      tdone = idone;
    fprintf(GFS_OUTPUT(event)->file->fp, "\r%3.0f%% done", 100. * tdone);
    if (tdone > 0.) {
      ...
      fprintf(GFS_OUTPUT(event)->file->fp, ", %02.0f:%02.0f:%02.0f remaining ",
              hours, mins, secs);
    }
    if (tdone == 1.)
      ...
      if (v->sources) {
        ...
        while (j) {
          ...
          if (GFS_IS_SOURCE_DIFFUSION(o) &&
              GFS_SOURCE_DIFFUSION(o)->D->par.niter > 0 &&
              !g_slist_find(l, o)) {
            l = g_slist_prepend(l, o);
            fprintf(fp, "%s diffusion\n", v->name);
            gfs_multilevel_params_stats_write(&GFS_SOURCE_DIFFUSION(o)->D->par,
                                              fp);
          }
          j = j->next;
        }
...
static gboolean gfs_output_solid_stats_event(GfsEvent *     event,
                                             GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_output_solid_stats_class())->parent_class)
            ->event)(event, sim)) {
    ...
    fprintf(GFS_OUTPUT(event)->file->fp,
            ...
            stats.min, stats.mean, stats.stddev, stats.max, stats.n, ma.min,
            ma.mean, ma.stddev, ma.max, ma.n, mn.min, mn.mean, mn.stddev,
            mn.max, mn.n, sim->thin);
    return TRUE;
  }
  return FALSE;
}
...
static gboolean gfs_output_adapt_stats_event(GfsEvent *     event,
                                             GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_output_adapt_stats_class())->parent_class)
            ->event)(event, sim)) {
    ...
    fprintf(GFS_OUTPUT(event)->file->fp,
            ...
            "  Number of cells\n"
            "    min: %10.0f avg: %10.3f | %10.3f max: %10.0f n: %10d\n",
            sim->adapts_stats.removed, sim->adapts_stats.created,
            sim->adapts_stats.ncells.min, sim->adapts_stats.ncells.mean,
            sim->adapts_stats.ncells.stddev, sim->adapts_stats.ncells.max,
            sim->adapts_stats.ncells.n);
    if (sim->adapts_stats.cmax.n > 0)
      fprintf(GFS_OUTPUT(event)->file->fp,
              "  Maximum cost\n"
              "    min: %10.3e avg: %10.3e | %10.3e max: %10.3e n: %10d\n",
              sim->adapts_stats.cmax.min, sim->adapts_stats.cmax.mean,
              sim->adapts_stats.cmax.stddev, sim->adapts_stats.cmax.max,
  ...
  while (--count >= 0)
    if (timing[count].t->r.sum > 0.)
      fprintf(fp,
              ...
              timing[count].name, timing[count].t->r.min,
              timing[count].t->r.mean,
              domain->timestep.sum > 0.
                  ? 100. * timing[count].t->r.sum / domain->timestep.sum
                  : 0.,
              timing[count].t->r.stddev, timing[count].t->r.max);
  g_free(timing);
}
  ...
  if ((*GFS_EVENT_CLASS(gfs_output_class())->event)(event, sim)) {
    ...
    if (domain->timestep.mean > 0.) {
      fprintf(fp,
              ...
              domain->timestep.n, domain->size.mean / domain->timestep.mean,
              domain->timestep.min, domain->timestep.mean,
              domain->timestep.stddev, domain->timestep.max, domain->size.min,
              domain->size.mean, domain->size.stddev, domain->size.max,
      ...
      if (domain->mpi_messages.n > 0)
        fprintf(fp,
                ...
                domain->mpi_messages.n, domain->mpi_messages.sum);
    }
    return TRUE;
  }
  return FALSE;
...
static gboolean gfs_output_balance_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_output_balance_class())->parent_class)
            ->event)(event, sim)) {
    GfsDomain *domain = GFS_DOMAIN(sim);
    FILE *     fp     = GFS_OUTPUT(event)->file->fp;
    ...
    fprintf(fp,
            ...
            size.n, size.min, size.mean, size.stddev, size.max);
    if (boundary.max > 0.)
      fprintf(fp,
...
static void gfs_output_solid_force_destroy(GtsObject *object) {
  ...
  (*GTS_OBJECT_CLASS(gfs_output_solid_force_class())->parent_class->destroy)(
      object);
}
...
static void gfs_output_solid_force_read(GtsObject **o, GtsFile *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_output_solid_force_class())->parent_class->read)(o,
                                                                          fp);
  if (fp->type == GTS_ERROR)
    return;
...
static void gfs_output_solid_force_write(GtsObject *o, FILE *fp) {
  GfsOutputSolidForce *l = GFS_OUTPUT_SOLID_FORCE(o);
  (*GTS_OBJECT_CLASS(gfs_output_solid_force_class())->parent_class->write)(o,
                                                                           fp);
  if (l->weight)
...
static gboolean gfs_output_solid_force_event(GfsEvent *     event,
                                             GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_output_solid_force_class())->parent_class)
            ->event)(event, sim) &&
    ...
    gfs_domain_solid_force(domain, &pf, &vf, &pm, &vm,
                           GFS_OUTPUT_SOLID_FORCE(event)->weight);
    fprintf(fp, "%g %g %g %g %g %g %g %g %g %g %g %g %g\n", sim->time.t,
            pf.x * Ln, pf.y * Ln, pf.z * Ln, vf.x * Ln, vf.y * Ln, vf.z * Ln,
            pm.x * Ln * L, pm.y * Ln * L, pm.z * Ln * L, vm.x * Ln * L,
            vm.y * Ln * L, vm.z * Ln * L);
    return TRUE;
  }
  return FALSE;
...
static void gfs_output_location_destroy(GtsObject *object) {
  ...
  (*GTS_OBJECT_CLASS(gfs_output_location_class())->parent_class->destroy)(
      object);
}
...
static void gfs_output_location_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_output_location_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_output_location_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR)
    return;
  ...
  if (fp->type == GTS_STRING) {
    ...
    while (fp1->type != GTS_NONE) {
      ...
      if (!vector_read(fp1, &p)) {
        gts_file_error(fp, "%s:%d:%d: %s", fp->token->str, fp1->line, fp1->pos,
                       fp1->error);
        return;
      }
  ...
  if (fp->type == '{') {
    ...
    GtsFileVariable var[] = {{GTS_STRING, "label", TRUE, &label},
...
static gboolean gfs_output_location_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_output_location_class())->parent_class)
            ->event)(event, sim)) {
    GfsDomain *        domain   = GFS_DOMAIN(sim);
    ...
    GfsUnionFile       uf;
    FILE *             fpp =
        ((domain->pid < 0 || GFS_OUTPUT(event)->parallel)
             ? fp
    ...
    gchar *pformat =
        g_strdup_printf("%s %s %s %s", location->precision, location->precision,
    ...
    for (i = 0; i < location->p->len; i++) {
      ...
      if (cell != NULL) {
        ...
        while (i) {
          ...
          if (v->name)
            fprintf(fpp, vformat,
                    gfs_dimensional_value(v, location->interpolate
...
static gboolean gfs_output_particle_event(GfsEvent *event, GfsSimulation *sim) {
  GfsOutputLocation *location = GFS_OUTPUT_LOCATION(event);
  gboolean           ret      = FALSE;
  guint              i;
...
static void write_text(FttCell *cell, gpointer *data) {
  ...
  gchar *format = g_strdup_printf("%s %s %s", output->precision,
  ...
  if ((*GFS_EVENT_CLASS(gfs_output_class())->event)(event, sim)) {
    ...
    case GFS:
      if (GFS_OUTPUT(output)->parallel)
        gfs_simulation_write(sim, output->max_depth,
                             GFS_OUTPUT(event)->file->fp);
      else
        gfs_simulation_union_write(sim, output->max_depth,
                                   GFS_OUTPUT(event)->file->fp);
      break;
    ...
    case GFS_TEXT: {
      ...
      if (GFS_OUTPUT(output)->parallel) {
        ...
        gfs_domain_cell_traverse(
            domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEVEL | FTT_TRAVERSE_LEAFS,
            output->max_depth, (FttCellTraverseFunc)write_text, data);
      } else {
        GfsUnionFile uf;
        FILE *       fpp =
            gfs_union_open(GFS_OUTPUT(event)->file->fp, domain->pid, &uf);
    ...
    case GFS_VTK: {
      gfs_domain_write_vtk(domain, output->max_depth, domain->variables_io,
                           output->precision, GFS_OUTPUT(event)->file->fp);
      break;
    }
    ...
    case GFS_TECPLOT: {
      gfs_domain_write_tecplot(domain, output->max_depth, domain->variables_io,
                               output->precision, GFS_OUTPUT(event)->file->fp);
#if !FTT_2D
      gfs_domain_write_tecplot_surface(domain, output->max_depth,
                                       domain->variables_io, output->precision,
                                       GFS_OUTPUT(event)->file->fp);
#endif
      break;
    }
...
static void output_simulation_write(GtsObject *o, FILE *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_output_simulation_class())->parent_class->write)(o,
                                                                          fp);
...
static void output_simulation_read(GtsObject **o, GtsFile *fp) {
  ...
  if (fp->type == '{') {
    GtsFileVariable var[]     = {{GTS_INT, "depth", TRUE},
    ...
    if (variables != NULL) {
      ...
      GfsDomain *domain = GFS_DOMAIN(gfs_object_simulation(output));
      GSList *   vars =
          gfs_variables_from_list(domain->variables, variables, &error);

      if (vars == NULL) {
        gts_file_variable_error(fp, var, "variables", "unknown variable `%s'",
                                error);
        g_free(variables);
        return;
      }
    ...
    if (format != NULL) {
      if (!strcmp(format, "gfs"))
        ...
        gts_file_variable_error(fp, var, "format", "unknown format `%s'",
                                format);
        g_free(format);
        return;
      }
...
static void gfs_output_scalar_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_output_scalar_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_output_scalar_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR)
    return;
...
static void gfs_output_scalar_write(GtsObject *o, FILE *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_output_scalar_class())->parent_class->write)
    (*GTS_OBJECT_CLASS(gfs_output_scalar_class())->parent_class->write)(o, fp);
...
static void output_scalar_traverse(GfsOutputScalar *output,
  ...
  if (output->condition)
    gfs_domain_cell_traverse_condition(
        GFS_DOMAIN(gfs_object_simulation(output)), order, flags, max_depth,
        func, data, cell_condition, output->condition);
  else
    gfs_domain_cell_traverse(GFS_DOMAIN(gfs_object_simulation(output)), order,
                             flags, max_depth, func, data);
}

static gboolean gfs_output_scalar_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_output_scalar_class())->parent_class)
            ->event)(event, sim)) {
    ...
    if (output->maxlevel >= 0) {
      gfs_domain_cell_traverse(domain, FTT_POST_ORDER, FTT_TRAVERSE_NON_LEAFS,
                               -1, (FttCellTraverseFunc)output->v->fine_coarse,
                               output->v);
      gfs_domain_bc(domain, FTT_TRAVERSE_NON_LEAFS, -1, output->v);
    }
    if (output->autoscale) {
      GtsRange stats = gfs_domain_stats_variable(
          domain, output->v, FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL,
          output->maxlevel, output->condition ? cell_condition : NULL,
          output->condition);
...
static void gfs_output_scalar_post_event(GfsEvent *event, GfsSimulation *sim) {
  GfsOutputScalar *output = GFS_OUTPUT_SCALAR(event);
  if (output->v != gfs_function_get_variable(output->f)) {
  ...
  (*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_output_scalar_class())->parent_class)
        ->post_event)(event, sim);
}
...
static gboolean gfs_output_scalar_norm_event(GfsEvent *     event,
                                             GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_output_scalar_norm_class())->parent_class)
            ->event)(event, sim)) {
    GfsOutputScalar *output = GFS_OUTPUT_SCALAR(event);
    GfsNorm          norm   = gfs_domain_norm_variable(
        GFS_DOMAIN(sim), output->v, NULL,
        FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL, output->maxlevel,
        output->condition ? cell_condition : NULL, output->condition);
    fprintf(GFS_OUTPUT(event)->file->fp,
            "%s time: %g first: % 10.3e second: % 10.3e infty: % 10.3e\n",
            output->name, sim->time.t, norm.first, norm.second, norm.infty);
    return TRUE;
  }
  return FALSE;
}
...
static gboolean gfs_output_scalar_stats_event(GfsEvent *     event,
                                              GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_output_scalar_stats_class())->parent_class)
            ->event)(event, sim)) {
    GfsOutputScalar *output = GFS_OUTPUT_SCALAR(event);
    GtsRange         stats  = gfs_domain_stats_variable(
        GFS_DOMAIN(sim), output->v, FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL,
        output->maxlevel, output->condition ? cell_condition : NULL,
        output->condition);
    gchar *format;
    if (output->format) {
    ...
    fprintf(GFS_OUTPUT(event)->file->fp, format, output->name, sim->time.t,
...
static void add(FttCell *cell, SumData *s) {
  gdouble vol = s->w ? gfs_function_value(s->w, cell)
                     : gfs_cell_volume(cell, s->v->domain);
...
static void gfs_output_scalar_maxima_destroy(GtsObject *o) {
  ...
  (*GTS_OBJECT_CLASS(gfs_output_scalar_maxima_class())->parent_class->destroy)(
      o);
}
...
static void gfs_output_scalar_maxima_read(GtsObject **o, GtsFile *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_output_scalar_maxima_class())->parent_class->read)(o,
                                                                            fp);
  if (fp->type == GTS_ERROR)
    return;
...
static void gfs_output_scalar_maxima_write(GtsObject *o, FILE *fp) {
  (*GTS_OBJECT_CLASS(gfs_output_scalar_maxima_class())->parent_class->write)(
      o, fp);
...
static gboolean gfs_output_scalar_maxima_event(GfsEvent *     event,
                                               GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_output_scalar_maxima_class())->parent_class)
            ->event)(event, sim)) {
    GfsOutputScalar *      output = GFS_OUTPUT_SCALAR(event);
    ...
    output_scalar_traverse(output, FTT_PRE_ORDER,
                           FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL,
                           output->maxlevel, (FttCellTraverseFunc)maxima, m);
    for (i = 0; i < m->N; i++)
      fprintf(GFS_OUTPUT(event)->file->fp,
              "%s time: %g #: %d x: %g y: %g z: %g value: %g\n", output->name,
              sim->time.t, i, m->m[0][i], m->m[1][i], m->m[2][i], m->m[3][i]);
    return TRUE;
  }
  return FALSE;
}
...
static void gfs_output_scalar_histogram_destroy(GtsObject *o) {
  ...
  (*GTS_OBJECT_CLASS(gfs_output_scalar_histogram_class())
...
static void gfs_output_scalar_histogram_read(GtsObject **o, GtsFile *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_output_scalar_histogram_class())->parent_class->read)(
      o, fp);
  if (fp->type == GTS_ERROR)
    return;
...
static void gfs_output_scalar_histogram_write(GtsObject *o, FILE *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_output_scalar_histogram_class())->parent_class->write)(
      o, fp);
...
static gboolean gfs_output_scalar_histogram_event(GfsEvent *     event,
                                                  GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_output_scalar_histogram_class())->parent_class)
            ->event)(event, sim)) {
    ...
    if (h->dt > 0.) {
      ...
      if (gfs_restore_floating_point_exceptions()) {
        ...
        if (h->yf)
          s = g_strconcat(s, "\n", gfs_function_description(h->yf, FALSE),
                          NULL);
        /* fixme: memory leaks */
...
static void gfs_output_droplet_sums_destroy(GtsObject *object) {
  ...
  (*GTS_OBJECT_CLASS(gfs_output_droplet_sums_class())->parent_class->destroy)(
      object);
}

static void gfs_output_droplet_sums_read(GtsObject **o, GtsFile *fp) {
  (*GTS_OBJECT_CLASS(gfs_output_droplet_sums_class())->parent_class->read)(o,
                                                                           fp);
  if (fp->type == GTS_ERROR)
    return;
  ...
  if (fp->type == GTS_STRING) {
    if (!(d->tag = gfs_domain_get_or_add_variable(domain, fp->token->str,
                                                  "Droplet index"))) {
...
static void gfs_output_droplet_sums_write(GtsObject *o, FILE *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_output_droplet_sums_class())->parent_class->write)(o,
                                                                            fp);
...
static gboolean gfs_output_droplet_sums_event(GfsEvent *     event,
                                              GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_output_droplet_sums_class())->parent_class)
            ->event)(event, sim)) {
    ...
    if (p.n > 0) {
      ...
      output_scalar_traverse(GFS_OUTPUT_SCALAR(event), FTT_PRE_ORDER,
      ...
      guint i;
      for (i = 0; i < p.n; i++)
        fprintf(GFS_OUTPUT(event)->file->fp, format, sim->time.t, i + 1,
                p.v[i].f);
      g_free(p.v);
      g_free(format);
    }
...
static void output_error_norm_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_output_error_norm_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_output_error_norm_class())->parent_class->read)(o,
                                                                           fp);
  if (fp->type == GTS_ERROR)
    return;
  ...
  while (fp->type != GTS_ERROR && fp->type != '}') {
    ...
    if (fp->type != GTS_STRING) {
      ...
      if (!(n->v = gfs_domain_get_or_add_variable(domain, fp->token->str,
                                                  "Error field"))) {
...
static void output_error_norm_write(GtsObject *o, FILE *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_output_error_norm_class())->parent_class->write)
    (*GTS_OBJECT_CLASS(gfs_output_error_norm_class())->parent_class->write)(o,
                                                                            fp);
  fputs(" { s = ", fp);
...
static void substract(FttCell *cell, GfsOutputScalar *o) {
  GFS_VALUE(cell, GFS_OUTPUT_ERROR_NORM(o)->v) =
      GFS_VALUE(cell, o->v) - GFS_VALUE(cell, GFS_OUTPUT_ERROR_NORM(o)->v);
}
...
static gboolean gfs_output_error_norm_event(GfsEvent *     event,
                                            GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_output_error_norm_class())->parent_class)
            ->event)(event, sim)) {
    GfsDomain *         domain = GFS_DOMAIN(sim);
    GfsOutputScalar *   output = GFS_OUTPUT_SCALAR(event);
    ...
    if (enorm->relative) {
      gfs_catch_floating_point_exceptions();
      output_scalar_traverse(
          output, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL,
          output->maxlevel, (FttCellTraverseFunc)reference_solution, output);
      gfs_restore_fpe_for_function(enorm->s);
      snorm = gfs_domain_norm_variable(
          domain, enorm->v, enorm->w, FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL,
          output->maxlevel, output->condition ? cell_condition : NULL,
          output->condition);
      output_scalar_traverse(
          output, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL,
          output->maxlevel, (FttCellTraverseFunc)substract, output);
    } else {
      gfs_catch_floating_point_exceptions();
      output_scalar_traverse(
          output, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL,
          output->maxlevel, (FttCellTraverseFunc)compute_error, output);
      gfs_restore_fpe_for_function(enorm->s);
    }
    norm = gfs_domain_norm_variable(
        domain, enorm->v, enorm->w, FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL,
        output->maxlevel, output->condition ? cell_condition : NULL,
        output->condition);
    if (GFS_OUTPUT_ERROR_NORM(event)->unbiased) {
      ...
      output_scalar_traverse(
          output, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL,
          output->maxlevel, (FttCellTraverseFunc)remove_bias, data);
      norm = gfs_domain_norm_variable(
          domain, enorm->v, enorm->w, FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL,
          output->maxlevel, output->condition ? cell_condition : NULL,
          output->condition);
    ...
    if (enorm->relative) {
      if (snorm.first > 0.)
        norm.first /= snorm.first;
      if (snorm.second > 0.)
        norm.second /= snorm.second;
      if (snorm.infty > 0.)
        norm.infty /= snorm.infty;
    }
    gchar *format;
    if (output->format) {
      gchar *f = output->format;
      ...
      format = g_strdup("%s time: %g first: %10.3e second: %10.3e infty: "
                        "%10.3e bias: %10.3e\n");
    fprintf(GFS_OUTPUT(event)->file->fp, format, output->name, sim->time.t,
  ...
  if ((*GFS_EVENT_CLASS(
    ...
    if (enorm->unbiased) {
      output_scalar_traverse(
          output, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL,
          output->maxlevel, (FttCellTraverseFunc)compute_error, output);
      bias = gfs_domain_norm_variable(
                 GFS_DOMAIN(sim), enorm->v, NULL,
                 FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL, output->maxlevel,
                 output->condition ? cell_condition : NULL, output->condition)
                 .bias;
    }
    ...
    output_scalar_traverse(
        output, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL,
        output->maxlevel, (FttCellTraverseFunc)compute_correlation, data);
    gfs_restore_fpe_for_function(enorm->s);
    if (v == NULL) {
      gts_object_destroy(GTS_OBJECT(enorm->v));
      enorm->v = NULL;
...
GfsOutputClass *gfs_output_correlation_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_output_correlation_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass =
        gts_object_class_new(GTS_OBJECT_CLASS(gfs_output_error_norm_class()),
...
static gboolean gfs_output_squares_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_output_squares_class())->parent_class)
            ->event)(event, sim)) {
    GfsOutputScalar *output = GFS_OUTPUT_SCALAR(event);

    gfs_write_squares(GFS_DOMAIN(sim), output->v, output->min, output->max,
                      FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL, output->maxlevel,
                      NULL, GFS_OUTPUT(event)->file->fp);
    return TRUE;
  }
  return FALSE;
}
...
static void gfs_output_streamline_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_output_streamline_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_output_streamline_class())->parent_class->read)(o,
                                                                           fp);
  if (fp->type == GTS_ERROR)
    return;
...
static void gfs_output_streamline_write(GtsObject *o, FILE *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_output_streamline_class())->parent_class->write)
    (*GTS_OBJECT_CLASS(gfs_output_streamline_class())->parent_class->write)(o,
                                                                            fp);
...
static gboolean gfs_output_streamline_event(GfsEvent *     event,
                                            GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_output_streamline_class())->parent_class)
            ->event)(event, sim)) {
    FttVector p = GFS_OUTPUT_STREAMLINE(event)->p;
    gfs_simulation_map(sim, &p);
    GList *stream = gfs_streamline_new(
        GFS_DOMAIN(sim), gfs_domain_velocity(GFS_DOMAIN(sim)), p,
        GFS_OUTPUT_SCALAR(event)->v, 0., 0., TRUE, NULL, NULL);
...
static void gfs_output_ppm_read(GtsObject **o, GtsFile *fp) {
  if (GTS_OBJECT_CLASS(gfs_output_ppm_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_output_ppm_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR)
    return;
#if (!FTT_2D)
  if (!GFS_IS_OCEAN(gfs_object_simulation(*o))) {
    gts_file_error(fp, "In more than two dimensions PPM output is possible\n"
...
static gboolean gfs_output_ppm_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_output_ppm_class())->parent_class)
            ->event)(event, sim)) {
    GfsOutputScalar *output = GFS_OUTPUT_SCALAR(event);
#if FTT_2D
    GfsDomain *domain = GFS_DOMAIN(sim);
#else  /* 3D */
    GfsDomain *domain =
        GFS_IS_OCEAN(sim) ? GFS_OCEAN(sim)->toplayer : GFS_DOMAIN(sim);
#endif /* 3D */

    gfs_write_ppm(domain, output->condition, output->v, output->min,
                  output->max, FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL,
                  output->maxlevel, GFS_OUTPUT(event)->file->fp,
...
static gboolean gfs_output_grd_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_output_grd_class())->parent_class)
            ->event)(event, sim)) {
    GfsOutputScalar *output = GFS_OUTPUT_SCALAR(event);
    gfs_write_grd(sim, output->condition, output->v,
                  FTT_TRAVERSE_LEAFS | FTT_TRAVERSE_LEVEL, output->maxlevel,
                  GFS_OUTPUT(event)->file->fp, GFS_OUTPUT(event)->parallel,
                  TRUE);
    return TRUE;
  }
  return FALSE;
...
static void output_object_read(GtsObject **o, GtsFile *fp) {
  ...
  output->object = gfs_object_from_name(GFS_DOMAIN(gfs_object_simulation(*o)),
                                        fp->token->str);
