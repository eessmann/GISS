...
static gdouble minmod_limiter(gdouble r) { return generic_limiter(r, 1.); }

static gdouble superbee_limiter(gdouble r) { return generic_limiter(r, 2.); }

static gdouble sweby_limiter(gdouble r) { return generic_limiter(r, 1.5); }

static gdouble center_limited_gradient_full(FttCell *cell, FttComponent c,
                                            guint v,
...
static gdouble center_limited_gradient(FttCell *cell, FttComponent c, guint v,
                                       gdouble (*limiter)(gdouble)) {
  FttDirection d = 2 * c;
  FttCellFace  f1, f2;
  f1 = gfs_cell_face(cell, FTT_OPPOSITE_DIRECTION(d));
  f2 = gfs_cell_face(cell, d);
  if (!GFS_IS_MIXED(cell) && (!f1.neighbor || !GFS_IS_MIXED(f1.neighbor)) &&
  ...
  if (GFS_IS_MIXED(cell)) {
    ...
    if (s->s[2 * c] < s->s[2 * c + 1]) {
      if (f1.neighbor) {
        ...
        if (v == 2)
          vr = v0 -
               2. * (n.x * GFS_VALUEI(cell, 2) + n.y * GFS_VALUEI(cell, 3)) *
                   n.x;
        else if (v == 3)
          vr = v0 -
               2. * (n.x * GFS_VALUEI(cell, 2) + n.y * GFS_VALUEI(cell, 3)) *
        ...
        gdouble gs = ((v0 - v1) * x2 == 0. || x1 == 0.)
                         ? 0.
                         : (*limiter)((vr - v0) * x1 / ((v0 - v1) * x2)) *
                               (v0 - v1) / x1;
        return (s->s[2 * c] * g + (s->s[2 * c + 1] - s->s[2 * c]) * gs) /
               s->s[2 * c + 1];
      } else
        return 0;
    }
    /* solid is on the left side of the cell */
      ...
      if (f2.neighbor) {
        ...
        x2         = ((&cm2.x)[c] - (&cm.x)[c]) / h;
        gdouble gs = ((v0 - vr) * x2 == 0. || x1 == 0.)
                         ? 0.
                         : (*limiter)((v2 - v0) * x1 / ((v0 - vr) * x2)) *
                               (v0 - vr) / x1;
        return (s->s[2 * c + 1] * g + (s->s[2 * c] - s->s[2 * c + 1]) * gs) /
               s->s[2 * c];
      } else
        return 0;
    }
...
static gdouble center_minmod_gradient(FttCell *cell, FttComponent c, guint v) {
  return center_limited_gradient(cell, c, v, minmod_limiter);
}

static gdouble center_superbee_gradient(FttCell *cell, FttComponent c,
                                        guint v) {
  return center_limited_gradient(cell, c, v, superbee_limiter);
}

static gdouble center_sweby_gradient(FttCell *cell, FttComponent c, guint v) {
...
static gdouble min(gdouble a, gdouble b) { return a < b ? a : b; }

static gdouble max(gdouble a, gdouble b) { return a > b ? a : b; }
 ...
 */
static void riemann_hllc(const GfsRiver *r, const gdouble *uL,
                         const gdouble *uR, gdouble *f) {
  ...
  if (0. <= SL)
    ...
    f[U] =
        (SR * fL[U] - SL * fR[U] + SL * SR * (uR[H] * uR[U] - uL[H] * uL[U])) /
        (SR - SL);
...
#define PARENT_TRACER(v)                                                       \
  ((v)->vector[0]) /* hack: use vector[0] to store parent tracer */
...
static double density(GfsRiver *r, int l, const gdouble *u, FttCell *cell) {
  ...
  for (int i = 0; i < r->nt; i++)
    GFS_VALUE(cell, PARENT_TRACER(r->v[T(i, l)])) =
        u[T(i, l)] / sim->physical_params.L;
...
static void riemann_kinetic(const GfsRiver *r, const gdouble *uL,
                            const gdouble *uR, gdouble *f) {
  gdouble *dz = r->dz;
  f[H]        = 0.;
  int l;
...
#define CFL_CLAMP(u, umax)                                                     \
  (fabs(u) <= (umax) ? (u) : (u) > 0. ? (umax) : -(umax))

static double left(const GfsRiver *r, const FttCellFace *f, int i, double a) {
  return GFS_VALUE(f->cell, r->v1[i]) +
         a * GFS_VALUE(f->cell, r->dv[f->d / 2][i]);
}

static double right(const GfsRiver *r, const FttCellFace *f, int i, double a) {
  return GFS_VALUE(f->neighbor, r->v1[i]) -
         a * GFS_VALUE(f->neighbor, r->dv[f->d / 2][i]);
}

static void face_fluxes(FttCellFace *face, GfsRiver *r) {
  gdouble eta  = GFS_VALUE(face->cell, r->v1[H]),
  ...
  if (GFS_IS_MIXED(face->neighbor)) {
    ...
    gfs_face_ca(
        face,
        &ca); /* fixme?: this is not symmetric with the above for face->cell */
  ...
  static Sym sym[4] = {
      {U, 1., V, 1.}, {U, -1., V, -1.}, {V, 1., U, -1.}, {V, -1., U, 1.}};
  Sym *s = &sym[face->d];
  ...
  if (r->variable_density) {
    ...
    for (l = 0; l < r->nlayers; l++) {
      ...
      for (int i = 0; i < r->nt; i++) {
        u[T(i, l)] = etal > 0. ? GFS_VALUE(face->cell, r->v1[T(i, l)]) / etal
                               : 0.; /* fixme! */
        un[T(i, l)] = etanl > 0.
                          ? GFS_VALUE(face->neighbor, r->v1[T(i, l)]) / etanl
                          : 0.; /* fixme! */
      }
    }
  ...
  if (r->variable_density) {
   ...
   slope source term "Sc_{i,j,p}" of An2012, equations (11) and (12) */
    ...
    SbR =
        r->g / 2. * (uR[H] * uR[H] - etaR * etaR - (etaR + etan) * (zbR - zbn));
    SbL0 = SbR0 = 0.;
  }
  ...
  for (l = 0; l < r->nlayers; l++) {
    ...
    if (r->variable_density) {
      ...
      GFS_VALUE(face->cell, r->flux[s->u + 2 * l]) -=
          s->du * dt * (f[U + 3 * l] - SbL + SbL0);
      GFS_VALUE(face->neighbor, r->flux[s->u + 2 * l]) +=
          s->du * dt * (f[U + 3 * l] - SbR + SbR0) / nn;
      SbL0 = SbL;
      SbR0 = SbR;
    } else { /* constant density */
      GFS_VALUE(face->cell, r->flux[s->u + 2 * l]) -=
          s->du * dt * (f[U + 3 * l] - dz * SbL);
      GFS_VALUE(face->neighbor, r->flux[s->u + 2 * l]) +=
          s->du * dt * (f[U + 3 * l] - dz * SbR) / nn;
    }
    GFS_VALUE(face->cell, r->flux[s->v + 2 * l]) -= s->dv * dt * f[V + 3 * l];
    GFS_VALUE(face->neighbor, r->flux[s->v + 2 * l]) +=
        s->dv * dt * f[V + 3 * l] / nn;
    /* mass flux between layers */
    G += dt *
         (f[HL + 3 * l] -
          dz * f[H]); /* eq. (5.109) of Audusse2011a and (75) of Audusse2011b */
    if (l < r->nlayers - 1) {
...
static gdouble limited_gradient(const FttCell *cell, const GfsRiver *r, int i0,
                                int i1, int i2, int l,
                                gdouble (*limiter)(gdouble)) {
...
#define limited_gradient_u(l)                                                  \
  limited_gradient(cell, r, c + 2 * (l), c + 2 * ((l)-1), c + 2 * ((l) + 1),   \
                   l, minmod_limiter)
#define limited_gradient_t(l)                                                  \
  limited_gradient(cell, r, T(i, l), T(i, (l)-1), T(i, (l) + 1), l,            \
                   minmod_limiter)
  ...
  if (eta > r->dry)
    for (int l = 0; l < r->nlayers - 1; l++) {
      ...
      for (FttComponent c = U; c <= V; c++) {
        double flux = G < 0. ? G * (GFS_VALUE(cell, r->v1[c + 2 * l]) +
                                    limited_gradient_u(l) / 2.)
                             : G * (GFS_VALUE(cell, r->v1[c + 2 * (l + 1)]) -
                                    limited_gradient_u(l + 1) / 2.);
        GFS_VALUE(cell, r->flux[c + 2 * l]) += flux;
        GFS_VALUE(cell, r->flux[c + 2 * (l + 1)]) -= flux;
      }
      for (int i = 0; i < r->nt; i++) {
        double flux = G < 0. ? G * (GFS_VALUE(cell, r->v1[T(i, l)]) +
                                    limited_gradient_t(l) / 2.)
                             : G * (GFS_VALUE(cell, r->v1[T(i, l + 1)]) -
                                    limited_gradient_t(l + 1) / 2.);
...
static void solid_boundary_fluxes(FttCell *cell, GfsRiver *r) {
  ...
  gfs_cell_cm(cell, &cm);
  gdouble hs = hh + 2. *
                        ((s->ca.x - cm.x) * GFS_VALUE(cell, r->dv[0][H]) +
                         (s->ca.y - cm.y) * GFS_VALUE(cell, r->dv[1][H])) /
                        h;
  gdouble zbs = 2. *
                ((s->ca.x - cm.x) * GFS_VALUE(cell, r->dv[0][ZB]) +
                 (s->ca.y - cm.y) * GFS_VALUE(cell, r->dv[1][ZB])) /
                h;
  gdouble hszbs = r->dt / h * r->g / 2. *
                  (
                      /* the normal component of the velocity is zero at the
                         solid boundary. */
                      hs * hs +
                      /* Second-order correction for slope source term
              ("Sc_{i,j,s}" of An2012, equation (27) */
                      (hs + hh) * zbs);
  FttVector n;
  gfs_solid_normal(cell, &n);
...
static void metric_sources(FttCell *cell, GfsRiver *r) {
  if (GFS_VALUE(cell, r->v1[H]) > r->dry) {
    ...
    gdouble phiu = GFS_VALUE(cell, r->v1[U]), phiv = GFS_VALUE(cell, r->v1[V]);
    gdouble fG = phiv * dh_dl - phiu * dh_dt;
    gdouble g  = GFS_SIMULATION(r)->physical_params.g;

    gdouble eta = GFS_VALUE(cell, r->v1[H]);
    GFS_VALUE(cell, r->v[U]) +=
        r->dt * (g * eta * eta / 2. * dh_dl + fG * phiv) / dldh;
    GFS_VALUE(cell, r->v[V]) +=
        r->dt * (g * eta * eta / 2. * dh_dt - fG * phiu) / dldh;
  }
}
...
static void vertical_diffusion(double *u, const double *mu, const double *dz,
                               int N, double dt, double dut, double lambdab,
                               double ub, double k, Tridiagonal *t,
  ...
  t->b[0] =
      1. + a[0] + (1. - (2. * lambdab - dz[0]) / (2. * lambdab + dz[0])) * am;
  t->c[0] = -a[0];
...
static void domain_vertical_diffusion(GfsRiver *r, double dt) {
  ...
  FttCellTraverse *t = gfs_domain_cell_traverse_new(
      GFS_DOMAIN(r), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1);
  FttCell *cell;
  while ((cell = ftt_cell_traverse_next(t))) {
    ...
    if (h > r->dry) {
      ...
      double lambdab = 0., ub = 0.,
             dut = r->dut ? gfs_function_value(r->dut, cell) : 0.;
...
static void advance(GfsRiver *r, gdouble dt) {
  ...
  r->dt = dt;
  gfs_domain_face_traverse(domain, FTT_XYZ, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                           -1, (FttFaceTraverseFunc)face_fluxes, r);
  if (r->nlayers > 1)
    gfs_domain_traverse_leaves(domain, (FttCellTraverseFunc)vertical_advection,
                               r);
  ...
  for (i = 0; i < r->nvar; i++) {
    ...
    gfs_domain_traverse_merged(
        domain, (GfsMergedTraverseFunc)gfs_advection_update, &par);
  ...
  if (r->nlayers > 1) {
    ...
    for (int l = 0; l < r->nlayers; l++)
      for (FttComponent c = 0; c < 2; c++)
        gfs_domain_variable_centered_sources(domain, u[c], r->v[U + c + 2 * l],
                                             dt);
    /* and "global" sources on tracers */
    for (int i = 0; i < r->nt; i++)
      for (r->l = 0; r->l < r->nlayers; r->l++)
        gfs_domain_variable_centered_sources(
            domain, PARENT_TRACER(r->v[T(i, r->l)]), r->v[T(i, r->l)], dt);
  }
...
static void cell_gradients(FttCell *cell, const GfsRiver *r) {
  FttComponent c;
  guint        v;
  ...
  if (GFS_VALUE(cell, r->v[H]) <= r->dry) {
    ...
    for (c = 0; c < FTT_DIMENSION; c++) {
      ...
      /* recontruct Zb + eta rather than Zb: see Theorem 3.1 of Audusse et al,
       * 2004 */
      GFS_VALUE(cell, r->dv[c][ZB]) =
          (*r->gradient)(cell, c, r->h->i) / 2. - GFS_VALUE(cell, r->dv[c][H]);
    }
  }
}
...
static void domain_traverse_all_leaves(GfsDomain *         domain,
  ...
  for (p.d = 0; p.d < FTT_NEIGHBORS; p.d++)
    gfs_domain_cell_traverse_boundary(domain, p.d, FTT_PRE_ORDER,
  ...
  for (FttDirection d = 0; d < FTT_NEIGHBORS; d++)
    if (GFS_IS_BOUNDARY(box->neighbor[d])) {
      ...
      if (r->nlayers > 1) {
        ...
        if (bc) {
          /* applies explicit BC on global flux on all layer fluxes which have
           * no explicit BCs */
          for (int l = 0; l < r->nlayers; l++) {
...
static void river_run(GfsSimulation *sim) {
  ...
  while (sim->time.t < sim->time.end && sim->time.i < sim->time.iend) {
    ...
    /* events */
    gts_container_foreach(GTS_CONTAINER(sim->events), (GtsFunc)gfs_event_do,
                          sim);
  ...
  gts_container_foreach(GTS_CONTAINER(sim->events), (GtsFunc)gts_object_destroy,
                        NULL);
}

static gdouble maximum_face_metric(FttCell *cell, GfsDomain *domain,
                                   FttComponent c) {
...
static gdouble river_cfl(GfsSimulation *sim) {
  GfsRiver *r = GFS_RIVER(sim);
  r->cfl      = G_MAXDOUBLE;
  gfs_domain_traverse_leaves(GFS_DOMAIN(sim), (FttCellTraverseFunc)minimum_cfl,
                             r);
...
static void river_tracer_read(GtsObject **o, GtsFile *fp) {
  ...
  if (r->nlayers > 1)
    for (int l = 0; l < r->nlayers; l++) {
      ...
      gchar *description =
          g_strdup_printf("%s for layer %d", v->description, l);
      r->v[T(r->nt, l)] =
          gfs_domain_get_or_add_variable(domain, name, description);
...
static void river_read(GtsObject **o, GtsFile *fp) {
  ...
  if (fp->type == '{') {
    ...
    GtsFileVariable var[] = {{GTS_UINT, "time_order", TRUE, &river->time_order},
    ...
    if (var[5].set) {
      if (river->nlayers > 1 && !river->nu) {
        gts_file_variable_error(
            fp, var, "k", "Navier condition requires viscosity to be set");
        return;
      }
    } else {
...
static gdouble cell_velocity(FttCell *cell, FttCellFace *face, GfsRiver *r) {
  ...
  return D > r->dry
             ? L * gfs_vector_norm(cell, gfs_domain_velocity(GFS_DOMAIN(r))) / D
             : 0.;
}
...
static gdouble cell_velocity2(FttCell *cell, FttCellFace *face, GfsRiver *r) {
  ...
  return D > r->dry
             ? L * L *
                   gfs_vector_norm2(cell, gfs_domain_velocity(GFS_DOMAIN(r))) /
                   (D * D)
             : 0.;
}
...
static GfsVariable *massflux(GfsDomain *domain, int l) {
  ...
  gchar *description =
      g_strdup_printf("Mass flux between layer %d and %d", l + 1, l);
...
static void allocate_river(GfsRiver *r, int start, int nl) {
  ...
  r->flux      = g_realloc(r->flux, r->nvar * sizeof(GfsVariable *));
  r->massflux =
      g_realloc(r->massflux, (r->nlayers - 1) * sizeof(GfsVariable *));
  ...
  for (l = start; l < r->nlayers; l++) {
    ...
    r->dv[0][U + 2 * l] = tensor[0][0] =
        gfs_domain_add_variable(domain, NULL, NULL);
    r->dv[1][U + 2 * l] = tensor[0][1] =
        gfs_domain_add_variable(domain, NULL, NULL);
    r->dv[0][V + 2 * l] = tensor[1][0] =
        gfs_domain_add_variable(domain, NULL, NULL);
    r->dv[1][V + 2 * l] = tensor[1][1] =
        gfs_domain_add_variable(domain, NULL, NULL);
...
static void river_init(GfsRiver *r) {
  ...
  gts_object_destroy(
      GTS_OBJECT(gfs_variable_from_name(domain->variables, "Pmac")));
  ...
  GfsVariable *u[2];
  r->dv[0][H] = u[0] = gfs_domain_add_variable(
      domain, "Px", "x-component of the depth gradient");
  r->dv[1][H] = u[1] = gfs_domain_add_variable(
      domain, "Py", "y-component of the depth gradient");
  gfs_variable_set_vector(u, 2);

  r->dv[0][ZB] = u[0] =
      gfs_domain_add_variable(domain, "Zbx", "x-component of the bed slope");
  r->dv[1][ZB] = u[1] =
      gfs_domain_add_variable(domain, "Zby", "y-component of the bed slope");
  gfs_variable_set_vector(u, 2);
  ...
  GfsDerivedVariable *dv =
      gfs_derived_variable_from_name(domain->derived_variables, "Velocity");
...
GfsSimulationClass *gfs_river_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo info = {"GfsRiver",
                               sizeof(GfsRiver),
                               ...
                               (GtsArgGetFunc)NULL};
    klass =
        gts_object_class_new(GTS_OBJECT_CLASS(gfs_simulation_class()), &info);
  }
...
static void traverse_layers(GfsDomain *domain, FttCellTraverseFunc func,
                            gpointer data) {
  GfsRiver *    r = GFS_RIVER(domain);
  GfsVariable **u = gfs_domain_velocity(domain);
  for (r->l = 0; r->l < r->nlayers; r->l++) {
...
static gdouble cell_sigma(FttCell *cell, FttCellFace *face,
                          GfsSimulation *sim) {
  GfsRiver *r = GFS_RIVER(sim);
...
static gdouble cell_z(FttCell *cell, FttCellFace *face, GfsSimulation *sim) {
  GfsRiver *r  = GFS_RIVER(sim);
  double    zb = cell ? GFS_VALUE(cell, r->zb)
                   : gfs_face_interpolated_value(face, r->zb->i);
  double h = cell ? GFS_VALUE(cell, r->v[H])
                  : gfs_face_interpolated_value(face, r->v[H]->i);
...
static void gfs_layers_read(GtsObject **o, GtsFile *fp) {
  ...
  for (FttComponent c = 0; c < 2; c++)
    for (int l = 0; l < r->nlayers; l++) {
      ...
      gchar *description = g_strdup_printf(
          "%s-component of the fluid flux for layer %d", c ? "y" : "x", l);
      GfsVariable *v =
          gfs_domain_get_or_add_variable(domain, name, description);
      g_free(name);
      g_free(description);
  ...
  GfsDerivedVariable *z =
      gfs_derived_variable_from_name(domain->derived_variables, "z");
  z->func                     = cell_z;
...
static void gfs_layers_init(GfsLayers *l) { l->nl = 1; }

GtsObjectClass *gfs_layers_class(void) {
  static GtsObjectClass *klass = NULL;

  if (klass == NULL) {
    GtsObjectClassInfo info = {"GfsLayers",
                               sizeof(GfsLayers),
...
static void subcritical(FttCellFace *f, GfsBc *b) {
  ...
  GFS_VALUE(f->cell, b->v) =
      ...
      (FTT_FACE_DIRECT(f) ? -1. : 1.) * 2. * hi *
          (sqrt(river->g * hi) - sqrt(river->g * MAX(hb, 0.)));
}
...
static void bc_subcritical_read(GtsObject **o, GtsFile *fp) {
  ...
  if (!GFS_IS_RIVER(bc->v->domain)) {
    gts_file_error(
        fp, "GfsBcSubcritical only makes sense for GfsRiver simulations");
    return;
  }
...
static void discharge_elevation_destroy(GtsObject *o) {
  ...
  (*GTS_OBJECT_CLASS(gfs_discharge_elevation_class())->parent_class->destroy)(
      o);
}

static void discharge_elevation_read(GtsObject **o, GtsFile *fp) {
  (*GTS_OBJECT_CLASS(gfs_discharge_elevation_class())->parent_class->read)(o,
                                                                           fp);
  if (fp->type == GTS_ERROR)
    return;
  ...
  if (!GFS_IS_RIVER(domain)) {
    gts_file_error(
        fp, "GfsDischargeElevation only makes sense for GfsRiver simulations");
    return;
  }
  ...
  GFS_CONSTANT(bd)->derived->description =
      g_strdup("Elevation for a given discharge");
}

static void discharge_elevation_write(GtsObject *o, FILE *fp) {
  (*GTS_OBJECT_CLASS(gfs_discharge_elevation_class())->parent_class->write)(o,
                                                                            fp);
  gfs_function_write(GFS_DISCHARGE_ELEVATION(o)->Q, fp);
  if (gfs_function_get_constant_value(GFS_DISCHARGE_ELEVATION(o)->profile) !=
      0.)
...
static void boundary_flux(FttCellFace *f, GfsDischargeElevation *b) {
  ...
  if (profile != GFS_NODATA) {
    ...
    GFS_VALUE(f->cell, river->v1[H]) =
        MAX(0., profile + GFS_CONSTANT(b)->val -
...
static void traverse_dirichlet_boundaries(GfsBox *               box,
                                          GfsDischargeElevation *bd) {
  FttDirection d;
...
static gboolean discharge_elevation_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_discharge_elevation_class())->parent_class)
            ->event)(event, sim)) {
    ...
    gts_container_foreach(GTS_CONTAINER(sim),
                          (GtsFunc)traverse_dirichlet_boundaries, bd);
    ...
    do {
      ...
      n++;
             ...
             (n < nitermin || fabs(Q - bd->flow) / Q > bd->tolerance));
    if (n == nitermax)
      g_warning("discharge_elevation_event() did not converge after %d "
                "iterations: %g",
                n, fabs(Q - bd->flow) / Q);
    c->val *= L;
...
static void source_pipe_write(GtsObject *o, FILE *fp) {
  ...
  GfsSourcePipe *p = GFS_SOURCE_PIPE(o);
  fprintf(fp, " (%f,%f) (%f,%f) %f", p->start.x, p->start.y, p->end.x, p->end.y,
          p->diameter);
}

#define DQ (1e-4 / L3)

static double flow_rate_Q(double z1, double h1, double z2, double h2, double l,
                          double g, GfsSourcePipe *p, double a1, double a2,
                          double Q) {
...
static gboolean source_pipe_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*gfs_event_class()->event)(event, sim)) {
    ...
    /* fixme: this won't work in parallel if the ends of the pipe are on
     * different PEs */
    ...
    if (p->scell && p->ecell && p->scell != p->ecell) {
      ...
      gdouble      h1 = MAX(L * GFS_VALUE(p->scell, h), 0.),
              z1      = L * GFS_VALUE(p->scell, zb);
      gdouble h2      = MAX(L * GFS_VALUE(p->ecell, h), 0.),
              z2      = L * GFS_VALUE(p->ecell, zb);
...
properly (e.g. won't work for MetricLonLat) */
      ...
      gdouble a1 = L2 * gfs_cell_volume(p->scell, GFS_DOMAIN(sim)) /
                   sim->advection_params.dt;
      gdouble a2 = L2 * gfs_cell_volume(p->ecell, GFS_DOMAIN(sim)) /
                   sim->advection_params.dt;

      /* secant-bisection root-finding: solves flow_rate(h, l, Q) - Q = 0 for
       * the flow rate Q */
      p->Q       = (*p->flow_rate)(z1, h1, z2, h2, l, g, p) / L3;
      gdouble Q1 = p->Q * 2.;
      gdouble v1 =
          flow_rate_Q(z1, h1, z2, h2, l, g, p, a1, a2, Q1 * L3) / L3 - Q1;
      gdouble Q2 = 0.;
      gdouble v2 = p->Q;
      if (fabs(v1) > DQ && fabs(v2) > DQ) {
        ...
        if (v1 * v2 >= 0.)
          ...
          do {
            Qb = p->Q;
            gdouble v =
                flow_rate_Q(z1, h1, z2, h2, l, g, p, a1, a2, p->Q * L3) / L3 -
                p->Q;
            if (v < 0.) {
              v1 = v;
              Q1 = p->Q;
          ...
          if (nitermax == 0)
            g_warning("source_pipe_event: failed to converge! %g %g", p->Q * L3,
                      fabs(p->Q - Qb) * L3);
        }
      }
    }
...
static gdouble source_pipe_value(GfsSourceGeneric *s, FttCell *cell,
                                 GfsVariable *v) {
...
static double pipe_flow_rate(
    double z1, double h1, /* terrain elevation and flow depth at inlet */
...
GfsSourceGenericClass *gfs_source_pipe_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo info = {"GfsSourcePipe",
