...
static void boundary_mpi_read(GtsObject **o, GtsFile *fp) {
  if (fp->type == GTS_INT) {
    ...
    if (fp->type != GTS_INT)
      ...
      gts_file_next_token(fp);
      gfs_boundary_periodic_rotate(GFS_BOUNDARY_PERIODIC(*o), rotate,
                                   orientation);
    }
  } else
...
#define MATCHING_TAG(boundary)                                                 \
  (tag_shift * FTT_OPPOSITE_DIRECTION(GFS_BOUNDARY_PERIODIC(boundary)->d) +    \
...
#ifdef DEBUG
    fprintf(DEBUG,
            "%d send to %d with tag %d match variable size: bid: %d mid: %d d: "
            "%d dp: %d\n",
            domain->pid, mpi->process, TAG(bb), bb->box->id,
            GFS_BOUNDARY_MPI(bb)->id, bb->d, boundary->d);
    fflush(DEBUG);
#endif
    MPI_Isend(&boundary->sndcount, 1, MPI_UNSIGNED, mpi->process, TAG(bb),
              mpi->comm, &(mpi->request[mpi->nrequest++]));
    gts_range_add_value(&domain->mpi_messages, sizeof(guint));
  }
#ifdef DEBUG
  fprintf(DEBUG, "    %d send to %d with tag %d, size %d\n", domain->pid,
          mpi->process, TAG(bb), boundary->sndcount);
  fflush(DEBUG);
#endif
  MPI_Isend(boundary->sndbuf->data, boundary->sndcount, MPI_DOUBLE,
            mpi->process, TAG(bb), mpi->comm, &(mpi->request[mpi->nrequest++]));
...
static void receive(GfsBoundary *bb, FttTraverseFlags flags, gint max_depth) {
...
#ifdef DEBUG
    fprintf(DEBUG,
            "%d wait on %d with tag %d for match variable size: bid: %d mid: "
            "%d d: %d dp: %d\n",
            gfs_box_domain(bb->box)->pid, mpi->process, MATCHING_TAG(bb),
            bb->box->id, GFS_BOUNDARY_MPI(bb)->id, bb->d, boundary->d);
    fflush(DEBUG);
#endif
    MPI_Recv(&boundary->rcvcount, 1, MPI_UNSIGNED, mpi->process,
             MATCHING_TAG(bb), mpi->comm, &status);
#ifdef PROFILE_MPI
    end = MPI_Wtime();
  ...
  MPI_Recv(boundary->rcvbuf->data, boundary->rcvcount, MPI_DOUBLE, mpi->process,
           MATCHING_TAG(bb), mpi->comm, &status);
  ...
  if (count == MPI_UNDEFINED) {
    fprintf(DEBUG, "    %d on tag %d: count is undefined!\n",
            gfs_box_domain(bb->box)->pid, MATCHING_TAG(bb));
    g_assert_not_reached();
  } else if (count != boundary->rcvcount) {
    fprintf(DEBUG, "    %d on tag %d: count = %d boundary->rcvcount = %d\n",
            gfs_box_domain(bb->box)->pid, MATCHING_TAG(bb), count,
            boundary->rcvcount);
    g_assert_not_reached();
  }
#else
...
GfsBoundaryClass *gfs_boundary_mpi_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_boundary_mpi_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass =
        gts_object_class_new(GTS_OBJECT_CLASS(gfs_boundary_periodic_class()),
...
GfsBoundaryMpi *gfs_boundary_mpi_new(GfsBoundaryClass *klass, GfsBox *box,
  ...
  if (id >= tag_shift)
    g_warning(
        "GfsBoundaryMpi id (%d) is larger than the maximum MPI tag value\n"
        "allowed on this system (%d)",
        id, tag_shift);
#endif /* HAVE_MPI */
  boundary->id = id;
