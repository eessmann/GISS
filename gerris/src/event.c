...
gdouble gfs_event_next(GfsEvent *event, GfsSimulation *sim) {
  ...
  if (event->t >= event->end || event->i >= event->iend ||
      sim->time.t > event->end || sim->time.i > event->iend)
    return G_MAXDOUBLE;
  if (event->end_event)
    return G_MAXDOUBLE;
  if (sim->time.t >= event->t) {
...
static gboolean gfs_event_event(GfsEvent *event, GfsSimulation *sim) {
  ...
  if (event->end_event) {
    if (event->n == 0 &&
        (sim->time.t >= sim->time.end || sim->time.i >= sim->time.iend)) {
      event->n = 1;
...
static void gfs_event_read(GtsObject **o, GtsFile *fp) {
  ...
  if (fp->type == GTS_STRING && fp->token->str[0] == '*') {
    ...
    g_hash_table_insert(GFS_DOMAIN(gfs_object_simulation(*o))->objects,
                        event->name, *o);
    gts_file_next_token(fp);
  }

  if (fp->type == '{') {
    GtsFileVariable var[] = {{GTS_STRING, "start", TRUE},
                             {GTS_DOUBLE, "end", TRUE},
    ...
    if (var[2].set && event->step <= 0.) {
      gts_file_variable_error(
          fp, var, "step", "step `%g' must be strictly positive", event->step);
      return;
    }
...
static void gfs_event_destroy(GtsObject *o) {
  ...
  if (event->name) {
    g_hash_table_remove(GFS_DOMAIN(gfs_object_simulation(o))->objects,
                        event->name);
    g_free(event->name);
  }
 ...
 * is unchanged.
 */
void gfs_event_set(GfsEvent *e, gdouble start, gdouble end, gdouble step,
...
static void gfs_generic_init_init(GfsEvent *event) { event->start = -1; }
...
static void gfs_init_read(GtsObject **o, GtsFile *fp) {
  if (GTS_OBJECT_CLASS(gfs_init_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_init_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR)
    return;
  ...
  while (fp->type != GTS_ERROR && fp->type != '}') {
    ...
    if (fp->type == '(') {
      ...
      /* scalar */
      GfsVariable *v =
          gfs_domain_get_or_add_variable(domain, fp->token->str, NULL);
      GfsFunction *f;
  ...
  GfsEvent *event = GFS_EVENT(*o);
  if (event->start < 0. &&
      (event->istep < G_MAXINT || event->step < G_MAXDOUBLE))
    event->start = 0.;
}
...
static void gfs_init_write(GtsObject *o, FILE *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_init_class())->parent_class->write)
    (*GTS_OBJECT_CLASS(gfs_init_class())->parent_class->write)(o, fp);
...
static void gfs_init_destroy(GtsObject *object) {
  ...
  (*GTS_OBJECT_CLASS(gfs_init_class())->parent_class->destroy)(object);
}
...
static gboolean gfs_init_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_init_class())->parent_class)
            ->event)(event, sim)) {
    ...
    while (i) {
      ...
      if (!gfs_char_in_string(vf->v[0]->name[strlen(vf->v[0]->name) - 1],
                              "0123456789"))
...
static void gfs_init_mask_read(GtsObject **o, GtsFile *fp) {
  ...
  gfs_function_read(GFS_INIT_MASK(*o)->mask,
                    GFS_DOMAIN(gfs_object_simulation(*o)), fp);
}
...
static gboolean gfs_init_mask_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_init_mask_class())->parent_class)
            ->event)(event, sim)) {
...
static void gfs_init_flow_constant_read(GtsObject **o, GtsFile *fp) {
  if (GTS_OBJECT_CLASS(gfs_init_flow_constant_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_init_flow_constant_class())->parent_class->read)(o,
                                                                            fp);
  if (fp->type == GTS_ERROR)
    return;
...
static void gfs_init_vorticity_read(GtsObject **o, GtsFile *fp) {
  if (GTS_OBJECT_CLASS(gfs_init_vorticity_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_init_vorticity_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR)
    return;
#if FTT_2D
  ...
  for (c = 0; c < FTT_DIMENSION; c++)
    gfs_function_read(GFS_INIT_VORTICITY(*o)->fv[c], gfs_object_simulation(*o),
                      fp);
#endif /* 3D */
}
...
static void gfs_init_vorticity_destroy(GtsObject *object) {
  ...
  (*GTS_OBJECT_CLASS(gfs_init_vorticity_class())->parent_class->destroy)(
      object);
}
...
static void correct_div(GfsDomain *domain, GfsVariable *v) {
  ...
  div =
      gfs_domain_stats_variable(domain, v, FTT_TRAVERSE_LEAFS, -1, NULL, NULL);
  gts_range_init(&vol);
...
static void stream_from_vorticity(GfsDomain *domain, GfsVariable *stream,
                                  GfsVariable *vorticity, gdouble tolerance) {
  GfsNorm             norm;
  guint               maxit = 100;
  GfsVariable *       res, *dia;
  ...
  gfs_residual(domain, FTT_DIMENSION, FTT_TRAVERSE_LEAFS, -1, stream, vorticity,
               dia, res);
...
static void init_from_streamfunction(FttCell *cell, GfsInitVorticity *init) {
  gdouble size = ftt_cell_size(cell);
#if FTT_2D
  GFS_VALUE(cell, init->u[0]) =
      -gfs_center_gradient(cell, FTT_Y, init->stream->i) / size;
  GFS_VALUE(cell, init->u[1]) =
      gfs_center_gradient(cell, FTT_X, init->stream->i) / size;
#else  /* 3D */
  GFS_VALUE(cell, init->u[0]) =
      (gfs_center_gradient(cell, FTT_Z, init->stream[1]->i) -
       gfs_center_gradient(cell, FTT_Y, init->stream[2]->i)) /
      size;
  GFS_VALUE(cell, init->u[1]) =
      (gfs_center_gradient(cell, FTT_X, init->stream[2]->i) -
       gfs_center_gradient(cell, FTT_Z, init->stream[0]->i)) /
      size;
  GFS_VALUE(cell, init->u[2]) =
      (gfs_center_gradient(cell, FTT_Y, init->stream[0]->i) -
       gfs_center_gradient(cell, FTT_X, init->stream[1]->i)) /
      size;
#endif /* 3D */
}
...
static gboolean gfs_init_vorticity_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_init_vorticity_class())->parent_class)
            ->event)(event, sim)) {
...
static void gfs_event_sum_read(GtsObject **o, GtsFile *fp) {
  ...
  if (!(s->sv =
            gfs_domain_get_or_add_variable(domain, fp->token->str, "Sum"))) {
    gts_file_error(fp, "`%s' is a reserved keyword", fp->token->str);
    return;
  }
  gts_file_next_token(fp);
...
static gboolean gfs_event_sum_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_event_sum_class())->parent_class)
            ->event)(event, sim)) {
...
static void gfs_event_sum_direction_write(GtsObject *o, FILE *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_event_sum_direction_class())->parent_class->write)(o,
                                                                            fp);
...
static void gfs_event_sum_direction_read(GtsObject **o, GtsFile *fp) {
  ...
  (*GTS_OBJECT_CLASS(gfs_event_sum_direction_class())->parent_class->read)(o,
                                                                           fp);
  if (fp->type == GTS_ERROR)
    return;
...
static gboolean gfs_event_sum_direction_event(GfsEvent *     event,
                                              GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_event_sum_class())->parent_class)
            ->event)(event, sim)) {
    GfsEventSumDirection *s = GFS_EVENT_SUM_DIRECTION(event);

    gfs_domain_sum(GFS_DOMAIN(sim), s->d, GFS_EVENT_SUM(event)->v,
                   GFS_EVENT_SUM(event)->sv);
    return TRUE;
  }
  return FALSE;
}
...
static void gfs_event_harmonic_read(GtsObject **o, GtsFile *fp) {
  ...
  if (!(s->z = gfs_domain_get_or_add_variable(
            domain, fp->token->str, "Offset of harmonic decomposition"))) {
    gts_file_error(fp, "`%s' is a reserved keyword", fp->token->str);
    return;
  }
  s->z->units = s->v->units;
  ...
  if (fp->type != GTS_INT && fp->type != GTS_FLOAT) {
    ...
    if (!(s->e = gfs_domain_get_or_add_variable(
              domain, fp->token->str, "Remainder of harmonic decomposition"))) {
      gts_file_error(fp, "`%s' is a reserved keyword", fp->token->str);
      return;
    }
  ...
  s->Mn = gfs_matrix_new(2 * s->omega->len + 1, 2 * s->omega->len + 1,
                         sizeof(gdouble));
  for (i = 0; i < 2 * s->omega->len + 1; i++)
    s->Mn[i][i] = 1.;

  s->M  = gfs_matrix_new(2 * s->omega->len + 1, 2 * s->omega->len + 1,
                        sizeof(gdouble));
  s->iM = gfs_matrix_new(2 * s->omega->len + 1, 2 * s->omega->len + 1,
                         sizeof(gdouble));
  ...
  for (i = 0; i < s->omega->len; i++) {
    ...
    if (!(s->A[i] = gfs_domain_get_or_add_variable(
              domain, u, "In-phase component of the harmonic decomposition"))) {
    ...
    if (!(s->B[i] = gfs_domain_get_or_add_variable(
              domain, u,
...
static gboolean gfs_event_harmonic_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_event_harmonic_class())->parent_class)
            ->event)(event, sim)) {
...
static void gfs_event_stop_write(GtsObject *o, FILE *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_event_stop_class())->parent_class->write)
    (*GTS_OBJECT_CLASS(gfs_event_stop_class())->parent_class->write)(o, fp);
...
static void gfs_event_stop_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_event_stop_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_event_stop_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR)
    return;
  ...
  /* fixme: the lines below are necessary in the general case (e.g. when dealing
   * with a VOF tracer) but will crash if s->oldv is not of the same class as
   * s->v. s->oldv->fine_coarse = s->v->fine_coarse; s->oldv->coarse_fine =
   * s->v->coarse_fine;
   */

  if (fp->type == GTS_STRING) {
    if (!(s->diff = gfs_domain_get_or_add_variable(
              domain, fp->token->str, "Stopping field difference"))) {
      gts_file_error(fp, "`%s' is a reserved keyword", fp->token->str);
      return;
    }
  ...
  if (fp->type == '{') {
    GtsFileVariable var[] = {{GTS_INT, "relative", TRUE, &s->relative},
                             {GTS_NONE}};
    gts_file_assign_variables(fp, var);
  }
}
...
static gboolean gfs_event_stop_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_event_stop_class())->parent_class)
            ->event)(event, sim)) {
    GfsDomain *   domain = GFS_DOMAIN(sim);
    ...
    if (s->last >= 0.) {
      ...
      if (s->relative) {
        n = gfs_domain_norm_variable(domain, s->oldv, NULL, FTT_TRAVERSE_LEAFS,
                                     -1, NULL, NULL);
        max = n.infty;
      }
      gfs_domain_cell_traverse(domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
...
static void gfs_event_stop_init(GfsEventStop *object) { object->last = -1.; }
...
static void gfs_event_script_write(GtsObject *o, FILE *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_event_script_class())->parent_class->write)
    (*GTS_OBJECT_CLASS(gfs_event_script_class())->parent_class->write)(o, fp);
...
static void gfs_event_script_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_event_script_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_event_script_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR)
    return;
...
FILE *gfs_popen(GfsSimulation *sim, const char *command, const char *type) {
  ...
  gchar *scommand =
      g_strdup_printf("GfsTime=%g GfsIter=%d GfsPid=%d "
                      "GFS_STOP=%d sh %s",
                      sim->time.t, sim->time.i, GFS_DOMAIN(sim)->pid,
                      GFS_EVENT_SCRIPT_STOP, sname);
  fflush(stdout);
  fflush(stderr);
...
static gboolean gfs_event_script_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_event_script_class())->parent_class)
            ->event)(event, sim)) {
...
static void gfs_init_fraction_destroy(GtsObject *object) {
  ...
  (*GTS_OBJECT_CLASS(gfs_init_fraction_class())->parent_class->destroy)(object);
}
...
static void gfs_init_fraction_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_init_fraction_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_init_fraction_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR)
    return;
...
static gboolean gfs_init_fraction_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_init_fraction_class())->parent_class)
            ->event)(event, sim)) {
    gfs_domain_init_fraction(GFS_DOMAIN(sim), GFS_INIT_FRACTION(event)->surface,
...
static void gfs_init_fraction_init(GfsInitFraction *object) {
  object->surface = GFS_GENERIC_SURFACE(
      gts_object_new(GTS_OBJECT_CLASS(gfs_surface_class())));
}
...
static void gfs_remove_droplets_destroy(GtsObject *object) {
  ...
  (*GTS_OBJECT_CLASS(gfs_remove_droplets_class())->parent_class->destroy)(
      object);
}
...
static gboolean gfs_remove_droplets_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_remove_droplets_class())->parent_class)
            ->event)(event, sim)) {
...
static void gfs_remove_droplets_read(GtsObject **o, GtsFile *fp) {
  if (GTS_OBJECT_CLASS(gfs_remove_droplets_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_remove_droplets_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR)
    return;
  ...
  if ((r->c = gfs_variable_from_name(domain->variables, fp->token->str)) ==
      NULL) {
    gts_file_error(fp, "unknown variable `%s'", fp->token->str);
    return;
  }
...
static void gfs_remove_droplets_write(GtsObject *o, FILE *fp) {
  if (GTS_OBJECT_CLASS(gfs_remove_droplets_class())->parent_class->write)
    (*GTS_OBJECT_CLASS(gfs_remove_droplets_class())->parent_class->write)(o,
                                                                          fp);
  GfsRemoveDroplets *r = GFS_REMOVE_DROPLETS(o);
...
static gboolean gfs_remove_ponds_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_remove_ponds_class())->parent_class)
            ->event)(event, sim)) {
...
static void gfs_remove_ponds_read(GtsObject **o, GtsFile *fp) {
  if (GTS_OBJECT_CLASS(gfs_remove_ponds_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_remove_ponds_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR)
    return;
...
static void filtered(FttCell *cell, GfsEventFilter *f) {
  ...
  GFS_VALUE(cell, f->v) =
      ((1. - dt) * GFS_VALUE(cell, f->v) + dt * GFS_VALUE(cell, f->tmp));
}

static gboolean gfs_event_filter_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_event_filter_class())->parent_class)
            ->event)(event, sim)) {
    GfsEventFilter *f = GFS_EVENT_FILTER(event);

    f->tmp = gfs_temporary_variable(GFS_DOMAIN(sim));
    gfs_domain_cell_traverse(GFS_DOMAIN(sim), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             -1, (FttCellTraverseFunc)filter, f);
    gfs_traverse_and_bc(GFS_DOMAIN(sim), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
                        (FttCellTraverseFunc)filtered, f, f->v, f->v);
...
static void gfs_event_filter_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_event_filter_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_event_filter_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR)
    return;
  ...
  if ((GFS_EVENT_FILTER(*o)->v =
           gfs_variable_from_name(domain->variables, fp->token->str)) == NULL) {
    gts_file_error(fp, "unknown variable `%s'", fp->token->str);
    return;
  }
  gts_file_next_token(fp);
...
static void gfs_event_filter_write(GtsObject *o, FILE *fp) {
  if (GTS_OBJECT_CLASS(gfs_event_filter_class())->parent_class->write)
    (*GTS_OBJECT_CLASS(gfs_event_filter_class())->parent_class->write)(o, fp);
  fprintf(fp, " %s %g", GFS_EVENT_FILTER(o)->v->name,
          GFS_EVENT_FILTER(o)->scale);
}
...
static gboolean gfs_event_list_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_event_list_class())->parent_class)
            ->event)(event, sim)) {
...
static void copy_event(GfsEvent *event, GfsEvent *list) {
  gfs_event_set(event, list->start, list->end, list->step, list->istart,
                list->iend, list->istep);
}
...
static void gfs_event_list_destroy(GtsObject *o) {
  GfsEventList *l = GFS_EVENT_LIST(o);
  gts_container_foreach(GTS_CONTAINER(l->list), (GtsFunc)gts_object_destroy,
                        NULL);
