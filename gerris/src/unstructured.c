...
static float vertex_value(Vertex *vertex, GfsVariable *v, gint max_depth) {
  return gfs_dimensional_value(
      v, gfs_cell_corner_value(vertex->cell, d[vertex->i], v, max_depth));
}
...
static void allocate_vertices(FttCell *cell, AllocParams *par) {
  ...
  for (i = 0; i < NV; i++)
    if (GFS_DOUBLE_TO_POINTER(GFS_VALUE(cell, par->v[i])) == NULL) {
      ...
      for (j = 0; j < NV; j++)
        if (neighbor[j]) {
          g_assert(GFS_DOUBLE_TO_POINTER(GFS_VALUE(neighbor[j], par->v[j])) ==
                   NULL);
...
static GSList *allocate_domain_vertices(GfsDomain *domain, gint max_depth,
                                        GfsVariable *v[NV], guint size) {
...
void gfs_domain_write_vtk(GfsDomain *domain, gint max_depth, GSList *variables,
  ...
  GSList *vertices =
      allocate_domain_vertices(domain, max_depth, v, sizeof(Vertex));
...
static void write_tecplot_element(FttCell *cell, WriteParams *par) {
  ...
  for (i = 0; i < NV; i++) {
    Vertex *v =
        GFS_DOUBLE_TO_POINTER(GFS_VALUE(cell, par->v[tecplot_index[i]]));
...
void gfs_domain_write_tecplot(GfsDomain *domain, gint max_depth,
  ...
  /* header */
  fprintf(fp, " TITLE = \"Gerris simulation version %s (%s)\"\n", GFS_VERSION,
          GFS_BUILD_VERSION);

  fputs(FTT_DIMENSION == 2 ? " VARIABLES = \"X\", \"Y\""
                           : " VARIABLES = \"X\", \"Y\", \"Z\"",
        fp);
  GSList *j = variables;
  while (j) {
...
static void allocate_wallfaces(FttCell *cell, AllocateFacesParams *par) {
  ...
  guint     inode, inode2,
      jnode_max_sintheta = 0,
      nnodes = gfs_cut_cube_vertices(cell, -1, &s->ca, &normal, nodecutface, d,
                                     NULL, NULL);
  g_assert(nnodes <= 6);
  ...
  if (nnodecutface > 3) { /* reorder faces if necessary */
    ...
    gboolean switchnodes =
        FALSE; /* logical variable used to reorder cut face nodes */
    ...
    do {
      ...
      for (inode = 0; inode < nnodecutface; inode++) {
        ...
        FttVector diff1        = {facecenter.x - node.x, facecenter.y - node.y,
                           facecenter.z - node.z};
        gdouble   length_diff1 = ftt_vector_norm(&diff1);

        gdouble max_sintheta = 0.;
        /* cycle through all other nodes (jnode) where cut face intersects cell
         * edges */
        for (inode2 = 1; inode2 < nnodecutface; inode2++) {
          ...
          gdouble sintheta =
              ((diff1.y * diff2.z - diff1.z * diff2.y) * normal.x +
  ...
  /* assign data to nodeinfo array, increment number of wall faces and number of
   * nodes */
  if (nnodecutface <= 4) {
    ...
    nnodecutface -= 2;
    for (inode = 0; inode < nnodecutface; inode++)
      face2->node[inode] = nodecutface[(inode + 3) % (nnodecutface + 2)];
    face2->cell   = cell;
...
void gfs_domain_write_tecplot_surface(GfsDomain *domain, gint max_depth,
  ...
  fprintf(fp, " ZONE T=\"WALL DATA\", N=%i, E=%i, F=FEPOINT, ",
          par.nnodes_total, nwallfaces);
  ...
  while (j) {
    ...
    for (inode = 0; inode < face->nnodes; inode++) {
      ...
      while (k) {
        ...
        /* write out cell values; can use interpolate for more precise data
         * representation */
