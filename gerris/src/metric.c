...
static gdouble face_metric_direction(const GfsDomain *domain, FttCell *cell,
                                     FttDirection d) {
  FttCellFace f;
  f.cell = cell;
  f.d    = d;
...
static gdouble viscous_metric_implicit(const GfsDomain *domain, FttCell *cell,
...
/* see: doc/figures/viscous-metric.tm equation (4) */
static gdouble viscous_metric_explicit(const GfsDomain *domain, FttCell *cell,
                                       GfsVariable *v, GfsDiffusion *d) {
  ...
  /* fixme: this does not include the terms with derivatives of the viscosity
   * yet */
  /* fixme: this does not include the "curvature" of the metric yet */
  /* fixme: this does not take into account density */
  return eta * (+2. * (u2_1 * h1_2 / h1 - u2_2 * h2_1 / h2) / (size * size)) /
         h1h2;
}

static void advection_metric(const GfsDomain *domain, FttCell *cell,
                             FttComponent c1, gdouble m[2]) {
...
static double kpu_1d_l3[3][2] = {
    {.1127017, .2777778}, {.5, .4444444}, {.8872983, .2777778}};

static double kpu_2d_l2[5][3] = {{.1127017, .5, .2777778},
                                 {.5, .1127017, .2777778},
                                 {.5, .5, -.1111112},
                                 {.5, .8872983, .2777778},
                                 {.8872983, .5, .2777778}};

static double kpu_2d_l3[9][3] = {{.1127017, .1127017, .07716050617284001},
...
static double ru_rv(FttVector r, GfsMap *map) {
  ...
  rv.y -= r.y;
  rv.z -= r.z;
  return sqrt((ru.x * ru.x + ru.y * ru.y + ru.z * ru.z) *
                  (rv.x * rv.x + rv.y * rv.y + rv.z * rv.z)
              /* the cross-term should be zero for an orthogonal
        metric but we keep them for clarity*/
              - (ru.x * rv.x + ru.y * rv.y + ru.z * rv.z) *
                    (ru.x * rv.x + ru.y * rv.y + ru.z * rv.z));
}

static double integration2d(GfsMap *map, double u1, double v1, double u2,
                            double v2) {
  int       i;
  FttVector r;
  double    du = u2 - u1;
...
static double area(GfsMap *map, double u1, double v1, double u2, double v2) {
#if USE_GSL
  FttVector    r;
  gsl_function f;
...
static void metric_coarse_fine(FttCell *parent, GfsVariable *a) {
  ...
  for (i = 0; i < FTT_CELLS; i++) {
    ...
    GFS_VALUE(child.c[i], a) =
        area(map, p.x - h / 2., p.y - h / 2., p.x + h / 2., p.y + h / 2.) /
        (h * h);
...
static void metric_fine_coarse(FttCell *parent, GfsVariable *a) {
  ...
  GfsStoredMetric *m = GFS_STORED_METRIC(a);
  GFS_VALUE(parent, m->h[0]) =
      (GFS_VALUE(child.c[1], m->h[0]) + GFS_VALUE(child.c[3], m->h[0])) / 2.;
  GFS_VALUE(parent, m->h[1]) =
      (GFS_VALUE(child.c[0], m->h[1]) + GFS_VALUE(child.c[2], m->h[1])) / 2.;
  GFS_VALUE(parent, m->h[2]) =
      (GFS_VALUE(child.c[0], m->h[2]) + GFS_VALUE(child.c[1], m->h[2])) / 2.;
  GFS_VALUE(parent, m->h[3]) =
      (GFS_VALUE(child.c[2], m->h[3]) + GFS_VALUE(child.c[3], m->h[3])) / 2.;
}

static gdouble face_metric(const GfsDomain *domain, const FttCellFace *face) {
  if (face->d / 2 > FTT_Y)
    return 1.;
  return GFS_VALUE(face->cell,
                   GFS_STORED_METRIC(domain->metric_data)->h[face->d]);
}
...
static void solid_metric(const GfsDomain *domain, const FttCell *cell,
                         FttVector *m) {
  g_assert(GFS_IS_MIXED(cell));
  g_assert_not_implemented();
}

static gdouble scale_metric(const GfsDomain *domain, const FttCell *cell,
                            FttComponent c) {
  ...
  return (GFS_VALUE(cell, GFS_STORED_METRIC(domain->metric_data)->h[2 * d]) +
          GFS_VALUE(cell,
                    GFS_STORED_METRIC(domain->metric_data)->h[2 * d + 1])) /
         2.;
}

static gdouble face_scale_metric(const GfsDomain *  domain,
                                 const FttCellFace *face, FttComponent c) {
  /* fixme: this does not allow for Z-metric */
  if (c > FTT_Y)
    return 1.;
  /* fixme: this is not second-order for fine/coarse faces */
  return (scale_metric(domain, face->cell, c) +
          scale_metric(domain, face->neighbor, c)) /
         2.;
}
...
static void stored_metric_read(GtsObject **o, GtsFile *fp) {
  ...
  if (fp->type == GTS_STRING) {
    if (!(m->e = gfs_domain_get_or_add_variable(domain, fp->token->str,
                                                "Metric error"))) {
  ...
  gts_container_add(GTS_CONTAINER(GFS_SIMULATION(domain)->maps),
                    GTS_CONTAINEE(m->map));
...
GfsVariableClass *gfs_stored_metric_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo info = {"GfsStoredMetric",
...
#define GFS_IS_MAP_METRIC(obj)                                                 \
  (gts_object_is_from_class(obj, gfs_map_metric_class()))
...
static void map_metric_inverse(GfsMap *map, const FttVector *src,
                               FttVector *dest) {
...
static void metric_read(GtsObject **o, GtsFile *fp) {
  ...
  GtsFileVariable var[]  = {{GTS_OBJ, "x", TRUE, &m->x},
                           {GTS_OBJ, "y", TRUE, &m->y},
                           {GTS_OBJ, "z", TRUE, &m->z},
                           {GTS_NONE}};
  FttComponent    c;
...
static double A[N] = {
    1.47713062600964,  -0.38183510510174, -0.05573058001191, -0.00895883606818,
    -0.00791315785221, -0.00486625437708, -0.00329251751279, -0.00235481488325,
    -0.00175870527475, -0.00135681133278, -0.00107459847699, -0.00086944475948,
    -0.00071607115121, -0.00059867100093, -0.00050699063239, -0.00043415191279,
    -0.00037541003286, -0.00032741060100, -0.00028773091482, -0.00025458777519,
    -0.00022664642371, -0.00020289261022, -0.00018254510830, -0.00016499474461,
    -0.00014976117168, -0.00013646173946, -0.00012478875823, -0.00011449267279,
    -0.00010536946150, -0.00009725109376};

static double B[N] = {
    0.67698819751739, 0.11847293456554, 0.05317178134668, 0.02965810434052,
    0.01912447304028, 0.01342565621117, 0.00998873323180, 0.00774868996406,
    0.00620346979888, 0.00509010874883, 0.00425981184328, 0.00362308956077,
    0.00312341468940, 0.00272360948942, 0.00239838086555, 0.00213001905118,
    0.00190581316131, 0.00171644156404, 0.00155493768255, 0.00141600715207,
    0.00129556597754, 0.00119042140226, 0.00109804711790, 0.00101642216628,
    0.00094391366522, 0.00087919021224, 0.00082115710311, 0.00076890728775,
    0.00072168382969, 0.00067885087750};

#else
/* Conformal mapping Taylor coefficients: from map_xy2xyz.m from mitgcm */

static double A[N] = {
    1.47713057321600,  -0.38183513110512, -0.05573055466344, -0.00895884801823,
    -0.00791314396396, -0.00486626515498, -0.00329250387158, -0.00235482619663,
    -0.00175869000970, -0.00135682443774, -0.00107458043205, -0.00086946107050,
    -0.00071604933286, -0.00059869243613, -0.00050696402446, -0.00043418115349,
    -0.00037537743098, -0.00032745130951, -0.00028769063795, -0.00025464473946,
    -0.00022659577923, -0.00020297175587, -0.00018247947703, -0.00016510295548,
    -0.00014967258633, -0.00013660647356, -0.00012466390509, -0.00011468147908,
    -0.00010518717478, -0.00009749136078};

static double B[N] = {
    0.67698822171341, 0.11847295533659, 0.05317179075349, 0.02965811274764,
    0.01912447871071, 0.01342566129383, 0.00998873721022, 0.00774869352561,
    0.00620347278164, 0.00509011141874, 0.00425981415542, 0.00362309163280,
    0.00312341651697, 0.00272361113245, 0.00239838233411, 0.00213002038153,
    0.00190581436893, 0.00171644267546, 0.00155493871562, 0.00141600812949,
    0.00129556691848, 0.00119042232809, 0.00109804804853, 0.00101642312253,
    0.00094391466713, 0.00087919127990, 0.00082115825576, 0.00076890854394,
    0.00072168520663, 0.00067885239089};
#endif
...
static void plane_normal(const double p1[3], const double p2[3],
                         double plane[3]) {
...
static double excess_of_quad(const double v1[3], const double v2[3],
  ...
  return 2. * M_PI - angle_between_vectors(plane2, plane1) -
...
static void map_cubed_transform(GfsMap *map, const FttVector *src,
                                FttVector *dest) {
...
static void map_cubed_inverse(GfsMap *map, const FttVector *src,
                              FttVector *dest) {
  GfsSimulation *sim = gfs_object_simulation(map);
  double         X, Y, Z;
...
static void map_cubed_inverse_cell(GfsMap *map, const FttVector *src,
                                   FttVector *dest) {
  gint      i;
  FttVector o = {0., 0., 0.};
  for (i = 0; i < 4; i++) {
...
static gdouble cubed_face_scale_metric(const GfsDomain *  domain,
...
static Point **matrix_refine(Point **m, int n) {
  ...
  for (i = 0; i < n - 1; i++)
    for (j = 0; j < n - 1; j++) {
      point_new(&r[2 * i + 1][2 * j], (m[i][j].x + m[i + 1][j].x) / 2.,
                m[i][j].y);
      point_new(&r[2 * i][2 * j + 1], m[i][j].x,
                (m[i][j].y + m[i][j + 1].y) / 2.);
      point_new(&r[2 * i + 1][2 * j + 1], (m[i][j].x + m[i + 1][j].x) / 2.,
                (m[i][j].y + m[i][j + 1].y) / 2.);
    }
  i = n - 1;
  for (j = 0; j < n - 1; j++)
...
GfsVariableClass *gfs_metric_cubed_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo info = {"GfsMetricCubed",
...
#define GFS_MAP_LONLAT(obj)                                                    \
  GTS_OBJECT_CAST(obj, GfsMapLonLat, gfs_map_lonlat_class())
#define GFS_IS_MAP_LONLAT(obj)                                                 \
  (gts_object_is_from_class(obj, gfs_map_lonlat_class()))
...
static void map_lonlat_transform(GfsMap *map, const FttVector *src,
                                 FttVector *dest) {
...
static void map_lonlat_inverse(GfsMap *map, const FttVector *src,
                               FttVector *dest) {
...
static void map_lonlat_inverse_cell(GfsMap *map, const FttVector *src,
                                    FttVector *dest) {
  gint      i;
  FttVector q;
...
static gdouble lon_lat_face_metric(const GfsDomain *  domain,
                                   const FttCellFace *face) {
  if (face->d / 2 != FTT_Y)
    return 1.;
  return face->d == 2 ? GFS_VALUE(face->cell,
                                  GFS_METRIC_LON_LAT(domain->metric_data)->h2)
                      : GFS_VALUE(face->cell,
                                  GFS_METRIC_LON_LAT(domain->metric_data)->h3);
}

static gdouble lon_lat_cell_metric(const GfsDomain *domain,
                                   const FttCell *  cell) {
  return GFS_VALUE(cell, GFS_VARIABLE(domain->metric_data));
}

static gdouble lon_lat_scale_metric(const GfsDomain *domain,
                                    const FttCell *cell, FttComponent c) {
  if (c != FTT_X)
    return 1.;
  return GFS_VALUE(cell, GFS_VARIABLE(domain->metric_data));
}

static gdouble lon_lat_face_scale_metric(const GfsDomain *  domain,
                                         const FttCellFace *face,
                                         FttComponent       c) {
  if (c != FTT_X)
    return 1.;
  return gfs_face_interpolated_value(face,
                                     GFS_VARIABLE(domain->metric_data)->i);
}
...
static void lonlat_coarse_fine(FttCell *parent, GfsVariable *a) {
  ...
  GFS_VALUE(child.c[0], a) = GFS_VALUE(child.c[1], a) =
      (sin(theta1) - sintheta) / dtheta;
  GFS_VALUE(child.c[2], a) = GFS_VALUE(child.c[3], a) =
      (sintheta - sin(theta2)) / dtheta;

  GFS_VALUE(child.c[0], lonlat->h2) = GFS_VALUE(child.c[1], lonlat->h2) =
      cos(theta1);
  GFS_VALUE(child.c[0], lonlat->h3)     = GFS_VALUE(child.c[1], lonlat->h3) =
      GFS_VALUE(child.c[2], lonlat->h2) = GFS_VALUE(child.c[3], lonlat->h2) =
          cos(theta);
  GFS_VALUE(child.c[2], lonlat->h3) = GFS_VALUE(child.c[3], lonlat->h3) =
      cos(theta2);
}
...
static void lonlat_fine_coarse(FttCell *parent, GfsVariable *a) {
  ...
  GFS_VALUE(parent, lonlat->h2) =
      (GFS_VALUE(child.c[0], lonlat->h2) + GFS_VALUE(child.c[1], lonlat->h2)) /
      2.;
  GFS_VALUE(parent, lonlat->h3) =
      (GFS_VALUE(child.c[3], lonlat->h3) + GFS_VALUE(child.c[2], lonlat->h3)) /
      2.;
}
...
static void metric_lon_lat_read(GtsObject **o, GtsFile *fp) {
  ...
  lonlat->h2 =
      gfs_domain_get_or_add_variable(domain, name, "LonLat face metric");
...
static void metric_lon_lat_init(GfsMetricLonLat *m) { m->r = 1.; }
...
#define GFS_IS_MAP_STRETCH(obj)                                                \
  (gts_object_is_from_class(obj, gfs_map_stretch_class()))
...
static void map_stretch_transform(GfsMap *map, const FttVector *src,
                                  FttVector *dest) {
...
static void map_stretch_inverse(GfsMap *map, const FttVector *src,
                                FttVector *dest) {
  GfsMetricStretch *m = GFS_DOMAIN(gfs_object_simulation(map))->metric_data;
  dest->x             = src->x * m->sx;
  dest->y             = src->y * m->sy;
...
static void metric_stretch_write(GtsObject *o, FILE *fp) {
  ...
  fprintf(fp, " { sx = %g sy = %g sz = %g }", GFS_METRIC_STRETCH(o)->sx,
          GFS_METRIC_STRETCH(o)->sy, GFS_METRIC_STRETCH(o)->sz);
}

static gdouble stretch_face_metric(const GfsDomain *  domain,
                                   const FttCellFace *face) {
...
static gdouble stretch_cell_metric(const GfsDomain *domain,
                                   const FttCell *  cell) {
  GfsMetricStretch *s = GFS_METRIC_STRETCH(domain->metric_data);
#if FTT_2D
  return s->sx * s->sy;
...
static void stretch_solid_metric(const GfsDomain *domain, const FttCell *cell,
                                 FttVector *m) {
  GfsMetricStretch *s = GFS_METRIC_STRETCH(domain->metric_data);
  g_assert(GFS_IS_MIXED(cell));
#if FTT_2D
...
static gdouble stretch_scale_metric(const GfsDomain *domain,
                                    const FttCell *cell, FttComponent c) {
  return (&GFS_METRIC_STRETCH(domain->metric_data)->sx)[c];
}

static gdouble stretch_face_scale_metric(const GfsDomain *  domain,
                                         const FttCellFace *face,
                                         FttComponent       c) {
  return (&GFS_METRIC_STRETCH(domain->metric_data)->sx)[c];
}
...
static void metric_stretch_read(GtsObject **o, GtsFile *fp) {
  ...
  if (fp->type == '{') {
    GtsFileVariable var[] = {{GTS_DOUBLE, "sx", TRUE, &s->sx},
...
#define GFS_IS_MAP_CUBED1(obj)                                                 \
  (gts_object_is_from_class(obj, gfs_map_cubed1_class()))
...
static void map_cubed1_inverse(GfsMap *map, const FttVector *src,
                               FttVector *dest) {
...
GfsVariableClass *gfs_metric_cubed1_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo info = {"GfsMetricCubed1",
                               sizeof(GfsStoredMetric),
                               sizeof(GfsVariableClass),
...
#define GFS_IS_MAP_VARIABLE(obj)                                               \
  (gts_object_is_from_class(obj, gfs_map_variable_class()))
...
static void map_variable_inverse(GfsMap *map, const FttVector *src,
                                 FttVector *dest) {
...
static void metric_variable_read(GtsObject **o, GtsFile *fp) {
  ...
  for (c = 0; c < 3; c++) {
    ...
    m->x[c]            = gfs_domain_get_or_add_variable(domain, name,
                                             "coordinate for variable metric");
    g_free(name);
  }
...
static void update_metric(FttCell *cell, UpdateData *p) {
  ...
  gdouble x[4 * (FTT_DIMENSION - 1) + 1], y[4 * (FTT_DIMENSION - 1) + 1], dx,
      dy, dz;
...
static gdouble conformal_face_scale_metric(const GfsDomain *  domain,
                                           const FttCellFace *face,
                                           FttComponent       c) {
  if (c > FTT_Y)
    return 1.;
...
static void metric_laplace_read(GtsObject **o, GtsFile *fp) {
  ...
  if (fp->type == '{') {
    GtsFileVariable var[] = {{GTS_INT, "conformal", TRUE, &m->conformal},
...
static void spherical_laplacian(FttCell *cell, GfsVariable *dia) {
  gdouble h = ftt_cell_size(cell);
  GFS_VALUE(cell, dia) =
      -2. * h * h * gfs_domain_cell_fraction(dia->domain, cell);
}
...
static gboolean metric_laplace_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(gfs_stored_metric_class())->event)(event, sim)) {
    ...
    /* convergence is dominated by non-linear terms, 1 iteration of the
     * multigrid is enough */
    par.tolerance = 1.;
    par.nitermin  = 1;

    UpdateData p = {mv, 1e6, div};
    gfs_domain_traverse_leaves(domain,
                               (FttCellTraverseFunc)cartesian_coordinates, &p);
    if (!m->spherical)
    ...
    while (niter-- && p.max > 1e-3) {
      ...
      p.max = 0.;
      gfs_domain_traverse_leaves(domain, (FttCellTraverseFunc)update_metric,
                                 &p);
      gfs_domain_cell_traverse(
          domain, FTT_POST_ORDER, FTT_TRAVERSE_NON_LEAFS, -1,
          (FttCellTraverseFunc)GFS_VARIABLE(event)->fine_coarse, event);
...
GfsVariableClass *gfs_metric_laplace_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo info = {
        ...
        (GtsArgSetFunc)NULL,
        (GtsArgGetFunc)NULL};
    klass = gts_object_class_new(GTS_OBJECT_CLASS(gfs_metric_variable_class()),
                                 &info);
  }
