...
static void symmetry_stencil(FttCellFace *f, GfsBc *b) {
  set_stencil_neighbor(f, b,
                       (b->v->component == f->d / 2 && !b->v->even) ? -1. : 1.);
}
...
GfsBcClass *gfs_bc_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_bc_info = {"GfsBc",
                                      sizeof(GfsBc),
                                      sizeof(GfsBcClass),
...
static void bc_value_write(GtsObject *o, FILE *fp) {
  if (GTS_OBJECT_CLASS(gfs_bc_value_class())->parent_class->write)
    (*GTS_OBJECT_CLASS(gfs_bc_value_class())->parent_class->write)(o, fp);
...
static void bc_value_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_bc_value_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_bc_value_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR)
    return;
  gfs_function_read(GFS_BC_VALUE(*o)->val, gfs_box_domain(GFS_BC(bc)->b->box),
                    fp);
}
...
static GfsBc *gfs_bc_value_new(GfsBcClass *k, GfsVariable *v, GfsFunction *val,
                               gboolean extra) {
...
static void gfs_bc_dirichlet_init(GfsBc *object) {
  ...
  object->homogeneous_bc_stencil =
      (FttFaceTraverseFunc)homogeneous_dirichlet_stencil;
...
static void neumann(FttCellFace *f, GfsBc *b) {
  GFS_VALUE(f->cell, b->v) =
      GFS_VALUE(f->neighbor, b->v) +
      gfs_function_face_value(GFS_BC_VALUE(b)->val, f) * ftt_cell_size(f->cell);
}
...
GfsBcClass *gfs_bc_angle_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo info = {"GfsBcAngle",
                               sizeof(GfsBcValue),
                               sizeof(GfsBcClass),
                               ...
                               (GtsArgGetFunc)NULL};
    klass =
        gts_object_class_new(GTS_OBJECT_CLASS(gfs_bc_neumann_class()), &info);
  }
...
static void navier(FttCellFace *f, GfsBc *b) {
  ...
  GFS_VALUE(f->cell, b->v) =
      (2. * gfs_function_face_value(GFS_BC_VALUE(b)->val, f) * h -
       (h - 2. * lambda) * GFS_VALUE(f->neighbor, b->v)) /
      (h + 2. * lambda);
}
  ...
  GFS_STATE(f->cell)->f[f->d].v =
      GFS_STATE(f->neighbor)->f[FTT_OPPOSITE_DIRECTION(f->d)].v =
          (gfs_function_face_value(GFS_BC_VALUE(b)->val, f) * h +
           2. * lambda * GFS_VALUE(f->neighbor, b->v)) /
          (h + 2. * lambda);
}
...
static void gfs_boundary_write(GtsObject *o, FILE *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_boundary_class())->parent_class->write)
    (*GTS_OBJECT_CLASS(gfs_boundary_class())->parent_class->write)(o, fp);
...
static void gfs_boundary_read(GtsObject **o, GtsFile *fp) {
  if (GTS_OBJECT_CLASS(gfs_boundary_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_boundary_class())->parent_class->read)(o, fp);
  if (fp->type == GTS_ERROR)
    return;
 ...
 */
GfsBoundary *gfs_boundary_new(GfsBoundaryClass *klass, GfsBox *box,
                              FttDirection d) {
 ...
 */
void gfs_boundary_receive(GfsBoundary *boundary, FttTraverseFlags flags,
                          gint max_depth) {
  g_return_if_fail(boundary != NULL);

  if (GFS_BOUNDARY_CLASS(GTS_OBJECT(boundary)->klass)->receive)
    (*GFS_BOUNDARY_CLASS(GTS_OBJECT(boundary)->klass)->receive)(boundary, flags,
                                                                max_depth);
}
...
static void inflow_constant_write(GtsObject *o, FILE *fp) {
  if (GTS_OBJECT_CLASS(gfs_boundary_inflow_constant_class())
          ->parent_class->write)
    (*GTS_OBJECT_CLASS(gfs_boundary_inflow_constant_class())
...
static void inflow_constant_read(GtsObject **o, GtsFile *fp) {
  ...
  for (c = 0; c < FTT_DIMENSION; c++)
    if (c == b->d / 2)
      gfs_boundary_add_bc(
          b, gfs_bc_value_new(gfs_bc_dirichlet_class(), v[c], un, FALSE));
    else
      gfs_boundary_add_bc(
          b, gfs_bc_value_new(gfs_bc_dirichlet_class(), v[c], NULL, FALSE));
}
...
static void
gfs_boundary_inflow_constant_init(GfsBoundaryInflowConstant *object) {
...
static void outflow_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_boundary_outflow_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_boundary_outflow_class())->parent_class->read)(o,
                                                                          fp);
  if (fp->type == GTS_ERROR)
    return;
  ...
  gfs_boundary_add_bc(
      b, gfs_bc_value_new(gfs_bc_neumann_class(), v[b->d / 2], NULL, FALSE));
  gfs_boundary_add_bc(
      b, gfs_bc_value_new(gfs_bc_dirichlet_class(),
                          gfs_variable_from_name(domain->variables, "P"), NULL,
                          FALSE));
}
...
static void gradient_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_boundary_gradient_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_boundary_gradient_class())->parent_class->read)(o,
                                                                           fp);
  if (fp->type == GTS_ERROR)
    return;
...
static void center_periodic(FttCellFace *face, GfsBc *b) {
  ...
  g_array_index(boundary_periodic->sndbuf, gdouble,
                boundary_periodic->sndcount++) =
...
static void boundary_size(FttCell *cell, guint *count) { (*count)++; }

static void set_buffers_size(GfsBoundaryPeriodic *boundary) {
  guint count = 0;

  ftt_cell_traverse(GFS_BOUNDARY(boundary)->root, FTT_PRE_ORDER,
                    FTT_TRAVERSE_ALL, -1, (FttCellTraverseFunc)boundary_size,
                    &count);
...
static void boundary_tree(FttCell *cell, GfsBoundaryPeriodic *boundary) {
  ...
  if (!is_leaf) {
    ...
    for (i = 0; i < n; i++) {
      /* fixme: using a gdouble to store (and MPI transfer) a boolean is
       * wasteful... */
      ...
      if (boundary->sndcount == boundary->sndbuf->len)
        ...
        g_array_index(boundary->sndbuf, gdouble, boundary->sndcount) =
            is_destroyed;
      boundary->sndcount++;
    }
    for (i = 0; i < n; i++)
...
static void send(GfsBoundary *bb) {
  ...
  memcpy(matching->rcvbuf->data, boundary->sndbuf->data,
         boundary->sndcount * sizeof(gdouble));
}

static void center_update(FttCell *cell, GfsBoundaryPeriodic *boundary) {
...
static void face_update(FttCellFace *face, GfsBoundaryPeriodic *boundary) {
  g_assert(boundary->rcvcount < boundary->rcvbuf->len);
...
static void match_update(FttCell *cell, GfsBoundaryPeriodic *boundary) {
  gboolean is_leaf;
  ...
  if (!is_leaf) {
    ...
    if (FTT_CELL_IS_LEAF(cell)) {
      ...
      if (FTT_CELL_IS_LEAF(neighbor))
        ftt_cell_refine_single(neighbor, domain->cell_init,
    ...
    for (i = 0; i < n; i++) {
      ...
      if (is_destroyed[i] && child.c[i]) {
        ftt_cell_destroy(child.c[i], (FttCellCleanupFunc)gfs_cell_cleanup,
                         domain);
        child.c[i]                      = NULL;
        GFS_BOUNDARY(boundary)->changed = TRUE;
      }
...
static void receive(GfsBoundary *bb, FttTraverseFlags flags, gint max_depth) {
  GfsBoundaryPeriodic *boundary = GFS_BOUNDARY_PERIODIC(bb);
  ...
  case GFS_BOUNDARY_FACE_VARIABLE:
    ftt_face_traverse_boundary(
        GFS_BOUNDARY(boundary)->root, GFS_BOUNDARY(boundary)->d, FTT_PRE_ORDER,
        flags, max_depth, (FttFaceTraverseFunc)face_update, boundary);
    break;

  case GFS_BOUNDARY_MATCH_VARIABLE:
    match_update(GFS_BOUNDARY(boundary)->root, boundary);
    ftt_cell_flatten(GFS_BOUNDARY(boundary)->root, GFS_BOUNDARY(boundary)->d,
                     (FttCellCleanupFunc)gfs_cell_cleanup,
                     gfs_box_domain(GFS_BOUNDARY(boundary)->box));
    break;

  default:
    ftt_cell_traverse(GFS_BOUNDARY(boundary)->root, FTT_PRE_ORDER, flags,
                      max_depth, (FttCellTraverseFunc)center_update, boundary);
  }
}
 ...
 * @matching: a #GfsBox.
 *
 * Returns: a new #GfsBoundaryPeriodic connecting @box with @matching in
 * direction @d.
 */
GfsBoundaryPeriodic *gfs_boundary_periodic_new(GfsBoundaryClass *klass,
                                               GfsBox *box, FttDirection d,
                                               GfsBox *matching) {
...
static void face_periodic_rotate(FttCellFace *face, GfsBc *b) {
  ...
  if (b->v->component < 2) { /* 2D-vector-rotation only */
    ...
    g_array_index(boundary_periodic->sndbuf, gdouble,
                  boundary_periodic->sndcount++) =
        (2. * c - 1.) * boundary_periodic->rotate *
        GFS_VALUE(face->neighbor, b->v->face[c][d]);
  } else
    g_array_index(boundary_periodic->sndbuf, gdouble,
                  boundary_periodic->sndcount++) =
...
void gfs_boundary_periodic_rotate(GfsBoundaryPeriodic *boundary,
                                  FttDirection rotate, gdouble orientation) {
  g_return_if_fail(boundary != NULL);
 ...
 */
GfsBoundaryPeriodic *
gfs_boundary_periodic_rotate_new(GfsBoundaryClass *klass, GfsBox *box,
                                 FttDirection d, GfsBox *matching,
                                 FttDirection rotate, gdouble orientation) {
  GfsBoundaryPeriodic *boundary;
...
void gfs_gedge_link_boxes(GfsGEdge *edge) {
  ...
  if (edge->rotate < FTT_NEIGHBORS) {
    ...
    gfs_boundary_periodic_rotate_new(gfs_boundary_periodic_class(), b1, edge->d,
                                     b2, edge->rotate, 1.);
    gfs_boundary_periodic_rotate_new(gfs_boundary_periodic_class(), b2,
                                     edge->rotate, b1, edge->d, -1.);
  } else {
    ...
    if (p1[c] != G_MAXDOUBLE && p2[c] != G_MAXDOUBLE &&
        sign * (p2[c] - p1[c]) > 0.)
 ...
 */
GfsGEdge *gfs_gedge_new(GfsGEdgeClass *klass, GfsBox *b1, GfsBox *b2,
                        FttDirection d) {
  GfsGEdge *edge;
  ...
  edge = GFS_GEDGE(
      gts_gedge_new(GTS_GEDGE_CLASS(klass), GTS_GNODE(b1), GTS_GNODE(b2)));
  edge->d = d;
...
static void box_size(FttCell *cell, guint *size) { (*size)++; }
...
static void gfs_box_write(GtsObject *object, FILE *fp) {
  ...
  fprintf(fp, "%s { id = %u pid = %d size = %u x = %g y = %g z = %g",
          object->klass->info.name, box->id, box->pid, size, pos.x, pos.y,
          pos.z);
  for (d = 0; d < FTT_NEIGHBORS; d++)
    if (GFS_IS_BOUNDARY(box->neighbor[d])) {
      fprintf(fp, " %s = %s", ftt_direction_name[d],
  ...
  if (domain != NULL && domain->max_depth_write > -2) {
    ...
    if (domain->binary)
      ftt_cell_write_binary(box->root, domain->max_depth_write, fp,
                            (FttCellWriteFunc)gfs_cell_write_binary,
                            domain->variables_io);
    else
...
static void gfs_box_read(GtsObject **o, GtsFile *fp) {
  ...
  while ((v = gts_file_assign_next(fp, var)))
    if (v->type == GTS_FILE) {
      GtsObjectClass *boundary_class =
          gfs_object_class_from_name(fp->token->str);
      GtsObject *boundary;
      ...
      boundary =
          GTS_OBJECT(gfs_boundary_new(GFS_BOUNDARY_CLASS(boundary_class), b,
                                      ftt_direction_from_name(v->name)));
      gts_file_next_token(fp);
  ...
  if (fp->type == '{') {
    ...
    if (domain->binary) {
      ...
      root = ftt_cell_read_binary(fp, (FttCellReadFunc)gfs_cell_read_binary,
                                  domain);
      if (fp->type == GTS_ERROR)
        return;
      gts_file_next_token(fp);
  ...
  if (ftt_cell_level(b->root) != domain->rootlevel) {
    ...
    for (d = 0; d < FTT_NEIGHBORS; d++)
      if (GFS_IS_BOUNDARY(b->neighbor[d]))
        ftt_cell_set_level(GFS_BOUNDARY(b->neighbor[d])->root,
                           domain->rootlevel);
  }
