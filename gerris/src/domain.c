...
static void locate_index(FttVector *p, GfsLocateArray *a,
                         gint i[FTT_DIMENSION]) {
  gint c;
...
static void root_bounds(FttCell *root, GfsLocateArray *a) {
  ...
  for (i = 0; i < FTT_DIMENSION; i++) {
    if ((&p.x)[i] + a->h / 2. > a->max[i])
      a->max[i] = (&p.x)[i] + a->h / 2.;
    if ((&p.x)[i] - a->h / 2. < a->min[i])
      a->min[i] = (&p.x)[i] - a->h / 2.;
  }
}
...
static void domain_read(GtsObject **o, GtsFile *fp) {
  GfsDomain *     domain = GFS_DOMAIN(*o);
  GtsFileVariable var[]  = {
      {GTS_UINT, "rootlevel", TRUE}, {GTS_DOUBLE, "x", TRUE},
      {GTS_DOUBLE, "y", TRUE},       {GTS_DOUBLE, "z", TRUE},
      {GTS_DOUBLE, "lx", TRUE},      {GTS_DOUBLE, "ly", TRUE},
      {GTS_DOUBLE, "lz", TRUE},      {GTS_STRING, "variables", TRUE},
      {GTS_INT, "binary", TRUE},     {GTS_INT, "version", TRUE},
      {GTS_INT, "overlap", TRUE},    {GTS_NONE}};
  ...
  if (var[4].set || var[5].set || var[6].set)
    g_warning("the (lx,ly,lz) parameters are obsolete, please use "
              "GfsMetricStretch instead");
  ...
  if (variables != NULL) {
    ...
    g_free(variables1);
    domain->variables_io =
        gfs_variables_from_list(domain->variables, variables, &s);
    g_free(variables);
  }
}

static void box_set_pos(GfsBox *box, FttVector *pos, FttDirection dold) {
  FttVector        p;
  ...
  if (box->pid != p->pid)
    ...
    for (d = 0; d < FTT_NEIGHBORS; d++)
      if (GFS_IS_BOUNDARY_PERIODIC(box->neighbor[d]) &&
          ...
          (matching = GFS_BOUNDARY_PERIODIC(box->neighbor[d])->matching)->pid !=
              p->pid) {
        ...
        b = GFS_BOUNDARY_PERIODIC(gfs_boundary_mpi_new(
            gfs_boundary_mpi_class(), box, d, matching->pid, matching->id));
        if (orientation != 0.)
...
static void mpi_links(GfsBox *box, GfsDomain *domain) {
  ...
  for (d = 0; d < FTT_NEIGHBORS; d++)
    if (neighbor[d])
      gfs_boundary_mpi_new(gfs_boundary_mpi_class(), GFS_BOX(neighbor[d]),
                           FTT_OPPOSITE_DIRECTION(d), pid, id);
}
...
static void convert_boundary_mpi_into_edges(GfsBox *box, GPtrArray *ids) {
  ...
  for (d = 0; d < FTT_NEIGHBORS; d++)
    if (GFS_IS_BOUNDARY_MPI(box->neighbor[d])) {
      ...
      GfsBox *        nbox;
      if (b->id >= 0 && b->id <= ids->len &&
          (nbox = g_ptr_array_index(ids, b->id - 1))) {
...
static void domain_post_read(GfsDomain *domain, GtsFile *fp) {
  gts_graph_foreach_edge(GTS_GRAPH(domain), (GtsFunc)gfs_gedge_link_boxes,
                         NULL);
  ...
  if (domain->np > 1 && domain->pid >= 0) { /* Multiple PEs */
    ...
    gts_container_foreach(GTS_CONTAINER(domain), (GtsFunc)set_ref_pos,
                          &domain->refpos);
    ...
    if (domain->np != comm_size) {
      ...
      gts_file_error(fp, "it would be valid if one or %d PE were used",
                     domain->np);
      return;
    }
#endif /* HAVE_MPI */
    ...
    gts_container_foreach(GTS_CONTAINER(domain),
                          (GtsFunc)convert_boundary_mpi_into_edges, ids);
...
static void cleanup_each_box(GfsBox *box, GfsDomain *domain) {
  ...
  if (g_slist_length(GTS_SLIST_CONTAINEE(box)->containers) == 1) {
    ...
    for (d = 0; d < FTT_NEIGHBORS; d++)
      if (GFS_IS_BOUNDARY(box->neighbor[d]))
        ftt_cell_traverse(GFS_BOUNDARY(box->neighbor[d])->root, FTT_PRE_ORDER,
                          FTT_TRAVERSE_ALL, -1,
                          (FttCellTraverseFunc)gfs_cell_cleanup, domain);
  }
}
...
static void domain_destroy(GtsObject *o) {
  ...
  gts_container_foreach(GTS_CONTAINER(domain), (GtsFunc)cleanup_each_box,
                        domain);
...
static void add_item(gpointer item, GPtrArray *a) { g_ptr_array_add(a, item); }
...
static void domain_foreach(GtsContainer *c, GtsFunc func, gpointer data) {
  GPtrArray *a = GFS_DOMAIN(c)->sorted;
  if (a == NULL) /* domain is being destroyed */
    (*GTS_CONTAINER_CLASS(GTS_OBJECT_CLASS(gfs_domain_class())->parent_class)
          ->foreach)(c, func, data);
  else {
    if (GFS_DOMAIN(c)->dirty) {
      g_ptr_array_set_size(a, 0);
      (*GTS_CONTAINER_CLASS(GTS_OBJECT_CLASS(gfs_domain_class())->parent_class)
            ->foreach)(c, (GtsFunc)add_item, a);
  ...
  for (d = 0; d < FTT_NEIGHBORS; d++)
    if (GFS_IS_BOUNDARY(box->neighbor[d])) {
      ...
      if (bc) {
        ...
        bc->v = p->v1;
        ftt_face_traverse_boundary(b->root, b->d, FTT_PRE_ORDER, p->flags,
                                   p->max_depth, bc->bc, bc);
        bc->v = p->v;
        gfs_boundary_send(b);
      }
    }
...
static void direction_face_bc(GtsObject *neighbor, GfsVariable *v) {
  if (GFS_IS_BOUNDARY(neighbor)) {
    ...
    if (bc) {
      ...
      ftt_face_traverse_boundary(b->root, b->d, FTT_PRE_ORDER,
                                 FTT_TRAVERSE_LEAFS, -1, bc->face_bc, bc);
  ...
  if (r->c == FTT_XYZ) {
    ...
    for (d = 0; d < FTT_NEIGHBORS; d++) {
      ...
      if (GFS_IS_BOUNDARY(box->neighbor[od]))
        gfs_boundary_receive(GFS_BOUNDARY(box->neighbor[od]), r->flags,
                             r->max_depth);
    }
  } else {
    if (GFS_IS_BOUNDARY(box->neighbor[2 * r->c + 1]))
      gfs_boundary_receive(GFS_BOUNDARY(box->neighbor[2 * r->c + 1]), r->flags,
                           r->max_depth);
    if (GFS_IS_BOUNDARY(box->neighbor[2 * r->c]))
      gfs_boundary_receive(GFS_BOUNDARY(box->neighbor[2 * r->c]), r->flags,
                           r->max_depth);
  }
}
 ...
 */
void gfs_domain_copy_bc(GfsDomain *domain, FttTraverseFlags flags,
                        gint max_depth, GfsVariable *v, GfsVariable *v1) {
 ...
 */
void gfs_domain_bc(GfsDomain *domain, FttTraverseFlags flags, gint max_depth,
                   GfsVariable *v) {
  g_return_if_fail(domain != NULL);
  g_return_if_fail(v != NULL);
  ...
  for (d = 0; d < FTT_NEIGHBORS; d++)
    if (GFS_IS_BOUNDARY(box->neighbor[d])) {
      ...
      if (bc) {
        ...
        b->type = GFS_BOUNDARY_CENTER_VARIABLE;
        ftt_face_traverse_boundary(b->root, b->d, FTT_PRE_ORDER, p->flags,
                                   p->max_depth, bc->homogeneous_bc, bc);
        bc->v = p->v;
        gfs_boundary_send(b);
      }
    }
...
static void box_homogeneous_bc_stencil(GfsBox *box, BcData *p) {
  ...
  for (d = 0; d < FTT_NEIGHBORS; d++)
    if (GFS_IS_BOUNDARY(box->neighbor[d]) &&
        !GFS_IS_BOUNDARY_PERIODIC(box->neighbor[d])) {
      GfsBoundary *b  = GFS_BOUNDARY(box->neighbor[d]);
      GfsBc *      bc = gfs_boundary_lookup_bc(b, p->v);
      ...
      if (bc) {
        ...
        bc->lp  = p->lp;
        ftt_face_traverse_boundary(b->root, b->d, FTT_PRE_ORDER, p->flags,
                                   p->max_depth, bc->homogeneous_bc_stencil,
                                   bc);
        bc->v = p->v;
        gfs_boundary_send(b);
      }
 ...
 */
void gfs_domain_homogeneous_bc(GfsDomain *domain, FttTraverseFlags flags,
                               gint max_depth, GfsVariable *ov,
                               GfsVariable *v) {
...
void gfs_domain_homogeneous_bc_stencil(GfsDomain *      domain,
                                       FttTraverseFlags flags, gint max_depth,
                                       GfsVariable *ov, GfsVariable *v,
                                       GfsLinearProblem *lp) {
...
static void update_mpi_boundaries(GfsBox *box, TraverseBcData *p) {
  ...
  for (d = 0; d < FTT_NEIGHBORS; d++)
    if (GFS_IS_BOUNDARY_MPI(box->neighbor[d])) {
      ...
      if (bc) {
        ftt_cell_traverse_boundary(box->root, d, p->t.order, p->t.flags,
                                   p->t.max_depth,
                                   (FttCellTraverseFunc)update_mpi_cell, p);
        b->v    = p->b.v1;
        bc->v   = p->b.v1;
        b->type = GFS_BOUNDARY_CENTER_VARIABLE;
        ftt_face_traverse_boundary(b->root, b->d, FTT_PRE_ORDER, p->b.flags,
                                   p->b.max_depth, bc->bc, bc);
        bc->v = p->b.v;
        gfs_boundary_send(b);
      }
    }
 ...
 * the domain.
 */
void gfs_traverse_and_homogeneous_bc(GfsDomain *domain, FttTraverseType order,
                                     FttTraverseFlags flags, gint max_depth,
                                     FttCellTraverseFunc func, gpointer data,
                                     GfsVariable *ov, GfsVariable *v) {
  g_return_if_fail(domain != NULL);
  ...
  if (domain->pid < 0 || !domain->overlap) {
    ...
    TraverseBcData d = {{func, data, order, flags, max_depth},
                        {flags, max_depth, v, ov, FTT_XYZ}};
    /* Update and send MPI boundary values */
    gts_container_foreach(GTS_CONTAINER(domain), (GtsFunc)update_mpi_boundaries,
                          &d);
    ...
    gts_container_foreach(GTS_CONTAINER(domain), (GtsFunc)box_synchronize,
                          &d.b.c);
  }
}
 ...
 * the domain.
 */
void gfs_traverse_and_bc(GfsDomain *domain, FttTraverseType order,
                         FttTraverseFlags flags, gint max_depth,
                         FttCellTraverseFunc func, gpointer data,
                         GfsVariable *v, GfsVariable *v1) {
  g_return_if_fail(domain != NULL);
  ...
  if (domain->pid < 0 || !domain->overlap) {
    ...
    gts_container_foreach(GTS_CONTAINER(domain),
                          (GtsFunc)update_other_boundaries, &d.b);
    /* Receive and synchronize */
    gts_container_foreach(GTS_CONTAINER(domain), (GtsFunc)box_receive_bc, &d.b);
    gts_container_foreach(GTS_CONTAINER(domain), (GtsFunc)box_synchronize,
                          &d.b.c);
...
void gfs_domain_surface_bc(GfsDomain *domain, GfsVariable *v) {
  ...
  if (v->surface_bc)
    gfs_domain_traverse_mixed(domain, FTT_PRE_ORDER, FTT_TRAVERSE_ALL,
                              (FttCellTraverseFunc)GFS_SURFACE_GENERIC_BC_CLASS(
...
static void box_traverse(GfsBox *box, TraverseData *d) {
  ftt_cell_traverse(box->root, d->order, d->flags, d->max_depth, d->func,
                    d->data);
}
 ...
 * each cell visited.
 */
void gfs_domain_cell_traverse(GfsDomain *domain, FttTraverseType order,
                              FttTraverseFlags flags, gint max_depth,
                              FttCellTraverseFunc func, gpointer data) {
 ...
 */
void gfs_domain_traverse_layers(GfsDomain *domain, FttCellTraverseFunc func,
                                gpointer data) {
  g_return_if_fail(domain != NULL);
  g_return_if_fail(func != NULL);
...
static void box_traverse_box(GfsBox *box, gpointer *datum) {
  ...
  ftt_cell_traverse_box(box->root, bb, *order, *flags, *max_depth, func, data);
}
 ...
 */
void gfs_domain_cell_traverse_box(GfsDomain *domain, GtsBBox *box,
                                  FttTraverseType order, FttTraverseFlags flags,
                                  gint max_depth, FttCellTraverseFunc func,
                                  gpointer data) {
  gpointer datum[6];
  ...
  gts_container_foreach(GTS_CONTAINER(domain), (GtsFunc)box_traverse_box,
                        datum);
}
 ...
 * is not verified.
 */
void gfs_domain_cell_traverse_condition(
    GfsDomain *domain, FttTraverseType order, FttTraverseFlags flags,
    gint max_depth, FttCellTraverseFunc func, gpointer data,
    gboolean (*condition)(FttCell *, gpointer), gpointer cdata) {
  gpointer datum[7];
  ...
  gts_container_foreach(GTS_CONTAINER(domain), (GtsFunc)box_traverse_condition,
                        datum);
}
 ...
 */
void gfs_domain_traverse_mixed(GfsDomain *domain, FttTraverseType order,
                               FttTraverseFlags flags, FttCellTraverseFunc func,
                               gpointer data) {
 ...
 */
void gfs_domain_traverse_cut(GfsDomain *domain, GfsGenericSurface *s,
                             FttTraverseType order, FttTraverseFlags flags,
...
static void traverse_cut_2D(GfsBox *box, TraverseCut *p) {
  gfs_cell_traverse_cut_2D(box->root, p->s, p->order, p->flags, p->func,
                           p->data);
}
 ...
 */
void gfs_domain_traverse_cut_2D(GfsDomain *domain, GfsGenericSurface *s,
                                FttTraverseType order, FttTraverseFlags flags,
                                FttCellTraverseCutFunc func, gpointer data) {
  TraverseCut p = {func, data, order, flags, s};
 ...
 * face is never %NULL.
 */
void gfs_domain_face_traverse(GfsDomain *domain, FttComponent c,
                              FttTraverseType order, FttTraverseFlags flags,
                              gint max_depth, FttFaceTraverseFunc func,
                              gpointer data) {
  FttDirection d;
  gpointer     datum[6];
  ...
  if (c == FTT_XYZ) {
    if (boundary_faces) {
      check = TRUE;
      gfs_domain_cell_traverse(domain, order, flags, max_depth,
                               (FttCellTraverseFunc)traverse_all_faces, datum);
    } else {
      gfs_domain_cell_traverse(domain, order, flags, max_depth,
    ...
    gfs_domain_face_traverse(domain, FTT_X, order, flags, max_depth, func,
                             data);
    gfs_domain_face_traverse(domain, FTT_Y, order, flags, max_depth, func,
                             data);
  } else {
    if (boundary_faces) {
      check    = TRUE;
...
static void cell_traverse_boundary(GfsBox *box, gpointer *datum) {
  ...
  if (!GFS_IS_BOX(box->neighbor[*d])) {
    ...
    ftt_cell_traverse_boundary(box->root, *d, *order, *flags, *max_depth, func,
                               data);
  }
}
 ...
 * function for each cell visited.
 */
void gfs_domain_cell_traverse_boundary(GfsDomain *domain, FttDirection d,
                                       FttTraverseType  order,
                                       FttTraverseFlags flags, gint max_depth,
                                       FttCellTraverseFunc func,
                                       gpointer            data) {
  ...
  gts_container_foreach(GTS_CONTAINER(domain), (GtsFunc)cell_traverse_boundary,
                        datum);
}
...
static void range_reduce(void *i, void *o, int *len, MPI_Datatype *type) {
 ...
 */
GtsRange gfs_domain_stats_variable(GfsDomain *domain, GfsVariable *v,
                                   FttTraverseFlags flags, gint max_depth,
 ...
 * per merged cell.
 */
void gfs_domain_stats_merged(GfsDomain *domain, GtsRange *solid,
                             GtsRange *number) {
  gpointer data[2];
  ...
  data[0] = solid;
  data[1] = number;
  gfs_domain_traverse_merged(domain, (GfsMergedTraverseFunc)add_stats_merged,
                             data);
...
static void cell_count(FttCell *cell, guint *count) { (*count)++; }
  ...
  for (d = 0; d < FTT_NEIGHBORS; d++)
    if (GFS_IS_BOUNDARY_MPI(box->neighbor[d]) ||
        (GFS_IS_BOX(box->neighbor[d]) &&
         GFS_BOX(box->neighbor[d])->pid != box->pid))
      ftt_cell_traverse_boundary(box->root, d, FTT_PRE_ORDER,
 ...
 */
void gfs_domain_stats_balance(GfsDomain *domain, GtsRange *size,
                              GtsRange *boundary, GtsRange *mpiwait) {
  g_return_if_fail(domain != NULL);
...
static void add_norm_weighted(FttCell *cell, gpointer *data) {
  ...
  gfs_norm_add(n, GFS_VALUE(cell, v),
               gfs_cell_volume(cell, v->domain) * gfs_function_value(w, cell));
}

#ifdef HAVE_MPI

static void norm_reduce(void *i, void *o, int *len, MPI_Datatype *type) {
  gdouble *in    = (gdouble *)i;
  gdouble *inout = (gdouble *)o;
  g_assert(*len == 5);
 ...
 */
GfsNorm gfs_domain_norm_variable(GfsDomain *domain, GfsVariable *v,
                                 GfsFunction *w, FttTraverseFlags flags,
                                 gint max_depth,
                                 gboolean (*condition)(FttCell *, gpointer),
                                 gpointer cdata) {
  GfsNorm  n;
  ...
  data[1]                  = v;
  data[2]                  = w;
  FttCellTraverseFunc func = w != NULL ? (FttCellTraverseFunc)add_norm_weighted
 ...
 */
GfsNorm gfs_domain_norm_residual(GfsDomain *domain, FttTraverseFlags flags,
                                 gint max_depth, gdouble dt, GfsVariable *res) {
  ResData p = {res, 0.};
...
static void add_norm_velocity(FttCell *cell, gpointer *data) {
  ...
  gfs_norm_add(n, gfs_vector_norm(cell, u),
               gfs_cell_volume(cell, u[0]->domain));
}
...
static void box_split(GfsBox *box, SplitPar *p) {
  ...
  for (d = 0; d < FTT_NEIGHBORS; d++)
    if (GFS_IS_BOUNDARY(box->neighbor[d])) {
      ...
      ftt_cell_destroy(boundary->root, (FttCellCleanupFunc)gfs_cell_cleanup,
                       domain);
      boundary->root = NULL;
      ...
      for (i = 0; i < FTT_CELLS / 2; i++)
        if (child.c[i]) {
          GfsBox *newbox =
              GFS_DOUBLE_TO_POINTER(GFS_VALUE(child.c[i], p->newboxp));
          GtsObject *newboundary =
              GTS_OBJECT(gfs_boundary_new(klass, newbox, d));

          if (GFS_IS_BOUNDARY_PERIODIC(newboundary)) {
            GFS_BOUNDARY_PERIODIC(newboundary)->matching =
                GFS_BOUNDARY_PERIODIC(boundary)->matching;
            GFS_BOUNDARY_PERIODIC(newboundary)->d =
...
static GtsGEdge *node_is_linked(GtsGNode *n1, GtsGNode *n2, FttDirection d) {
  ...
  while (i) {
    if (GTS_GNODE_NEIGHBOR(n1, i->data) == n2 && GFS_GEDGE(i->data)->d == d)
      return i->data;
    i = i->next;
  }
  return NULL;
    ...
    if (child.c[i]) {
      ...
      for (d = 0; d < FTT_NEIGHBORS; d++)
        if (newbox->neighbor[d] != NULL &&
            GFS_IS_BOUNDARY_PERIODIC(newbox->neighbor[d])) {
          GfsBox *matching =
              GFS_BOUNDARY_PERIODIC(newbox->neighbor[d])->matching;
          ...
          g_assert(neighbor);
          GfsBox *newbox1 =
              GFS_DOUBLE_TO_POINTER(GFS_VALUE(neighbor, p->newboxp));
          g_assert(newbox1);
          ...
          if (!node_is_linked(GTS_GNODE(newbox1), GTS_GNODE(newbox),
            ...
            GfsGEdge *edge =
                GFS_GEDGE(gts_gedge_new(GTS_GRAPH(p->domain)->edge_class,
                                        GTS_GNODE(newbox), GTS_GNODE(newbox1)));
            edge->d = d;
          }
 ...
 * @max_depth: maximum depth to consider (-1 means no restriction, see below for
 * -2).
 ...
 */
FttCell *gfs_domain_boundary_locate(GfsDomain *domain, FttVector target,
                                    gint max_depth, GtsObject **where) {
  g_return_val_if_fail(domain != NULL, NULL);
  g_return_val_if_fail(domain->array != NULL, NULL);
  ...
  if (GFS_IS_BOX(b->data)) {
    ...
    while (b) {
      ...
      FttCell *cell =
          ftt_cell_locate(GFS_BOUNDARY(b->data)->root, target, max_depth);
 ...
 */
gdouble gfs_domain_cell_point_distance2(GfsDomain *domain, GtsPoint *p,
                                        gdouble (*distance2)(FttCell *,
                                                             GtsPoint *,
                                                             gpointer),
                                        gpointer data, FttCell **closest) {
...
static void count(FttCell *cell, guint *n) { (*n)++; }
 ...
 * @max_depth.
 */
guint gfs_domain_size(GfsDomain *domain, FttTraverseFlags flags,
                      gint max_depth) {
  guint n = 0;
 ...
 * acceleration.
 */
gdouble gfs_domain_cfl(GfsDomain *domain, FttTraverseFlags flags,
                       gint max_depth) {
  CflData p;
 ...
 *
 * Allocates the memory for fluid state data associated to @cell or its
 * children.
 */
...
void gfs_cell_read_binary(FttCell *cell, GtsFile *fp, GfsDomain *domain) {
  ...
  if (s0 >= 0.) {
    ...
    if (gts_file_read(fp, &s->solid->s[1], sizeof(gdouble),
                      FTT_NEIGHBORS - 1) != FTT_NEIGHBORS - 1) {
      gts_file_error(fp, "expecting numbers (solid->s[1..%d])",
                     FTT_NEIGHBORS - 1);
      return;
    }
    ...
    if (gts_file_read(fp, &s->solid->cm.x, sizeof(gdouble), FTT_DIMENSION) !=
        FTT_DIMENSION) {
      gts_file_error(fp, "expecting numbers (solid->cm[0..%d])",
                     FTT_DIMENSION - 1);
      return;
    }
    if (domain->version >= 90628 &&
        gts_file_read(fp, &s->solid->ca.x, sizeof(gdouble), FTT_DIMENSION) !=
            FTT_DIMENSION) {
      gts_file_error(fp, "expecting numbers (solid->ca[0..%d])",
                     FTT_DIMENSION - 1);
      return;
    }
  }
...
guint gfs_domain_alloc(GfsDomain *domain) {
  ...
  while (i < domain->allocated->len &&
         g_array_index(domain->allocated, gboolean, i))
    i++;
...
GfsVariable *gfs_domain_add_variable(GfsDomain *domain, const gchar *name,
  ...
  if ((v = gfs_variable_new(gfs_variable_class(), domain, name, description)) ==
      NULL)
    return NULL;
...
void gfs_domain_solid_force(GfsDomain *domain, FttVector *pf, FttVector *vf,
                            FttVector *pm, FttVector *vm, GfsFunction *weight) {
  FttComponent  c;
  GfsVariable **v;
...
static void tag_cell_fraction(GtsFifo *fifo, FttCell *cell, GfsVariable *c,
                              GfsVariable *v, guint tag) {
  FttDirection     d;
  FttCellNeighbors n;
...
static void reduce_touching_regions(void *in, void *inout, int *len,
                                    MPI_Datatype *type) {
...
static void unify_tag_range(GfsDomain *domain, TagPar *p) {
  ...
  if (domain->pid >= 0) {
    ...
    MPI_Allgather(&p->tag, 1, MPI_UNSIGNED, tags, 1, MPI_UNSIGNED,
                  MPI_COMM_WORLD);
...
static void match_box_bc(GfsBox *box, TagPar *p) {
  for (p->d = 0; p->d < FTT_NEIGHBORS; p->d++)
    if (GFS_IS_BOUNDARY_PERIODIC(box->neighbor[p->d]))
      ftt_cell_traverse_boundary(box->root, p->d, FTT_PRE_ORDER,
                                 FTT_TRAVERSE_LEAFS, -1,
 ...
 */
guint gfs_domain_tag_droplets(GfsDomain *domain, GfsVariable *c,
                              GfsVariable *tag) {
  g_return_val_if_fail(domain != NULL, 0);
 ...
 */
void gfs_domain_remove_droplets(GfsDomain *domain, GfsVariable *c,
                                GfsVariable *v, gint min, gdouble val) {
  RemoveDropletsPar p;
...
static void tag_cell(GtsFifo *fifo, FttCell *cell, GfsVariable *v, guint tag,
                     guint *size) {
  FttDirection     d;
  FttCellNeighbors n;
  ...
  ftt_cell_neighbors(cell, &n);
  for (d = 0; d < FTT_NEIGHBORS; d++)
    if (n.c[d] && GFS_VALUE(n.c[d], v) == 0. && !GFS_CELL_IS_BOUNDARY(n.c[d]) &&
        (!solid || solid->s[d] > 0.)) {
      if (FTT_CELL_IS_LEAF(n.c[d])) {
        GFS_VALUE(n.c[d], v) = tag;
        gts_fifo_push(fifo, n.c[d]);
        ...
        for (i = 0; i < j; i++)
          if (child.c[i] && GFS_VALUE(child.c[i], v) == 0. &&
              (!GFS_IS_MIXED(child.c[i]) ||
               GFS_STATE(child.c[i])->solid->s[od] > 0.)) {
            GFS_VALUE(child.c[i], v) = tag;
            gts_fifo_push(fifo, child.c[i]);
          }
      }
  ...
  if (FTT_CELL_IS_LEAF(cell)) {
    ...
    for (i = 0; i < FTT_CELLS; i++)
      if (child.c[i] && remove_small(child.c[i], data))
        changed = TRUE;
    if (FTT_CELL_IS_LEAF(cell)) {
      /* all the children have been destroyed i.e. the cell belongs to a small
       * pond */
      if (FTT_CELL_IS_ROOT(cell))
        g_log(G_LOG_DOMAIN, G_LOG_LEVEL_ERROR, "root cell belongs to a pond");
      else
 ...
 * gfs_domain_match().
 */
void gfs_domain_remove_ponds(GfsDomain *domain, gint min,
                             FttCellCleanupFunc cleanup, gpointer data) {
  GArray *     sizes;
  gpointer     dat[6];
  guint        minsize;
  gboolean     changed = FALSE;
...
static gboolean tag_speck(FttCell *cell, GfsVariable *v) {
  if (GFS_VALUE(cell, v) == 0.) {
    ...
    GFS_VALUE(cell, v) = 1.;
    for (d = 0; d < FTT_NEIGHBORS; d++)
      if (GFS_VALUE(n.c[d], v) == 0. && !GFS_CELL_IS_BOUNDARY(n.c[d]) &&
        ...
        if (FTT_CELL_IS_LEAF(n.c[d])) {
          ...
          for (i = 0; i < FTT_CELLS / 2; i++)
            if (!child.c[i] ||
                (GFS_VALUE(child.c[i], v) == 0 && GFS_IS_MIXED(child.c[i]) &&
...
void gfs_domain_remove_specks(GfsDomain *domain) {
  ...
  if (changed)
    gfs_domain_cell_traverse(
        domain, FTT_POST_ORDER, FTT_TRAVERSE_NON_LEAFS, -1,
...
static void cell_combine_traverse(FttCell *cell, FttCell *parent,
                                  FttCellCombineTraverseFunc inside,
                                  gpointer idata, FttCellTraverseFunc outside,
                                  gpointer odata) {
  FttCell * locate;
  FttVector p;
  ...
  if (locate == NULL) {
    if (outside)
      ftt_cell_traverse(cell, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1, outside,
                        odata);
  } else {
    if (FTT_CELL_IS_LEAF(cell))
      ...
      for (i = 0; i < FTT_CELLS; i++)
        if (child.c[i])
          cell_combine_traverse(child.c[i], locate, inside, idata, outside,
                                odata);
    }
  }
}
...
static void box_combine_traverse(GfsBox *box, gpointer *data) {
  ...
  if (locate == NULL) {
    ...
    cell_combine_traverse(box->root, locate, data[1], data[2], data[3],
                          data[4]);
}
 ...
 */
void gfs_domain_combine_traverse(GfsDomain *domain1, GfsDomain *domain2,
                                 FttCellCombineTraverseFunc inside,
                                 gpointer idata, FttCellTraverseFunc outside,
                                 gpointer odata) {
  gpointer data[5];
  ...
  gts_container_foreach(GTS_CONTAINER(domain1), (GtsFunc)box_combine_traverse,
                        data);
}
 ...
 */
GfsDerivedVariable *
gfs_domain_add_derived_variable(GfsDomain *            domain,
  ...
  if (gfs_variable_from_name(domain->variables, info.name) ||
    ...
    return NULL;
  v = GFS_DERIVED_VARIABLE(
      gts_object_new(GTS_OBJECT_CLASS(gfs_derived_variable_class())));
...
gboolean gfs_domain_remove_derived_variable(GfsDomain *  domain,
  ...
  while (i) {
    ...
    if (!strcmp(u->name, name)) {
      ...
      domain->derived_variables =
          g_slist_remove_link(domain->derived_variables, i);
      g_slist_free(i);
      return TRUE;
    }
    i = i->next;
...
static gdouble product(FttCell *cell, GfsFunction *f) {
  GfsSolidVector *solid = GFS_STATE(cell)->solid;
  return ftt_cell_volume(cell) * (solid ? solid->a : 1.) *
         gfs_function_value(f, cell);
}
 ...
 */
void gfs_domain_sum(GfsDomain *domain, FttDirection d, GfsFunction *f,
                    GfsVariable *v) {
  SumData data;
...
GfsRequest *gfs_send_objects(GSList *list, int dest) {
  ...
  if (length > 0) {
    r->buf = buf;
    MPI_Isend(r->buf, length, MPI_BYTE, dest, 1, MPI_COMM_WORLD,
              &r->request[1]);
  }
  return r;
#else  /* not HAVE_MPI */
...
GSList *gfs_receive_objects(GfsDomain *domain, int src) {
  ...
  if (length > 0) {
    ...
    while (fp->type == GTS_STRING) {
      ...
      if (klass == NULL)
        g_error("gfs_receive_object():%d:%d: unknown class '%s'", fp->line,
                fp->pos, fp->token->str);
...
static void unlink_box(GfsBox *box, gint *dest) {
  ...
  for (d = 0; d < FTT_NEIGHBORS; d++)
    if (GFS_IS_BOX(box->neighbor[d])) {
      ...
      box->neighbor[d] = NULL;
      gfs_boundary_mpi_new(gfs_boundary_mpi_class(), box, d, nbox->pid,
                           nbox->id);
...
static void project_refine(FttCell *cell, ProjData *p) {
  ...
  p->cell = cell;
  gfs_domain_cell_traverse_condition(
      p->proj->domain, FTT_PRE_ORDER, FTT_TRAVERSE_ALL, level + 1,
      (FttCellTraverseFunc)update_maxlevel, &maxlevel, overlap, p);
  GFS_VALUE(cell, p->maxlevel) = maxlevel;
  if (FTT_CELL_IS_LEAF(cell)) {
    if (maxlevel > level) {
      ftt_cell_refine_single(cell, GFS_DOMAIN(p->proj)->cell_init,
                             GFS_DOMAIN(p->proj)->cell_init_data);
      ftt_cell_flatten(cell, 2 * p->proj->c,
                       (FttCellCleanupFunc)gfs_cell_cleanup, p->proj);
    }
  } else
    ftt_cell_flatten(cell, 2 * p->proj->c, (FttCellCleanupFunc)gfs_cell_cleanup,
                     p->proj);
}
...
static void project_coarsen_box(GfsBox *box, ProjData *p) {
  ftt_cell_coarsen(box->root, (FttCellCoarsenFunc)finer, p,
                   (FttCellCleanupFunc)gfs_cell_cleanup, p->proj);
}
...
void gfs_domain_projection_reshape(GfsDomainProjection *proj) {
  ...
  gts_container_foreach(GTS_CONTAINER(domain), (GtsFunc)project_coarsen_box,
                        &p);
...
static void traverse_overlapping(FttCell *cell, ProjectionTraverse *p) {
  p->cell = cell;
  gfs_domain_cell_traverse_condition(
      p->proj->domain, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, ftt_cell_level(cell),
      (FttCellTraverseFunc)apply_func, p, overlap, p);
}
