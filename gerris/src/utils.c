...
static gint get_tmp_file(gchar *tmpl, GTmpFileCallback f, int flags, int mode) {
  char *            XXXXXX;
  int               count, fd;
  static const char letters[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
  static const int  NLETTERS  = sizeof(letters) - 1;
  glong             value;
  GRand *           ran_gen = g_rand_new();
  guint32           r_1     = g_rand_int(ran_gen);
  guint32           r_2     = g_rand_int(ran_gen);
  g_rand_free(ran_gen);
...
/* we could use mkdtemp() but we don't trust some implementations
   (e.g. on AIX) */
static gint wrap_mkdir(gchar *tmpl, int flags G_GNUC_UNUSED, int mode) {
...
static gint wrap_mkfifo(gchar *tmpl, int flags G_GNUC_UNUSED, int mode) {
...
gchar *gfs_template(void) {
  ...
  return tmpdir ? g_strconcat(tmpdir, "/gfsXXXXXX", NULL)
                : g_strdup("/tmp/gfsXXXXXX");
}
...
typedef gdouble (*GfsFunctionFunc)(const FttCell *cell, const FttCellFace *face,
...
typedef gdouble (*GfsFunctionDerivedFunc)(const FttCell *    cell,
                                          ...
                                          GfsSimulation *sim, gpointer data);
...
static void append_pending_function(const GfsFunction *f, guint line,
                                    guint id) {
  GfsSimulation *sim    = gfs_object_simulation(f);
    ...
    if (lv || ldv) {
      ...
      if (lv) {
        ...
        while (i) {
          ...
          g_string_append_printf(
              pending_functions,
              "    %s = gfs_dimensional_value (var[%d],\n"
              "           gfs_face_interpolated_value_generic (face, "
              "var[%d]));\n",
              v->name, index, index);
          i = i->next;
          index++;
      ...
      if (ldv) {
        ...
        while (i) {
          GfsDerivedVariable *v = i->data;
          g_string_append_printf(pending_functions,
                                 "  %s = (* (Func) dvar[%d]->func) (cell, "
                                 "face, sim, dvar[%d]->data);\n",
                                 v->name, index, index);
          i = i->next;
          index++;
        }
...
static void link_module(GfsFunction *f) {
  ...
  if (f->constant) {
    ...
    if (n > 0) {
      ...
      n                 = 0;
      while (*s) {
        g_assert((f->dvar[n] = gfs_derived_variable_from_name(
                      domain->derived_variables, *s)));
        n++;
        s++;
      }
    }
...
static GModule *compile(GtsFile *fp, const gchar *dirname,
                        const gchar *finname) {
  GModule *module = NULL;
...
static gboolean fit_index_dimension(GfsCartesianGrid *grid, guint *val,
                                    GtsFile *fp) {
  guint  i, j;
...
static void function_read(GtsObject **o, GtsFile *fp) {
  ...
  if (fp->type == GTS_STRING && !f->spatial && !f->constant &&
      strlen(fp->token->str) > 3) {
  ...
  if (f->isexpr) {
    if (fp->type == GTS_INT || fp->type == GTS_FLOAT) {
      ...
      if ((f->v = gfs_variable_from_name(domain->variables, f->expr->str)) ||
          (f->dv = gfs_derived_variable_from_name(domain->derived_variables,
                                                  f->expr->str))) {
        gts_file_next_token(fp);
        return;
      }
...
static void function_destroy(GtsObject *object) {
  ...
  (*GTS_OBJECT_CLASS(gfs_function_class())->parent_class->destroy)(object);
}
...
gdouble gfs_function_value(GfsFunction *f, FttCell *cell) {
  ...
  if (f->s) {
    ...
    dimensional = (*(GfsFunctionDerivedFunc)f->dv->func)(
        cell, NULL, gfs_object_simulation(f), f->dv->data);
  else if (f->f)
    dimensional =
        (*f->f)(cell, NULL, gfs_object_simulation(f), f->var, f->dvar);
  else
    dimensional = f->val;
...
gdouble gfs_function_face_value(GfsFunction *f, FttCellFace *fa) {
  ...
  if (f->s) {
    ...
    dimensional = gfs_dimensional_value(
        f->v, gfs_face_interpolated_value_generic(fa, f->v));
  else if (f->dv)
    dimensional = (*(GfsFunctionDerivedFunc)f->dv->func)(
        NULL, fa, gfs_object_simulation(f), f->dv->data);
  else if (f->f)
...
static void gfs_function_spatial_init(GfsFunction *f) { f->spatial = TRUE; }
...
static void gfs_function_map_init(GfsFunction *f) { f->nomap = TRUE; }
...
static void gfs_function_constant_init(GfsFunction *f) { f->constant = TRUE; }
...
static void eigsrt(gdouble d[FTT_DIMENSION],
...
void gfs_eigenvalues(gdouble a[FTT_DIMENSION][FTT_DIMENSION],
                     ...
                     gdouble v[FTT_DIMENSION][FTT_DIMENSION]) {
  gint    j, iq, ip, i;
  gdouble tresh, theta, tau, t, sm, s, h, g, c, b[FTT_DIMENSION],
      z[FTT_DIMENSION];
...
static GfsFormat *format_new(const gchar *s, guint len, GfsFormatType t) {
 ...
 */
GSList *gfs_format_new(const gchar *format, GtsFile *fp, gboolean *dynamic,
                       gboolean *parallel) {
  ...
  while (*c != '\0') {
    if (*c == '%') {
      ...
      if (len > 0)
        formats =
            g_slist_prepend(formats, format_new(start, len, GFS_NONE_FORMAT));
      ...
      if (*c == '%')
        ...
        if (*prev == 'l') {
          formats = g_slist_prepend(formats,
                                    format_new(startf, len, GFS_ITER_FORMAT));
          if (dynamic)
            *dynamic = TRUE;
        } else {
          formats =
              g_slist_prepend(formats, format_new(startf, len, GFS_PID_FORMAT));
          if (parallel)
            *parallel = TRUE;
        }
      } else if (gfs_char_in_string(*c, "eEfFgGaA")) {
        formats =
            g_slist_prepend(formats, format_new(startf, len, GFS_TIME_FORMAT));
        if (dynamic)
          *dynamic = TRUE;
      } else {
        if (fp)
          gts_file_error(fp, "unknown conversion specifier `%c' of format `%s'",
                         *c, format);
        return NULL;
      }
      start = c;
...
gdouble gfs_format_time_value(GSList *format, const gchar *string) {
  ...
  while (format) {
    ...
    case GFS_ITER_FORMAT:
      c = s;
      while (gfs_char_in_string(*s, "0123456789"))
        s++;
      c1  = *s;
      *s  = '\0';
      tmp = atoi(c);
    ...
    case GFS_TIME_FORMAT:
      c = s;
      while (gfs_char_in_string(*s, "0123456789eE-+."))
        s++;
      c1  = *s;
      *s  = '\0';
      tmp = atof(c);
...
void gfs_cell_message(const FttCell *cell, const gchar *format, ...) {
  ...
  FttVector p;
  ftt_cell_pos(cell, &p);
  gchar * s = g_strdup_printf("%p:(%g,%g,%g):%d", cell, p.x, p.y, p.z,
                             ftt_cell_level(cell));
  va_list ap;
  va_start(ap, format);
 ...
 */
void gfs_debug_enabled(gboolean enabled) { GfsDebug = enabled; }
...
gboolean gfs_read_variable_vector(GtsFile *    fp,
  ...
  for (i = 0; i < FTT_DIMENSION && status; i++) {
    if (!(vector[i] =
              gfs_variable_from_name(domain->variables, component[i]))) {
      gts_file_error(fp, "unknown variable '%s'", component[i]);
      status = FALSE;
    } else if (vector[i]->component != i) {
      gts_file_error(fp, "variable '%s' is not the correct vector component",
                     component[i]);
      status = FALSE;
    }
  }
