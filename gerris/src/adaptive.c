...
static void gfs_adapt_read(GtsObject **o, GtsFile *fp) {
  ...
  while (fp->type != GTS_ERROR && fp->type != '}') {
    ...
    if (fp->type != GTS_STRING) {
      ...
      a->c   = gfs_domain_get_or_add_variable(domain, fp->token->str,
...
static gboolean gfs_adapt_event(GfsEvent *event, GfsSimulation *sim) {
  ...
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_adapt_class())->parent_class)
            ->event)(event, sim)) {
...
static gboolean gfs_adapt_vorticity_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_adapt_vorticity_class())->parent_class)
            ->event)(event, sim)) {
    GfsAdaptVorticity *a = GFS_ADAPT_VORTICITY(event);

    a->u = gfs_domain_velocity(GFS_DOMAIN(sim));
    a->maxa =
        gfs_domain_norm_velocity(GFS_DOMAIN(sim), FTT_TRAVERSE_LEAFS, -1).infty;
    return TRUE;
  }
  return FALSE;
}
...
static gboolean gfs_adapt_gradient_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_adapt_gradient_class())->parent_class)
            ->event)(event, sim)) {
    ...
    if (!gfs_function_get_variable(GFS_ADAPT_FUNCTION(event)->f)) {
      ...
      gfs_domain_cell_traverse(GFS_DOMAIN(sim), FTT_POST_ORDER,
...
static void compute_gradient(FttCell *cell, GfsAdaptError *a) {
  GFS_VALUE(cell, a->dv[a->c]) =
      gfs_center_regular_gradient(cell, a->c, GFS_ADAPT_GRADIENT(a)->v);
}
...
static void scale(FttCell *cell, GfsAdaptError *a) {
  GFS_VALUE(cell, a->v) =
      sqrt(GFS_VALUE(cell, a->v)) / 8. * GFS_ADAPT_GRADIENT(a)->dimension;
}

static gboolean gfs_adapt_error_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_adapt_error_class())->parent_class)
            ->event)(event, sim)) {
...
static gboolean gfs_adapt_thickness_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_adapt_thickness_class())->parent_class)
            ->event)(event, sim)) {
    gfs_domain_cell_traverse(GFS_DOMAIN(sim), FTT_PRE_ORDER, FTT_TRAVERSE_ALL,
                             -1, (FttCellTraverseFunc)update_thickness, event);
    return TRUE;
  }
  return FALSE;
}
...
static gboolean adapt_global(GfsSimulation *simulation, guint *depth,
  ...
  while (changed) {
    ...
    if (fine && ((cfine < -ccoarse && apar.nc > maxcells) ||
      ...
      ftt_cell_coarsen(fine, (FttCellCoarsenFunc)fine_cell_coarsenable, &apar,
...
static void coarsen_box(GfsBox *box, AdaptLocalParams *p) {
  ftt_cell_coarsen(box->root, (FttCellCoarsenFunc)coarsen_cell, p,
...
static void refine_cell_mark(FttCell *cell, AdaptLocalParams *p) {
  ...
  COARSENABLE(cell, p) =
      !GFS_CELL_IS_PERMANENT(cell) && !ftt_refine_corner(cell);
