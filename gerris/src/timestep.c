...
void gfs_reset_gradients(GfsDomain *domain, guint dimension, GfsVariable **g) {
  ...
  gfs_domain_traverse_leaves(domain, (FttCellTraverseFunc)reset_cell_gradients,
                             &p);
}
...
static void correct_normal_velocity(FttCellFace *face, CorrectPar *par) {
  GfsGradient g;
  gdouble     dp, f;
  ...
  if (par->gv)
    GFS_VALUE(face->cell, par->gv[face->d / 2]) +=
  ...
  if (par->gv)
    GFS_VALUE(face->neighbor, par->gv[face->d / 2]) +=
        dp * GFS_FACE_FRACTION_RIGHT(face);
}
...
static void add_face_source(FttCellFace *face, FaceSource *f) {
  gdouble      dp;
  FttComponent c;
      ...
      while (i) {
        ...
        if (s->face_value) {
          ...
          gts_container_foreach(GTS_CONTAINER(domain),
                                (GtsFunc)ignore_dirichlet_boundaries, &f);
          gfs_domain_face_traverse(domain, c, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                                   -1, (FttFaceTraverseFunc)add_face_source,
                                   &f);
        }
        i = i->next;
      }
    ...
    while (i) {
      if (GFS_IS_SOURCE_TENSION(i->data)) {
        ...
        gfs_correct_normal_velocities(domain, FTT_DIMENSION,
                                      GFS_SOURCE_TENSION_GENERIC(s)->c, g, dt);
      }
      i = i->next;
    }
  }
...
static void volume_sources(GfsDomain *domain, GfsVariable *p,
                           GfsVariable *div) {
  SourceVolumePar par;
  par.div = div;
  par.v   = p;
 ...
 */
void gfs_mac_projection(GfsDomain *domain, GfsMultilevelParams *par, gdouble dt,
  ...
  if (par->residual.infty > par->tolerance)
    g_warning("MAC projection: max residual %g > %g", par->residual.infty,
              par->tolerance);
}
...
void gfs_approximate_projection(
  ...
  if (par->residual.infty > par->tolerance)
    g_warning("approx projection: max residual %g > %g", par->residual.infty,
              par->tolerance);
}
...
static void face_values_init(FttCellTraverseFunc face_values,
                             GfsAdvectionParams *par) {
  if (par->scheme == GFS_GODUNOV && par->v->component < FTT_DIMENSION &&
    ...
    for (c = 0; c < 2; c++) {
      ...
      gfs_domain_traverse_leaves(domain, (FttCellTraverseFunc)save_face_values,
                                 par->v->face[c]);
    }
...
static void face_values_set(FttCellTraverseFunc face_values,
                            GfsAdvectionParams *par) {
  if (par->v->component < 2 && par->v->domain->has_rotated_bc)
    /* vector: restore stored face values */
    gfs_domain_traverse_leaves(par->v->domain,
                               (FttCellTraverseFunc)restore_face_values,
...
void gfs_predicted_face_velocities(GfsDomain *domain, guint d,
  ...
  gfs_domain_face_traverse(
      domain, d == 2 ? FTT_XY : FTT_XYZ, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
      (FttFaceTraverseFunc)gfs_face_reset_normal_velocity, NULL);
  ...
  for (c = 0; c < d; c++) {
    ...
    gfs_domain_face_traverse(
        domain, c, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
...
void gfs_diffusion(GfsDomain *domain, GfsMultilevelParams *par, GfsVariable *v,
  ...
  par->residual_before = par->residual = gfs_domain_norm_variable(
      domain, res, NULL, FTT_TRAVERSE_LEAFS, -1, NULL, NULL);
  ...
  while (par->niter < par->nitermin ||
    ...
    gfs_diffusion_cycle(domain, minlevel, maxlevel, par->nrelax, v, rhs, rhoc,
                        metric, res);
    par->residual = gfs_domain_norm_variable(
        domain, res, NULL, FTT_TRAVERSE_LEAFS, -1, NULL, NULL);
  ...
  if (par->residual.infty > par->tolerance)
    g_warning("gfs_diffusion(): max residual %g > %g", par->residual.infty,
              par->tolerance);
}
...
static void add_pressure_gradient(FttCell *cell, GfsAdvectionParams *par) {
  GFS_VALUE(cell, par->fv) -=
      GFS_VALUE(cell, par->g[par->v->component]) * par->dt;
}

static void add_sinking(FttCellFace *face, GfsAdvectionParams *par) {
  GFS_FACE_NORMAL_VELOCITY_LEFT(face) +=
      gfs_function_face_value(par->sink[face->d / 2], face);
...
void gfs_add_sinking_velocity(GfsDomain *domain, GfsAdvectionParams *par) {
  ...
  if (par->sink[0])
    gfs_domain_face_traverse(domain, FTT_XYZ, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             -1, (FttFaceTraverseFunc)add_sinking, par);
}
...
void gfs_remove_sinking_velocity(GfsDomain *domain, GfsAdvectionParams *par) {
  ...
  if (par->sink[0])
    gfs_domain_face_traverse(domain, FTT_XYZ, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             -1, (FttFaceTraverseFunc)remove_sinking, par);
}

static void variable_sources(GfsDomain *domain, GfsAdvectionParams *par,
                             GfsVariable *sv, GfsVariable **gmac,
                             GfsVariable **g) {
...
static void variable_diffusion(GfsDomain *domain, GfsSourceDiffusion *d,
                               GfsAdvectionParams *par, GfsVariable *rhs,
                               GfsFunction *alpha) {
  ...
  gfs_diffusion_coefficients(domain, d, par->dt, rhoc, metric, alpha,
                             d->D->par.beta);
...
void gfs_centered_velocity_advection_diffusion(
    GfsDomain *domain, guint dimension, GfsAdvectionParams *par,
...
void gfs_tracer_advection_diffusion(GfsDomain *domain, GfsAdvectionParams *par,
  ...
  if ((d = source_diffusion(par->v))) {
    ...
    variable_diffusion(domain, d, par, rhs,
                       par->v->component < FTT_DIMENSION ? alpha : NULL);
    gts_object_destroy(GTS_OBJECT(rhs));
  } else {
...
static void gfs_surface_generic_bc_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_surface_generic_bc_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_surface_generic_bc_class())->parent_class->read)(o,
                                                                            fp);
  ...
  if (!gts_object_class_is_from_class(klass, gfs_surface_generic_bc_class())) {
    gts_file_error(fp, "class `%s' is not a GfsSurfaceGenericClass",
                   fp->token->str);
    return;
  }
...
GfsSurfaceGenericBcClass *gfs_surface_bc_class(void) {
  ...
  if (klass == NULL) {
    ...
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_surface_generic_bc_class()), &gfs_surface_bc_info);
  }
