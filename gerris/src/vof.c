 ...
 */
void gfs_line_center(const FttVector *m, gdouble alpha, gdouble a,
                     FttVector *p) {
  FttVector n;
  gdouble   b;
 ...
 */
void gfs_plane_center(const FttVector *m, gdouble alpha, gdouble a,
                      FttVector *p) {
  FttVector n;
  gdouble   b, amax;
...
gdouble gfs_plane_area_center(const FttVector *m, gdouble alpha, FttVector *p) {
  ...
  return area *
         sqrt(1. / (n.x * n.x * n.y * n.y) + 1. / (n.x * n.x * n.z * n.z) +
              1. / (n.z * n.z * n.y * n.y)) /
         6.;
}
#endif /* 3D */
 ...
 */
gdouble gfs_vof_plane_interpolate(FttCell *cell, FttVector *p, guint level,
                                  GfsVariableTracerVOF *t, FttVector *m) {
 ...
 */
gdouble gfs_vof_interpolate(FttCell *cell, FttVector *p, guint level,
    ...
    for (x = -1; x <= 1; x++)
      for (y = -1; y <= 1; y++)
        if (x != 0 || y != 0 || z != 0) {
          ...
          o.z = p.z + h * z;
          FttCell *neighbor =
              gfs_domain_boundary_locate(v->domain, o, level, NULL);
          if (neighbor)
            F(x + 1, y + 1, z + 1) = gfs_vof_interpolate(
                neighbor, &o, level, GFS_VARIABLE_TRACER_VOF(v));
          else
            F(x + 1, y + 1, z + 1) = -1.;
        }
...
static void youngs_normal(FttCell *cell, GfsVariable *v, FttVector *n) {
  ...
  stencil(cell, v, f);
#if FTT_2D
  n->x = (f[0][2] + 2. * f[0][1] + f[0][0] - 2. * f[2][1] - f[2][2] - f[2][0]) /
         8.;
  n->y = (f[2][0] + 2. * f[1][0] + f[0][0] - 2. * f[1][2] - f[2][2] - f[0][2]) /
         8.;
  n->z = 0.;
#else  /* 3D */
...
static void allocate_normal_alpha(GfsVariableTracerVOF *t) {
  ...
  for (c = 0; c < FTT_DIMENSION; c++) {
    ...
    gchar *      description = g_strdup_printf(
        "%s-component of the normal to the interface defined by %s", index[c],
        v->name);
...
static gboolean variable_tracer_vof_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_variable_tracer_vof_class())->parent_class)
            ->event)(event, sim)) {
    (*GFS_VARIABLE_TRACER_VOF_CLASS(GTS_OBJECT(event)->klass)->update)(
        GFS_VARIABLE(event), GFS_DOMAIN(sim));
    return TRUE;
  }
  return FALSE;
}
...
static void variable_tracer_vof_destroy(GtsObject *o) {
  ...
  (*GTS_OBJECT_CLASS(gfs_variable_tracer_vof_class())->parent_class->destroy)(
      o);
}

static void variable_tracer_vof_read(GtsObject **o, GtsFile *fp) {
  (*GTS_OBJECT_CLASS(gfs_variable_tracer_vof_class())->parent_class->read)(o,
                                                                           fp);
...
static void variable_tracer_vof_init(GfsVariable *v) {
  ...
  GFS_VARIABLE_TRACER_VOF(v)->concentrations = GTS_SLIST_CONTAINER(
...
static gdouble plane_volume_shifted(FttVector m, gdouble alpha,
                                    FttVector p[2]) {
  FttComponent c;
...
static gdouble fine_fraction(FttCellFace *face, GfsVariable *v, gdouble un,
                             FttVector q[2]) {
...
static gdouble coarse_fraction(FttCellFace *face, GfsVariable *v, gdouble un,
                               FttVector q[2]) {
  ...
  if (f == 0. || f == 1.)
    ...
    FttVector    m, o, q1[2];
    gdouble      alpha =
        GFS_VALUE(face->neighbor, GFS_VARIABLE_TRACER_VOF(v)->alpha);
  ...
  if (ftt_face_type(face) == FTT_FINE_COARSE) {
    ...
    if (un > 0.) {
      ...
      if (GFS_IS_FULL(f)) {
        ...
        if (fine_fraction(face, p->vof,
                          un * p->par->dt / ftt_cell_size(face->cell),
                          q) != f) {
          p->too_coarse++;
...
static void vof_cell_fine_init(FttCell *parent, VofParms *p) {
  ...
  for (n = 0; n < FTT_CELLS; n++) {
    ...
    if (child.c[n]) {
      ...
      FttComponent c;
      for (c = 0; c < FTT_DIMENSION; c++)
        div[n] += GFS_STATE(child.c[n])->f[2 * c].un -
                  GFS_STATE(child.c[n])->f[2 * c + 1].un;
    }
  }
...
static void vof_cell_fine_init_with_streamfunction(FttCell * parent,
                                                   VofParms *p) {
  gfs_cell_fine_init(parent, p->domain);
...
static void fix_too_coarse(GfsDomain *domain, VofParms *p) {
  ...
  p->domain     = domain;
  p->too_coarse = 0;
  gfs_domain_face_traverse(domain, p->c, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
...
#endif /* 2D */
  domain->cell_init =
      (FttCellInitFunc)(streamfunction ? vof_cell_fine_init_with_streamfunction
                                       : vof_cell_fine_init);
...
static void concentration_face_values(FttCell *cell, VofParms *p) {
  ...
  gdouble unorm =
      p->par->dt * (s->f[2 * p->c].un + s->f[2 * p->c + 1].un) / (2. * size);
  if (p->sink)
...
static void vof_flux(FttCellFace *face, VofParms *p) {
  ...
  gdouble un   = GFS_FACE_NORMAL_VELOCITY(face) * p->par->dt / size,
          dun[FTT_DIMENSION - 1];
  if (p->sink)
  ...
  for (i = 0; i < n; i++) {
    ...
    case FTT_FINE_COARSE: {
      flux = uni > 0. ? fine_fraction(face, p->vof, uni, q) * uni * f
                      : coarse_fraction(face, p->vof, -uni / 2., q) * uni * f;
      if (p->par->v == p->vof)
        GFS_VALUE(face->neighbor, p->vpar.fv) += uni * f / FTT_CELLS;
      else
        flux *= uni > 0. ? GFS_STATE(face->cell)->f[face->d].v
                         : GFS_STATE(face->neighbor)
                               ->f[FTT_OPPOSITE_DIRECTION(face->d)]
                               .v;
...
static void reset_fluxes(FttCellFace *face, VofParms *p) {
  GFS_VALUE(face->cell, p->par->fv) = GFS_VALUE(face->neighbor, p->par->fv) =
      0.;
  GFS_VALUE(face->cell, p->vpar.fv) = GFS_VALUE(face->neighbor, p->vpar.fv) =
      0.;
}
...
static void grad_u(FttCell *cell, VofParms *p) {
  ...
  for (c = 0; c < FTT_DIMENSION - 1; c++) {
    GFS_VALUE(cell, p->du[c]) =
        gfs_center_gradient(cell, d, p->u->i) / ftt_cell_size(cell);
...
static void concentration_times_dV(FttCell *cell, VofParms *p) {
  GFS_VALUE(cell, p->par->v) *=
      GFS_VALUE(cell, p->vpar.v) * GFS_VALUE(cell, p->vof);
}
...
static void concentration_over_dV(FttCell *cell, VofParms *p) {
  ...
  if (f > 0.)
    GFS_VALUE(cell, p->par->v) =
        GFS_VALUE(cell, p->par->v) / (GFS_VALUE(cell, p->vpar.v) * f);
  else
...
void gfs_tracer_vof_advection(GfsDomain *domain, GfsAdvectionParams *par) {
  ...
  gfs_domain_traverse_leaves(domain, (FttCellTraverseFunc)initialize_dV,
                             p.vpar.v);
  par->fv = gfs_temporary_variable(domain);
  GSList *concentrations =
             GFS_VARIABLE_TRACER_VOF(p.vof)->concentrations->items,
         *j;
  j = concentrations;
  while (j) {
    GFS_VARIABLE_TRACER(j->data)->advection.fv = gfs_temporary_variable(domain);
    gfs_domain_traverse_leaves(domain, (FttCellTraverseFunc)per_vof_volume,
                               j->data);
    j = j->next;
  }
  for (c = 0; c < FTT_DIMENSION; c++) {
    ...
    while (j) {
      ...
      if (par->sink[0]) {
        ...
        gfs_domain_traverse_leaves(domain,
                                   (FttCellTraverseFunc)add_sink_velocity, &p);
        gfs_domain_traverse_leaves(domain, (FttCellTraverseFunc)grad_u, &p);
        for (d = 0; d < FTT_DIMENSION - 1; d++)
          gfs_domain_bc(domain, FTT_TRAVERSE_LEAFS, -1, p.du[d]);
      }
      gfs_domain_traverse_leaves(
          domain, (FttCellTraverseFunc)concentration_face_values, &p);
      gfs_domain_face_bc(domain, p.c, p.par->v);
      gfs_domain_face_traverse(domain, p.c, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                               -1, (FttFaceTraverseFunc)vof_flux, &p);
      if (p.sink) {
        gfs_domain_traverse_leaves(
            domain, (FttCellTraverseFunc)remove_sink_velocity, &p);
        p.sink = NULL;
        gfs_domain_traverse_leaves(domain, (FttCellTraverseFunc)grad_u, &p);
        for (d = 0; d < FTT_DIMENSION - 1; d++)
          gfs_domain_bc(domain, FTT_TRAVERSE_LEAFS, -1, p.du[d]);
      ...
      gfs_domain_traverse_leaves(
          domain, (FttCellTraverseFunc)concentration_times_dV, &p);
      gfs_domain_traverse_merged(domain, (GfsMergedTraverseFunc)par->update,
                                 par);
      p.par->fv = fv;
      p.par->v  = p.vof;
      j         = j->next;
    ...
    while (j) {
      p.par->v = j->data;
      gfs_domain_traverse_leaves(
          domain, (FttCellTraverseFunc)concentration_over_dV, &p);
      p.par->v = p.vof;
      j        = j->next;
    }

    /* update VOF data (normals etc...) */
    (*GFS_VARIABLE_TRACER_VOF_CLASS(GTS_OBJECT(p.par->v)->klass)->update)(
        p.par->v, domain);
  }
  ...
  while (j) {
    ...
    gfs_domain_traverse_leaves(domain, (FttCellTraverseFunc)per_cell_volume,
                               j->data);
    j = j->next;
  }
...
gdouble gfs_vof_face_value(const FttCellFace *face, GfsVariableTracerVOF *t) {
  ...
  GfsVariable *v = GFS_VARIABLE(t);
  gdouble      vright,
      vleft = GFS_VALUE(face->cell, v); // face_value (face->cell, face->d, v);
  if (ftt_face_type(face) == FTT_FINE_COARSE) {
    gdouble f = GFS_VALUE(face->neighbor, v);
    ...
    vright = GFS_VALUE(
        face->neighbor,
        v); // face_value (face->neighbor, FTT_OPPOSITE_DIRECTION (face->d), v);
 ...
 */
guint gfs_vof_facet(FttCell *cell, GfsVariableTracerVOF *t, FttVector *p,
                    FttVector *m) {
  g_return_val_if_fail(cell != NULL, 0);
  g_return_val_if_fail(t != NULL, 0);
  g_return_val_if_fail(p != NULL, 0);
 ...
 */
gdouble gfs_vof_facet_distance2(FttCell *cell, GfsVariableTracerVOF *t,
                                GtsPoint *p) {
...
static gdouble fraction(FttVector *p, guint level, GfsVariable *v) {
...
static gdouble local_height(FttVector *p, FttVector *origin, guint level,
                            GfsVariable *v, FttDirection d,
                            GtsVector interface) {
...
static gboolean curvature_along_direction(FttCell *             cell,
                                          GfsVariableTracerVOF *t,
                                          FttComponent c, gdouble *kappa,
                                          gdouble *kmax, GtsVector *interface,
                                          guint *n) {
  GfsVariable *v = GFS_VARIABLE(t);
  ...
  if (found_all_heights) {
    ...
    *kappa      = hxx / (size * sqrt(dnm * dnm * dnm));
    ...
    if (found_all_heights) {
      ...
      *kappa =
          (hxx + hyy + hxx * hy * hy + hyy * hx * hx - 2. * hxy * hx * hy) /
          (size * sqrt(dnm * dnm * dnm));
      if (kmax) {
        gdouble km = *kappa / 2.;
        /* Gaussian curvature */
...
#endif
    gdouble dnm = 1. + hx * hx + hy * hy;
    kappa = (hxx + hyy + hxx * hy * hy + hyy * hx * hx - 2. * hxy * hx * hy) /
            sqrt(dnm * dnm * dnm);
    if (kmax) {
...
static void parabola_fit_axi_curvature(const ParabolaFit *p, gdouble r,
                                       gdouble h, gdouble *kappa,
                                       gdouble *kmax) {
  gdouble nr = (p->m[0] * p->a[1] + p->m[1]) / sqrt(1. + p->a[1] * p->a[1]);
  gdouble kaxi =
      -nr / MAX(r, h / 2.); /* limit the minimum radius to half the grid size */
  *kappa += kaxi;
  if (kmax)
    *kmax = MAX(*kmax, fabs(kaxi));
}
...
static void add_vof_center(FttCell *cell, FttVector *p, guint level,
                           FttVector *origin, GfsVariableTracerVOF *t,
...
static void fit_from_fractions(FttCell *cell, GfsVariable *v,
                               ParabolaFit *fit) {
  gdouble   h     = ftt_cell_size(cell);
  guint     level = ftt_cell_level(cell);
  gint      x, y, z = 0;
 ...
 * the mean curvature).
 *
 * Returns: (double in 3D) the mean curvature of the interface contained in
 * @cell.
 */
...
static guint independent_positions(GtsVector *interface, guint n) {
  ...
  for (j = 1; j < n; j++) {
    ...
    for (i = 0; i < j && !depends; i++) {
      ...
      FttComponent c;
      for (c = 0; c < FTT_DIMENSION; c++)
        d2 += (interface[i][c] - interface[j][c]) *
              (interface[i][c] - interface[j][c]);
      depends = d2 < 0.5 * 0.5;
    }
    ni += !depends;
  }
...
static gdouble neighboring_column(FttCell *cell, GfsVariable *h, FttComponent c,
                                  gdouble orientation, FttDirection d,
                                  gdouble *x) {
  ...
  if (ftt_cell_level(cell) == ftt_cell_level(n)) {
    ...
    FttCellChildren child;
    int             i,
        m = ftt_cell_children_direction(n, FTT_OPPOSITE_DIRECTION(d), &child);
    for (i = 0; i < m; i++)
...
static void curvature_from_h(FttCell *cell, GfsDomain *domain, gdouble x[3],
                             gdouble h[3], gdouble orientation, FttComponent c,
                             gdouble *kappa, gdouble *kmax) {
  gdouble size = ftt_cell_size(cell);
  gdouble det = x[0] * x[1] * (x[0] - x[1]), a = x[1] * (h[0] - h[2]),
          b   = x[0] * (h[1] - h[2]);
...
gboolean gfs_curvature_along_direction(FttCell *                   cell,
  ...
  if (h[0] != GFS_NODATA && x[0] == 1.) {
    ...
    if (h[1] != GFS_NODATA && x[1] == 1.) {
      x[1] = -x[1];
      curvature_from_h(cell, GFS_VARIABLE(t)->domain, x, h, orientation, c,
                       kappa, kmax);
      return TRUE;
    }
  }
...
static gboolean curvature_along_direction_new(FttCell *                   cell,
  ...
  if (!hv) {
   ...
   collect potential interface positions */
    hv =
        gfs_closest_height(ftt_cell_neighbor(cell, 2 * oc), t, c, &orientation);
    if (!hv)
      hv = gfs_closest_height(ftt_cell_neighbor(cell, 2 * oc + 1), t, c,
                              &orientation);
    if (!hv) /* give up */
      return FALSE;
  } else if (fabs(GFS_VALUE(cell, hv)) > 1.)
  ...
  if (h[2] != GFS_NODATA && h[0] != GFS_NODATA && h[1] != GFS_NODATA) {
    curvature_from_h(cell, GFS_VARIABLE(t)->domain, x, h, orientation, c, kappa,
                     kmax);
    return TRUE;
...
gdouble gfs_height_curvature_new(FttCell *cell, GfsVariableTracerVOFHeight *t,
  ...
  guint     n = 0;
  for (c = 0; c < FTT_DIMENSION; c++) /* try each direction */
    if (curvature_along_direction_new(cell, t, try[c], &kappa, kmax, interface,
                                      &n))
      return kappa;
...
#else
    gdouble dx =
        (f[2][0][0] + f[2][1][0] + f[2][2][0] - f[0][0][0] - f[0][1][0] -
         f[0][2][0] + f[2][0][1] + f[2][1][1] + f[2][2][1] - f[0][0][1] -
         f[0][1][1] - f[0][2][1] + f[2][0][2] + f[2][1][2] + f[2][2][2] -
         f[0][0][2] - f[0][1][2] - f[0][2][2]);
    gdouble dy =
        (f[0][2][0] + f[1][2][0] + f[2][2][0] - f[0][0][0] - f[1][0][0] -
         f[2][0][0] + f[0][2][1] + f[1][2][1] + f[2][2][1] - f[0][0][1] -
         f[1][0][1] - f[2][0][1] + f[0][2][2] + f[1][2][2] + f[2][2][2] -
         f[0][0][2] - f[1][0][2] - f[2][0][2]);
    gdouble dz =
        (f[0][0][2] + f[1][0][2] + f[2][0][2] - f[0][0][0] - f[1][0][0] -
         f[2][0][0] + f[0][1][2] + f[1][1][2] + f[2][1][2] - f[0][1][0] -
         f[1][1][0] - f[2][1][0] + f[0][2][2] + f[1][2][2] + f[2][2][2] -
         f[0][2][0] - f[1][2][0] - f[2][2][0]);
...
static void variable_vof_concentration_read(GtsObject **o, GtsFile *fp) {
  (*GTS_OBJECT_CLASS(gfs_variable_vof_concentration_class())
...
static void variable_vof_concentration_write(GtsObject *o, FILE *fp) {
  ...
  fprintf(fp, " %s",
          GFS_VARIABLE(GFS_VARIABLE_VOF_CONCENTRATION(o)->vof)->name);
}
...
static gint children_half_height(FttCell *cell, FttDirection d, GfsVariable *fv,
  ...
  FttCellChildren child;
  guint           i,
      m  = ftt_cell_children_direction(cell, FTT_OPPOSITE_DIRECTION(d), &child);
  gint s = 0;
  for (i = 0; i < m; i++)
    if (child.c[i]) {
...
static void height_propagation(FttCell *cell, HFState *hf, GfsVariable *h,
                               gdouble orientation) {
  guint        level = ftt_cell_level(cell);
  FttDirection d;
        ...
        if (child.c[i]) {
          if (h == NULL)
            h = gfs_closest_height(child.c[i],
                                   GFS_VARIABLE_TRACER_VOF_HEIGHT(hf->f), hf->c,
                                   &orientation);
...
static void height_propagation_from_boundary(FttCell *cell, HFState *hf,
                                             GfsVariable *h) {
  guint        level       = ftt_cell_level(cell);
  ...
  while (cell && GFS_HAS_DATA(cell, h) &&
         GFS_VALUE(cell, h) > BOUNDARY_HIT / 2. &&
...
static void height_periodic_bc(FttCell *cell, HFState *hf) {
  ...
  if (h) {
    ...
    if (hn == h) {
       ...
       */
      gdouble orientation = (hf->d % 2 ? -1 : 1) * (h == hf->hb ? 1 : -1);
      gdouble Hn = GFS_VALUE(neighbor, h) + 0.5 + (orientation - 1.) / 2. -
                   2. * BOUNDARY_HIT;
      GFS_VALUE(cell, h) += Hn;
...
static gdouble contact_angle_bc(FttCell *cell, HFState *hf) {
  if (hf->angle) {
    ...
    return gfs_function_face_value(GFS_BC_VALUE(hf->angle)->val, &f) * M_PI /
           180.;
  } else
    return M_PI / 2.;
}
...
static void height_contact_normal_bc(FttCell *cell, HFState *hf) {
  ...
  if (h) {
    ...
    FttCell *    n1 = ftt_cell_neighbor(cell, nd),
            *n2     = ftt_cell_neighbor(cell, nd + 1);
    if (n1 && GFS_HAS_DATA(n1, h) && n2 && GFS_HAS_DATA(n2, h)) {
      ...
      if (!n1 || GFS_VALUE(n1, hf->f) != full_or_empty) {
        /* column is not neighbouring a full or empty cell => it is not a
         * contact line */
        /* the boundary is dry or wet i.e. the column height is well defined */
        /* reset the BOUNDARY_HIT flag on the whole column */
        GFS_VALUE(cell, h) -= BOUNDARY_HIT;
...
the non-interfacial neighbour */
          if (orientation * alpha > orientation * m.x)
            alpha = m.x;
          GFS_VALUE(n1, h) =
              ftt_cell_level(n1) == ftt_cell_level(cell)
                  ? orientation * ((alpha - m.x * 3. / 2.) / m.y - 0.5)
                  : /* neighbour at same level */
                  orientation * ((alpha - m.x * 2.) / m.y - 1.) /
                      2.; /* coarser neighbor */
...
static void contact_angle_height(FttCell *cell, GfsVariable *h, HFState *hf) {
  if (GFS_HAS_DATA(cell, h)) {
    ...
    if (theta == M_PI / 2.)
       ...
       * THETA_MIN), the contact angle will saturate at THETA_MIN = atan
       * (1./SLOPE_MAX).
       */
      gdouble cotantheta =
          (theta < THETA_MIN
               ? SLOPE_MAX
               : theta > M_PI - THETA_MIN ? -SLOPE_MAX : 1. / tan(theta));
...
static void box_periodic_bc(GfsBox *box, HFState *hf) {
  for (hf->d = 2 * hf->c; hf->d <= 2 * hf->c + 1; hf->d++)
    if (GFS_IS_BOUNDARY_PERIODIC(box->neighbor[hf->d]))
      ftt_cell_traverse_boundary(box->root, hf->d, FTT_PRE_ORDER,
                                 FTT_TRAVERSE_ALL, -1,
...
static void box_contact_bc(GfsBox *box, HFState *hf) {
  ...
  for (hf->d = 0; hf->d < FTT_NEIGHBORS; hf->d++)
    if (GFS_IS_BOUNDARY(box->neighbor[hf->d]) &&
      ...
      hf->angle =
          gfs_boundary_lookup_bc(GFS_BOUNDARY(box->neighbor[hf->d]), hf->f);
      if (!GFS_IS_BC_ANGLE(hf->angle))
        hf->angle = NULL; /* symmetry i.e. angle = PI/2 */
      FttCellTraverseFunc contact_bc = (FttCellTraverseFunc)(
          hf->d / 2 == hf->c ? height_contact_normal_bc
                             : height_contact_tangential_bc);
      ftt_cell_traverse_boundary(box->root, hf->d, FTT_POST_ORDER,
                                 FTT_TRAVERSE_ALL, -1, contact_bc, hf);
    }
}
...
static gboolean height_normal(FttCell *cell, GfsVariable *v, FttVector *m) {
  ...
  for (c = 0; c < 2; c++) {
    ...
    if (hv != NULL && fabs(GFS_VALUE(cell, hv)) <= 1.) {
      ...
      gdouble det = x[0] * x[1] * (x[0] - x[1]), a = x[1] * (h[0] - H),
              b  = x[0] * (h[1] - H);
      gdouble hx = (x[0] * b - x[1] * a) / det;
      if (fabs(hx) < slope) {
        slope                = fabs(hx);
...
static void variable_tracer_vof_height_update(GfsVariable *v,
                                              GfsDomain *  domain) {
  gfs_domain_cell_traverse(domain, FTT_POST_ORDER, FTT_TRAVERSE_NON_LEAFS, -1,
                           (FttCellTraverseFunc)v->fine_coarse, v);
  gfs_domain_bc(domain, FTT_TRAVERSE_ALL, -1, v);
  ...
  for (hf.c = 0; hf.c < FTT_DIMENSION; hf.c++) {
    ...
    gfs_domain_cell_traverse_condition(domain, FTT_PRE_ORDER, FTT_TRAVERSE_ALL,
                                       -1, (FttCellTraverseFunc)height, &hf,
                                       is_interfacial, hf.f);
    ...
    gfs_domain_cell_traverse(
        domain, FTT_PRE_ORDER, FTT_TRAVERSE_ALL, -1,
...
static void variable_tracer_vof_height_destroy(GtsObject *o) {
  ...
  (*GTS_OBJECT_CLASS(gfs_variable_tracer_vof_height_class())
...
static void variable_tracer_vof_height_read(GtsObject **o, GtsFile *fp) {
  ...
  for (c = 0; c < FTT_DIMENSION; c++) {
    ...
    gchar *      description =
        g_strdup_printf("%s-component (bottom) of the height function for the "
                        "interface defined by %s",
                        index[c], v->name);
    ...
    description = g_strdup_printf("%s-component (top) of the height function "
                                  "for the interface defined by %s",
                                  index[c], v->name);
...
static void variable_tracer_vof_height_class_init(GtsObjectClass *klass) {
  GFS_VARIABLE_TRACER_VOF_CLASS(klass)->update =
      variable_tracer_vof_height_update;
...
GfsVariableTracerVOFClass *gfs_variable_tracer_vof_height_class(void) {
  ...
  if (klass == NULL) {
    ...
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_variable_tracer_vof_class()), &info);
  }
 ...
 */
GfsVariable *gfs_closest_height(FttCell *cell, GfsVariableTracerVOFHeight *t,
