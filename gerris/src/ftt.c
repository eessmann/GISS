...
gchar *ftt_direction_name[FTT_NEIGHBORS] = {"right",
                                            "left",
                                            "top",
                                            "bottom"
#if (!FTT_2D)
...
static void oct_new(FttCell *parent, gboolean check_neighbors,
                    FttCellInitFunc init, gpointer data) {
  FttOct *oct;
  guint   n;
  ...
  for (i = 0; i < FTT_NEIGHBORS; i++)
    if (neighbor.c[i] && !FTT_CELL_IS_LEAF(neighbor.c[i]) &&
        ...
        neighbor.c[i]->children->neighbors.c[FTT_OPPOSITE_DIRECTION(i)] !=
            cell) {
      g_warning(
          "ftt_cell_check (%p): neighbor %d = %p: %d/%d", cell, i,
          neighbor.c[i]->children->neighbors.c[FTT_OPPOSITE_DIRECTION(i)],
 ...
 */
void ftt_cell_refine_single(FttCell *cell, FttCellInitFunc init,
                            gpointer init_data) {
 ...
 */
void ftt_cell_refine(FttCell *root, FttCellRefineFunc refine,
                     gpointer refine_data, FttCellInitFunc init,
                     gpointer init_data) {
  guint   n;
  FttOct *oct;
...
void ftt_cell_draw(const FttCell *cell, FILE *fp) {
  ...
  fprintf(fp,
          ...
          p.x + size, p.y - size, p.z + size, p.x + size, p.y + size,
          p.z + size, p.x - size, p.y + size, p.z + size);
  fputs("4 3 2 1 0\n"
        "4 4 5 6 7\n"
        "4 2 3 7 6\n"
        "4 0 1 5 4\n"
...
void ftt_face_draw(const FttCellFace *face, FILE *fp) {
  ...
  FttVector p;
#if FTT_2D
  static FttVector dp[FTT_NEIGHBORS][2] = {{{1., -1., 0.}, {1., 1., 0.}},
                                           {{-1., 1., 0.}, {-1., -1, 0.}},
                                           {{1., 1., 0.}, {-1., 1., 0.}},
                                           {{-1., -1., 0.}, {1., -1., 0.}}};
#else  /* FTT_3D */
  ...
  fprintf(fp, "VECT 1 2 0 2 0 %g %g 0 %g %g 0\n", p.x + dp[face->d][0].x * size,
          p.y + dp[face->d][0].y * size, p.x + dp[face->d][1].x * size,
  ...
  fprintf(fp,
          ...
          "%g %g %g "
          "4 0 1 2 3\n",
          p.x + dp[face->d][0].x * size, p.y + dp[face->d][0].y * size,
          p.z + dp[face->d][0].z * size, p.x + dp[face->d][1].x * size,
          p.y + dp[face->d][1].y * size, p.z + dp[face->d][1].z * size,
          p.x + dp[face->d][2].x * size, p.y + dp[face->d][2].y * size,
          p.z + dp[face->d][2].z * size, p.x + dp[face->d][3].x * size,
          p.y + dp[face->d][3].y * size, p.z + dp[face->d][3].z * size);
#endif /* FTT_3D */
}
 ...
 * the center of @cell.
 */
void ftt_corner_relative_pos(const FttCell *cell, FttDirection d[FTT_DIMENSION],
                             FttVector *pos) {
 ...
 * @d.
 */
void ftt_corner_pos(const FttCell *cell, FttDirection d[FTT_DIMENSION],
                    FttVector *pos) {
  gdouble   size;
  FttVector p;
...
static void update_neighbor(FttCell *cell, FttDirection d, FttCellInitFunc init,
                            gpointer init_data) {
  if (!FTT_CELL_IS_LEAF(cell)) {
    ...
    if (neighbor) {
      ...
      g_assert(oct->neighbors.c[d] == NULL || oct->neighbors.c[d] == neighbor);
 ...
 * trees.
 */
void ftt_cell_set_neighbor(FttCell *root, FttCell *neighbor, FttDirection d,
                           FttCellInitFunc init, gpointer init_data) {
  FttDirection od;
...
static void update_neighbor_match(FttCell *cell, FttDirection d,
                                  FttCellInitFunc init, gpointer init_data) {
  if (!FTT_CELL_IS_LEAF(cell)) {
    FttCell *neighbor = ftt_cell_neighbor_not_cached(cell, d);
 ...
 * trees.
 */
void ftt_cell_set_neighbor_match(FttCell *root, FttCell *neighbor,
                                 FttDirection d, FttCellInitFunc init,
                                 gpointer init_data) {
  FttDirection od;
...
static void cell_traverse_pre_order_all(FttCell *cell, gint max_depth,
...
static void cell_traverse_post_order_all(FttCell *cell, gint max_depth,
                                         FttCellTraverseFunc func,
                                         gpointer            data) {
  if (max_depth >= 0 && ftt_cell_level(cell) > max_depth)
    return;
...
static void cell_traverse_leafs(FttCell *cell, gint max_depth,
...
static void cell_traverse_pre_order_nonleafs(FttCell *cell, gint max_depth,
                                             FttCellTraverseFunc func,
                                             gpointer            data) {
  if (max_depth >= 0 && ftt_cell_level(cell) > max_depth)
    return;
...
static void cell_traverse_post_order_nonleafs(FttCell *cell, gint max_depth,
                                              FttCellTraverseFunc func,
                                              gpointer            data) {
  if (max_depth >= 0 && ftt_cell_level(cell) > max_depth)
    return;
...
static void cell_traverse_level(FttCell *cell, gint max_depth,
                                FttCellTraverseFunc func, gpointer data) {
...
static void cell_traverse_level_leafs(FttCell *cell, gint max_depth,
                                      FttCellTraverseFunc func, gpointer data) {
...
static void cell_traverse_level_non_leafs(FttCell *cell, gint max_depth,
                                          FttCellTraverseFunc func,
                                          gpointer            data) {
  if (ftt_cell_level(cell) == max_depth && !FTT_CELL_IS_LEAF(cell))
    (*func)(cell, data);
 ...
 */
void ftt_cell_traverse(FttCell *root, FttTraverseType order,
 ...
 * is not verified.
 */
void ftt_cell_traverse_condition(FttCell *root, FttTraverseType order,
                                 FttTraverseFlags flags, gint max_depth,
                                 FttCellTraverseFunc func, gpointer data,
  ...
  if (!FTT_CELL_IS_LEAF(root)) {
    ...
    for (n = 0; n < FTT_CELLS; n++) {
      ...
      if (!FTT_CELL_IS_DESTROYED(c))
        ftt_cell_traverse_condition(c, order, flags, max_depth, func, data,
                                    condition, cdata);
    }
  }
 ...
 */
void ftt_cell_traverse_box(FttCell *root, GtsBBox *box, FttTraverseType order,
                           FttTraverseFlags flags, gint max_depth,
                           FttCellTraverseFunc func, gpointer data) {
  g_return_if_fail(root != NULL);
  ...
  ftt_cell_traverse_condition(root, order, flags, max_depth, func, data,
                              cell_is_in_box, box);
}

static void cell_traverse_boundary_pre_order_all(FttCell *cell, FttDirection d,
                                                 gint                max_depth,
                                                 FttCellTraverseFunc func,
                                                 gpointer            data) {
  FttCell *parent;
  ...
  if (!FTT_CELL_IS_LEAF(cell)) {
    ...
    for (i = 0; i < n; i++)
      if (children.c[i])
        cell_traverse_boundary_pre_order_all(children.c[i], d, max_depth, func,
                                             data);
  }
}

static void cell_traverse_boundary_post_order_all(FttCell *cell, FttDirection d,
                                                  gint                max_depth,
                                                  FttCellTraverseFunc func,
                                                  gpointer            data) {
  if (max_depth >= 0 && ftt_cell_level(cell) > max_depth)
  ...
  if (!FTT_CELL_IS_LEAF(cell)) {
    ...
    for (i = 0; i < n; i++)
      if (children.c[i])
        cell_traverse_boundary_post_order_all(children.c[i], d, max_depth, func,
                                              data);
  }
...
static void cell_traverse_boundary_leafs(FttCell *cell, FttDirection d,
                                         gint                max_depth,
                                         FttCellTraverseFunc func,
                                         gpointer            data) {
  if (max_depth >= 0 && ftt_cell_level(cell) > max_depth)
  ...
  if (FTT_CELL_IS_LEAF(cell))
    ...
    for (i = 0; i < n; i++)
      if (children.c[i])
        cell_traverse_boundary_leafs(children.c[i], d, max_depth, func, data);
  }
}
...
static void cell_traverse_boundary_pre_order_nonleafs(FttCell *    cell,
  ...
  if (!FTT_CELL_IS_LEAF(cell)) {
    ...
    if (!FTT_CELL_IS_LEAF(cell)) {
      ...
      for (i = 0; i < n; i++)
        if (children.c[i])
          cell_traverse_boundary_pre_order_nonleafs(children.c[i], d, max_depth,
                                                    func, data);
    }
  }
}
...
static void cell_traverse_boundary_post_order_nonleafs(FttCell *    cell,
  ...
  if (!FTT_CELL_IS_LEAF(cell)) {
    ...
    for (i = 0; i < n; i++)
      if (children.c[i])
        cell_traverse_boundary_post_order_nonleafs(children.c[i], d, max_depth,
                                                   func, data);
    (*func)(cell, data);
  }
}

static void cell_traverse_boundary_level(FttCell *cell, FttDirection d,
                                         gint                max_depth,
                                         FttCellTraverseFunc func,
                                         gpointer            data) {
  if (ftt_cell_level(cell) == max_depth)
    ...
    for (i = 0; i < n; i++)
      if (children.c[i])
        cell_traverse_boundary_level(children.c[i], d, max_depth, func, data);
  }
}

static void cell_traverse_boundary_level_leafs(FttCell *cell, FttDirection d,
                                               gint                max_depth,
                                               FttCellTraverseFunc func,
                                               gpointer            data) {
  if (ftt_cell_level(cell) == max_depth || FTT_CELL_IS_LEAF(cell))
    ...
    for (i = 0; i < n; i++)
      if (children.c[i])
        cell_traverse_boundary_level_leafs(children.c[i], d, max_depth, func,
                                           data);
  }
}
...
static void cell_traverse_boundary_level_non_leafs(FttCell *    cell,
  ...
  if (ftt_cell_level(cell) == max_depth && !FTT_CELL_IS_LEAF(cell))
    ...
    for (i = 0; i < n; i++)
      if (children.c[i])
        cell_traverse_boundary_level_non_leafs(children.c[i], d, max_depth,
                                               func, data);
  }
}
 ...
 * visited.
 */
void ftt_cell_traverse_boundary(FttCell *root, FttDirection d,
  ...
  if (flags == FTT_TRAVERSE_ALL) {
    ...
    if (order == FTT_PRE_ORDER)
      cell_traverse_boundary_pre_order_nonleafs(root, d, max_depth, func, data);
    else
      cell_traverse_boundary_post_order_nonleafs(root, d, max_depth, func,
                                                 data);
  }
}

static void oct_destroy(FttOct *oct, FttCellCleanupFunc cleanup,
                        gpointer data) {
  guint n;
 ...
 */
void ftt_cell_destroy(FttCell *cell, FttCellCleanupFunc cleanup,
                      gpointer data) {
  FttCellNeighbors neighbor;
  guint            i, level;
 ...
 */
void ftt_cell_destroy_root(FttCell *root, FttCellChildren *children,
 ...
 */
void ftt_cell_flatten(FttCell *root, FttDirection d, FttCellCleanupFunc cleanup,
                      gpointer data) {
  g_return_if_fail(root != NULL);
  g_return_if_fail(d < FTT_NEIGHBORS);
...
void ftt_cell_point_distance2_internal(
    FttCell *root, GtsPoint *p, gdouble d,
    gdouble (*distance2)(FttCell *, GtsPoint *, gpointer), gpointer data,
    FttCell **closest, gdouble *dmin) {
  if (FTT_CELL_IS_LEAF(root)) {
    if (d < *dmin) {
      *dmin = d;
      if (closest)
    ...
    for (i = 0; i < FTT_CELLS; i++)
      if (dc[i] < *dmin)
        ftt_cell_point_distance2_internal(child.c[i], p, dc[i], distance2, data,
                                          closest, dmin);
  }
}
 ...
 */
gdouble ftt_cell_point_distance2(FttCell *root, GtsPoint *p,
                                 gdouble (*distance2)(FttCell *, GtsPoint *,
                                                      gpointer),
  ...
  if (d < dmin)
    ftt_cell_point_distance2_internal(root, p, d, distance2, data, closest,
                                      &dmin);
  return dmin;
}
 ...
 * with each cell.
 */
void ftt_cell_write(const FttCell *root, gint max_depth, FILE *fp,
                    FttCellWriteFunc write, gpointer data) {
  guint flags;
 ...
 * with each cell.
 */
void ftt_cell_write_binary(const FttCell *root, gint max_depth, FILE *fp,
                           FttCellWriteFunc write, gpointer data) {
  guint flags;
...
static gboolean oct_read(FttCell *parent, GtsFile *fp, FttCellReadFunc read,
                         gpointer data);

static gboolean cell_read(FttCell *cell, GtsFile *fp, FttCellReadFunc read,
                          gpointer data) {
  guint flags;
...
static gboolean oct_read_binary(FttCell *parent, GtsFile *fp,
                                FttCellReadFunc read, gpointer data);

static gboolean cell_read_binary(FttCell *cell, GtsFile *fp,
                                 FttCellReadFunc read, gpointer data) {
  guint flags;
 ...
 * read the extra user data associated with each cell.
 */
FttCell *ftt_cell_read_binary(GtsFile *fp, FttCellReadFunc read,
                              gpointer data) {
  FttCell *root;
  guint    l, depth;
      ...
      for (j = 0; j < k; j++) {
        ...
        if (c) {
          ...
          if (!FTT_CELL_IS_LEAF(c)) {
            ...
            k = ftt_cell_children_direction(c, FTT_OPPOSITE_DIRECTION(i),
                                            &child);
            for (j = 0; j < k; j++)
              if (child.c[j])
                return TRUE;
...
static void copy_cell(const FttCell *from, FttCell *to, FttCellCopyFunc copy,
                      gpointer data) {
  to->flags = from->flags;
 ...
 * function.
 */
FttCell *ftt_cell_copy(const FttCell *root, FttCellCopyFunc copy,
                       gpointer data) {
  FttCell *root_copy;
 ...
 * face is never %NULL.
 */
void ftt_face_traverse(FttCell *root, FttComponent c, FttTraverseType order,
                       FttTraverseFlags flags, gint max_depth,
                       FttFaceTraverseFunc func, gpointer data) {
  FttDirection d;
  gpointer     datum[6];
  ...
  if (c == FTT_XYZ) {
    if (boundary_faces) {
      ...
      ftt_cell_traverse(root, order, flags, max_depth,
                        (FttCellTraverseFunc)traverse_all_direct_faces, datum);
      check    = TRUE;
      datum[0] = &d;
    ...
    if (boundary_faces) {
      ...
      datum[0] = &c;
      ftt_cell_traverse(root, order, flags, max_depth,
                        (FttCellTraverseFunc)traverse_face_component, datum);
    } else {
      d        = 2 * c;
      datum[0] = &d;
      ftt_cell_traverse(root, order, flags, max_depth,
 ...
 */
void ftt_face_traverse_boundary(FttCell *root, FttDirection d,
                                FttTraverseType order, FttTraverseFlags flags,
 ...
 */
gboolean ftt_cell_coarsen(FttCell *root, FttCellCoarsenFunc coarsen,
                          gpointer coarsen_data, FttCellCleanupFunc cleanup,
  ...
  for (i = 0; i < FTT_CELLS; i++)
    if (!FTT_CELL_IS_DESTROYED(&(root->children->cell[i])))
      coarsenable &= ftt_cell_coarsen(&(root->children->cell[i]), coarsen,
                                      coarsen_data, cleanup, cleanup_data);
    ...
    for (d = 0; d < FTT_NEIGHBORS; d++) {
      ...
      for (i = 0; i < n; i++) {
        ...
        if (child.c[i] && (neighbor = ftt_cell_neighbor(child.c[i], d)) &&
          ...
          k = ftt_cell_children_direction(neighbor, FTT_OPPOSITE_DIRECTION(d),
                                          &child1);
          for (j = 0; j < k && empty; j++)
            if (child1.c[j])
              empty = FALSE;
 ...
 */
FttCellTraverse *ftt_cell_traverse_new(FttCell *root, FttTraverseType order,
                                       FttTraverseFlags flags, gint max_depth) {
  FttCellTraverse *t;
  GPtrArray *      a;
