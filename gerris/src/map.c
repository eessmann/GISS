...
static void not_implemented(GfsMap *map, const FttVector *src,
                            FttVector *dest) {
  g_assert_not_implemented();
}

static double evaluate(GfsMap *map, const FttVector *x, const FttVector *rhs,
                       FttVector *f) {
  gdouble delta = 0.;
  (*map->inverse)(map, x, f);
  int i;
...
static void jacobian(GfsMap *map, const FttVector *x, const FttVector *rhs,
                     FttVector *f, GtsMatrix *J) {
  int i, j;
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++) {
      FttVector df, dx = *x;
...
static void map_function_inverse(GfsMap *map, const FttVector *src,
                                 FttVector *dest) {
...
static void gfs_map_transform_read(GtsObject **o, GtsFile *fp) {
  ...
  GtsFileVariable  var[] = {{GTS_DOUBLE, "tx", TRUE, &map->translate[0]},
...
static void gfs_map_transform_write(GtsObject *o, FILE *fp) {
  ...
  if (gts_vector_norm(map->translate) > 0.)
    fprintf(fp, "  tx = %g ty = %g tz = %g\n", map->translate[0],
            map->translate[1], map->translate[2]);
  if (gts_vector_norm(map->rotate) > 0.)
    fprintf(fp, "  rx = %g ry = %g rz = %g\n", map->rotate[0], map->rotate[1],
            map->rotate[2]);
  fputc('}', fp);
}
...
static void map_transform_transform(GfsMap *map, const FttVector *src,
                                    FttVector *dest) {
  GtsPoint p;
  p.x = src->x;
  p.y = src->y;
...
static void map_transform_inverse(GfsMap *map, const FttVector *src,
                                  FttVector *dest) {
  GtsPoint p;
  p.x = src->x;
  p.y = src->y;
