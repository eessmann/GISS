...
static void gfs_variable_read(GtsObject **o, GtsFile *fp) {
  ...
  domain = (*o)->reserved;
  if (gfs_derived_variable_from_name(domain->derived_variables,
                                     fp->token->str)) {
 ...
 */
GfsVariable *gfs_variable_new(GfsVariableClass *klass, GfsDomain *domain,
...
GfsVariable *gfs_variable_from_name(GSList *i, const gchar *name) {
  ...
  while (i && (!GFS_VARIABLE(i->data)->name ||
               strcmp(name, GFS_VARIABLE(i->data)->name)))
    i = i->next;
...
void gfs_variable_set_tensor(GfsVariable *t[2][2]) {
  ...
  /* for rotated boundary conditions i.e. gfs_boundary_periodic_rotate() */
  t[0][0]->component   = 0;
  t[0][0]->vector[0]   = t[0][0];
  t[0][0]->vector[1]   = t[1][1];
  t[0][0]->orientation = +1.;
  t[0][0]->even        = TRUE;
  t[1][1]->component   = 1;
  t[1][1]->vector[0]   = t[0][0];
  t[1][1]->vector[1]   = t[1][1];
  t[1][1]->orientation = +1.;
  t[1][1]->even        = TRUE;
  t[0][1]->component   = 0;
  t[0][1]->vector[0]   = t[0][1];
  t[0][1]->vector[1]   = t[1][0];
  t[0][1]->orientation = -1.;
  t[0][1]->even        = TRUE;
  t[1][0]->component   = 1;
  t[1][0]->vector[0]   = t[0][1];
  t[1][0]->vector[1]   = t[1][0];
  t[1][0]->orientation = -1.;
  t[1][0]->even        = TRUE;
}
...
static void variable_tracer_read(GtsObject **o, GtsFile *fp) {
  ...
  if (fp->type != GTS_ERROR && fp->type == '{')
    g_warning(
        "%d:%d: specifying diffusion parameters is not done here anymore!",
        fp->line, fp->pos);
}
...
static gboolean variable_residual_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(gfs_variable_class())->event)(event, sim)) {
    gfs_domain_cell_traverse(GFS_DOMAIN(sim), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             -1, (FttCellTraverseFunc)scale_residual, event);
    return TRUE;
  }
  return FALSE;
}
...
static void variable_filtered_read(GtsObject **o, GtsFile *fp) {
  ...
  GFS_VARIABLE(v)->description =
      g_strjoin(" ", "Variable", v->v->name, "filtered", NULL);

  GFS_VARIABLE(v)->units = v->v->units;
}

static void variable_filtered_write(GtsObject *o, FILE *fp) {
  (*GTS_OBJECT_CLASS(gfs_variable_filtered_class())->parent_class->write)(o,
                                                                          fp);

  fprintf(fp, " %s %d", GFS_VARIABLE_FILTERED(o)->v->name,
          GFS_VARIABLE_FILTERED(o)->niter);
}
...
static gboolean variable_filtered_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_variable_filtered_class())->parent_class)
            ->event)(event, sim)) {
...
static void unity(FttCell *cell, GfsVariable *v) { GFS_VALUE(cell, v) = 1.; }
...
static gboolean variable_diagonal_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_variable_diagonal_class())->parent_class)
            ->event)(event, sim)) {
    GfsDomain *  domain = GFS_DOMAIN(sim);
...
static void variable_function_write(GtsObject *o, FILE *fp) {
  (*GTS_OBJECT_CLASS(gfs_variable_function_class())->parent_class->write)(o,
                                                                          fp);
...
static gdouble face_metric_inverse(FttCell *cell, FttDirection d,
                                   GfsDomain *domain) {
...
static void init_mac_from_stream_function(FttCell *cell, gdouble psi0,
                                          gdouble psi1, gdouble psi2,
                                          gdouble psi3, gdouble h,
                                          GfsDomain *domain, GfsVariable **u) {
  GFS_STATE(cell)->f[0].un =
      (psi2 - psi1) * face_metric_inverse(cell, 0, domain) / h;
  GFS_STATE(cell)->f[1].un =
      (psi3 - psi0) * face_metric_inverse(cell, 1, domain) / h;
  GFS_STATE(cell)->f[2].un =
      (psi3 - psi2) * face_metric_inverse(cell, 2, domain) / h;
  GFS_STATE(cell)->f[3].un =
      (psi0 - psi1) * face_metric_inverse(cell, 3, domain) / h;

  GFS_VALUE(cell, u[0]) =
      (GFS_STATE(cell)->f[0].un + GFS_STATE(cell)->f[1].un) / 2.;
  GFS_VALUE(cell, u[1]) =
      (GFS_STATE(cell)->f[2].un + GFS_STATE(cell)->f[3].un) / 2.;
}

static void variable_stream_function_coarse_fine(FttCell *    parent,
                                                 GfsVariable *v) {
  ...
  init_mac_from_stream_function(child.c[0], psi7, psi8, psi6, psi3, h,
                                v->domain, u);
  init_mac_from_stream_function(child.c[1], psi8, psi5, psi2, psi6, h,
                                v->domain, u);
  init_mac_from_stream_function(child.c[2], psi0, psi4, psi8, psi7, h,
                                v->domain, u);
  init_mac_from_stream_function(child.c[3], psi4, psi1, psi5, psi8, h,
                                v->domain, u);
  guint n;
  for (n = 0; n < FTT_CELLS; n++) {
...
static void variable_stream_function_fine_coarse(FttCell *    cell,
                                                 GfsVariable *v) {
  FttCellChildren child;
  ...
  GFS_VALUE(cell, v) = (GFS_VALUE(child.c[0], v) + GFS_VALUE(child.c[1], v) +
                        GFS_VALUE(child.c[2], v) + GFS_VALUE(child.c[3], v)) /
                       4.;
}
...
static void init_streamfunction(FttCell *cell, GfsVariable *v) {
  ...
  p.y          = o.y + h;
  gdouble psi3 = gfs_function_spatial_value(f, &p);
  init_mac_from_stream_function(cell, psi0, psi1, psi2, psi3, 2. * h, v->domain,
...
static void init_streamfunction_from_variable(FttCell *cell, GfsVariable *v) {
  ...
  init_mac_from_stream_function(cell, psi0, psi1, psi2, psi3,
                                ftt_cell_size(cell), v->domain,
                                gfs_domain_velocity(v->domain));
}

static gboolean variable_stream_function_event(GfsEvent *     event,
                                               GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(gfs_variable_function_class())->event)(event, sim)) {
    ...
    if (v)
      gfs_domain_traverse_leaves(
          domain, (FttCellTraverseFunc)init_streamfunction_from_variable, v);
    else
      gfs_domain_traverse_leaves(
          domain, (FttCellTraverseFunc)init_streamfunction, event);
...
GfsVariableClass *gfs_variable_stream_function_class(void) {
  ...
  if (klass == NULL) {
    GtsObjectClassInfo gfs_variable_stream_function_info = {
        ...
        (GtsArgGetFunc)NULL};
    klass =
        gts_object_class_new(GTS_OBJECT_CLASS(gfs_variable_function_class()),
...
static gboolean variable_poisson_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(gfs_variable_class())->event)(event, sim)) {
    ...
    if (pv->par.residual.infty > pv->par.tolerance) {
      g_warning("VariablePoisson %s: max residual %g > %g", v->name,
                pv->par.residual.infty, pv->par.tolerance);
...
static void laplacian(FttCell *cell, LapData *p) {
  ...
  GFS_VALUE(cell, p->lap) =
      (g.b - g.a * GFS_VALUE(cell, p->u)) /
...
static gboolean variable_laplacian_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(gfs_variable_class())->event)(event, sim)) {
    ...
    if (p.u == NULL) {
      ...
      gfs_domain_traverse_leaves(domain, (FttCellTraverseFunc)evaluate_function,
                                 &p);
...
GfsVariableClass *gfs_variable_laplacian_class(void) {
  ...
  if (klass == NULL) {
    ...
    klass = gts_object_class_new(
        GTS_OBJECT_CLASS(gfs_variable_function_class()), &info);
  }
...
static gboolean variable_age_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_variable_age_class())->parent_class)
            ->event)(event, sim)) {
    gfs_domain_cell_traverse(GFS_DOMAIN(sim), FTT_PRE_ORDER, FTT_TRAVERSE_ALL,
                             -1, (FttCellTraverseFunc)increment_age, event);
    return TRUE;
  }
  return FALSE;
}
...
#else
          if (gts_fifo_top(fifo) ==
              NULL) { /* only consider the last fine cell */
            FttDirection od = FTT_OPPOSITE_DIRECTION(d);
            double       dp = GFS_STATE(neighbor)->f[od].un *
                        ftt_cell_size(neighbor) / GFS_STATE(neighbor)->f[od].v;
            double          p = 0.;
            FttCellChildren child;
            int i, n = ftt_cell_children_direction(ftt_cell_parent(cell), d,
                                                   &child);
            for (i = 0; i < n; i++)
...
#else
        FttCellChildren child;
        int i, n = ftt_cell_children_direction(
                   neighbor, FTT_OPPOSITE_DIRECTION(d), &child);
        double dp = GFS_STATE(cell)->f[d].un * ftt_cell_size(cell) /
                    GFS_STATE(cell)->f[d].v;
...
static gboolean variable_average_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_variable_function_class())->parent_class)
            ->event)(event, sim)) {
    GfsVariable *       v  = GFS_VARIABLE(event);
    GfsVariableAverage *av = GFS_VARIABLE_AVERAGE(v);
    gfs_domain_cell_traverse_boundary(v->domain, 2 * av->c, FTT_PRE_ORDER,
...
static void hydrostatic_pressure_read(GtsObject **o, GtsFile *fp) {
  (*GTS_OBJECT_CLASS(gfs_hydrostatic_pressure_class())->parent_class->read)(o,
                                                                            fp);
  if (fp->type == GTS_ERROR)
    return;
...
static gboolean hydrostatic_pressure_event(GfsEvent *     event,
                                           GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(gfs_variable_class())->event)(event, sim)) {
    gfs_hydrostatic_pressure_update(GFS_HYDROSTATIC_PRESSURE(event),
...
void gfs_hydrostatic_pressure_update(GfsHydrostaticPressure *p,
  ...
  gfs_domain_face_traverse(
      v->domain, FTT_XYZ, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
      (FttFaceTraverseFunc)gfs_face_reset_normal_velocity, NULL);
  gfs_velocity_face_sources(v->domain, gfs_domain_velocity(v->domain), 1., NULL,
                            NULL);
  gfs_poisson_coefficients(v->domain, alpha, TRUE, TRUE, TRUE);
  gfs_domain_cell_traverse_boundary(
      v->domain, 2 * p->c, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
...
static void gfs_constant_destroy(GtsObject *object) {
  if (GFS_CONSTANT(object)->derived)
    gfs_domain_remove_derived_variable(
        GFS_DOMAIN(gfs_object_simulation(object)),
        GFS_CONSTANT(object)->derived->name);

  (*GTS_OBJECT_CLASS(gfs_constant_class())->parent_class->destroy)(object);
}

static gdouble constant_func(FttCell *cell, FttCellFace *face,
                             GfsDomain *domain, GfsConstant *c) {
  return c->val;
}
...
static void gfs_constant_read(GtsObject **o, GtsFile *fp) {
  ...
  GfsDerivedVariableInfo v = {fp->token->str, "Constant", constant_func, *o};
...
static void gfs_derived_variable_destroy(GtsObject *object) {
  ...
  (*GTS_OBJECT_CLASS(gfs_derived_variable_class())->parent_class->destroy)(
      object);
}
