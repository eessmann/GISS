...
static void group_velocity(int ik, int ith, FttVector *u, guint ntheta,
                           gdouble g) {
...
static gdouble cell_E(FttCell *cell, FttCellFace *face, GfsDomain *domain) {
  ...
  guint          ik, ith;
  gdouble        E = 0., sigma = 2. * M_PI * GFS_WAVE_F0,
          sgamma = (GFS_WAVE_GAMMA - 1. / GFS_WAVE_GAMMA) / 2.;
...
static void compute_gradient(FttCell *cell, GSEData *p) {
  GFS_VALUE(cell, p->dF) =
      gfs_center_regular_gradient(cell, p->dF->component, p->Fn);
}
...
static void diffusion(FttCell *cell, GSEData *p) {
  ...
  for (j = 0; j < 2; j++)
    if (j != p->dF->component)
      GFS_VALUE(cell, p->F) += p->D[j][p->dF->component] *
                               gfs_center_regular_gradient(cell, j, p->dF) / h2;
  /* diagonal */
  GFS_VALUE(cell, p->F) +=
...
static void wave_run(GfsSimulation *sim) {
  ...
  while (sim->time.t < sim->time.end && sim->time.i < sim->time.iend) {
    ...
    for (ik = 0; ik < wave->nk; ik++) {
      ...
      gfs_domain_face_traverse(domain, FTT_XYZ, FTT_PRE_ORDER,
      ...
      if (wave->alpha_s > 0.) {
        ...
        sim->advection_params.cfl =
            MIN(cfl, 2. / (4. * wave->alpha_s * M_PI / wave->ntheta));
        /* fixme: this should be:
           sim->advection_params.cfl = MIN (cfl,
           sqrt(3.)/(wave->alpha_s*2.*M_PI/wave->ntheta));
        */
        gfs_simulation_set_timestep(sim);
      ...
      g_assert(fabs(sim->time.t + sim->advection_params.dt * n - tnext) <
               1e-12);
      while (n--) {
        for (ith = 0; ith < wave->ntheta; ith++) {
          FttVector cg;
...
static void wave_read(GtsObject **o, GtsFile *fp) {
  ...
  if (fp->type == '{') {
    GtsFileVariable var[] = {{GTS_UINT, "nk", TRUE, &wave->nk},
  ...
  for (ik = 0; ik < wave->nk; ik++)
    for (ith = 0; ith < wave->ntheta; ith++) {
      ...
      gchar *description = g_strdup_printf(
          "Action density for f = %g Hz and theta = %g degrees", frequency(ik),
          theta(ith, wave->ntheta) * 180. / M_PI);
      wave->F[ik][ith] =
...
static gdouble cell_frequency(FttCell *cell, FttCellFace *face,
                              GfsDomain *domain) {
  return frequency(GFS_WAVE(domain)->ik);
}

static gdouble cell_direction(FttCell *cell, FttCellFace *face,
                              GfsDomain *domain) {
...
static void gfs_init_wave_read(GtsObject **o, GtsFile *fp) {
  ...
  if (!GFS_IS_WAVE(domain)) {
    gts_file_error(fp,
                   "GfsInitWave can only be used within a GfsWave simulation");
    return;
  }
...
static void init_energy(FttCell *cell, GfsInitWave *event) {
  ...
  for (wave->ik = 0; wave->ik < wave->nk; wave->ik++)
    for (wave->ith = 0; wave->ith < wave->ntheta; wave->ith++)
      GFS_VALUE(cell, wave->F[wave->ik][wave->ith]) =
          gfs_function_value(event->d, cell);
}
...
static gboolean gfs_init_wave_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_init_wave_class())->parent_class)
            ->event)(event, sim)) {
    gfs_catch_floating_point_exceptions();
    gfs_domain_cell_traverse(GFS_DOMAIN(sim), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             -1, (FttCellTraverseFunc)init_energy, event);
    gfs_restore_fpe_for_function(GFS_INIT_WAVE(event)->d);
    gfs_catch_floating_point_exceptions();
    gfs_domain_cell_traverse(GFS_DOMAIN(sim), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             -1, (FttCellTraverseFunc)scale_energy, event);
