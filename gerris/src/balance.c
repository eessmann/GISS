...
static void count(FttCell *cell, int *n) { (*n)++; }
...
static BalancingFlow *balancing_flow_new(GfsDomain *domain, int average) {
  ...
  gdouble *lambda      = g_malloc(sizeof(gdouble) * (pe->len + 1)), lambda1,
          eps          = G_MAXDOUBLE;
  ...
  while (niter-- && eps > tolerance) {
    ...
    for (i = 0; i < pe->len; i++)
      MPI_Isend(&(lambda[0]), 1, MPI_DOUBLE, g_array_index(pe, guint, i),
                domain->pid, MPI_COMM_WORLD, &(request[i]));
    ...
    for (i = 0; i < pe->len; i++) {
      MPI_Recv(&(lambda[i + 1]), 1, MPI_DOUBLE, g_array_index(pe, guint, i),
               g_array_index(pe, guint, i), MPI_COMM_WORLD, &status);
      rhs += lambda[i + 1];
    }
  ...
  if (niter < 0 && domain->pid == 0)
    g_warning("balancing_flow(): could not converge after %d iterations",
              NITERMAX);
...
static void reset_box_size(GfsBox *box) { box->size = 0; }
...
static void gfs_event_balance_write(GtsObject *o, FILE *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_event_balance_class())->parent_class->write)
    (*GTS_OBJECT_CLASS(gfs_event_balance_class())->parent_class->write)(o, fp);
...
static void gfs_event_balance_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_event_balance_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_event_balance_class())->parent_class->read)(o, fp);
...
static gboolean gfs_event_balance_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(
            GTS_OBJECT_CLASS(gfs_event_balance_class())->parent_class)
            ->event)(event, sim)) {
    ...
    if (size.max / size.min > 1. + s->max) {
      ...
      int            i;
      /* Send boxes */
      gts_container_foreach(GTS_CONTAINER(domain), (GtsFunc)reset_box_size,
                            NULL);
        ...
        if (balance->flow[i] > 0.) { /* largest subdomain */
          ...
          while (b.flow > 0 && nb > 1) {
            ...
            gts_container_foreach(GTS_CONTAINER(domain),
                                  (GtsFunc)select_neighbouring_box, &b);
...
#if MPI_VERSION == 2
        MPI_Allreduce(MPI_IN_PLACE, pid->data, nb, MPI_UNSIGNED, MPI_MAX,
                      MPI_COMM_WORLD);
#else  /* MPI-1 does not have the MPI_IN_PLACE option */
        GArray *recv = g_array_new(FALSE, TRUE, sizeof(guint));
        g_array_set_size(recv, nb);
        MPI_Allreduce(pid->data, recv->data, nb, MPI_UNSIGNED, MPI_MAX,
                      MPI_COMM_WORLD);
        g_array_free(pid, TRUE);
        pid = recv;
#endif /* MPI-1 */
        /* pid[id] now contains the current pid of box with index id */
        gts_container_foreach(GTS_CONTAINER(domain), (GtsFunc)update_box_pid,
                              pid);
        g_array_free(pid, TRUE);
        gfs_domain_reshape(domain, gfs_domain_depth(domain));
        /* applies BCs again in case a BC on one variable depends on another
         * variable */
