...
static void correct_normal_velocity(FttCellFace *face, gpointer *data) {
  GfsGradient   g;
  gdouble       dp;
  FttFaceType   type;
  ...
  if (ftt_face_type(face) == FTT_FINE_COARSE)
    dp *= GFS_FACE_FRACTION_LEFT(face) /
          (GFS_FACE_FRACTION_RIGHT(face) * FTT_CELLS / 2);
...
static void scale_gradients(FttCell *cell, gpointer *data) {
  ...
  for (c = 0; c < *dimension; c++) {
    ...
    if (c1 && c2 && !GFS_CELL_IS_GRADIENT_BOUNDARY(c1) &&
        !GFS_CELL_IS_GRADIENT_BOUNDARY(c2))
 ...
 *
 * Also allocates the @g variables and fills them with the centered gradient of
 * @p.
 */
static void gfs_correct_normal_velocities_weighted(GfsDomain *   domain,
  ...
  if (weighted) {
    ...
    gfs_domain_face_traverse(domain, dimension == 2 ? FTT_XY : FTT_XYZ,
                             ...
                             (FttFaceTraverseFunc)correct_normal_velocity,
                             data);
    data[0] = g;
    data[1] = &dimension;
...
static void scale_divergence_helmoltz(FttCell *cell, FreeSurfaceParams *p) {
  ...
  GFS_VALUE(cell, p->div) =
      2. * GFS_VALUE(cell, p->div) / p->dt - c * GFS_VALUE(cell, p->pn);
}
...
static void gfs_free_surface_pressure(GfsDomain *          toplayer,
                                      ...
                                      GfsVariable *div, GfsVariable *divn,
                                      GfsVariable *res, gdouble G) {
...
#if !FTT_2D
  gfs_domain_traverse_merged(toplayer, (GfsMergedTraverseFunc)merge_pressures,
                             p);
#endif
...
static void normal_velocities(GfsDomain *domain, GfsVariable **u) {
  gfs_domain_face_traverse(
      domain, FTT_XY, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
...
static void ocean_run(GfsSimulation *sim) {
  ...
  GtsFile *fp =
      gts_file_new_from_string("(H + P)"); /* fixme: should be H + P/g */
  ...
  while (sim->time.t < sim->time.end && sim->time.i < sim->time.iend) {
    ...
    gfs_domain_traverse_leaves(
        domain, (FttCellTraverseFunc)gfs_normal_divergence_2D, div);
    ...
    gfs_correct_normal_velocities_weighted(
        domain, 2, p, g, 0., sim->approx_projection_params.weighted);
    gfs_centered_velocity_advection_diffusion(domain, 2, &sim->advection_params,
                                              g, g, sim->physical_params.alpha);
    gfs_source_coriolis_implicit(domain, sim->advection_params.dt);
    gfs_correct_centered_velocities(domain, 2, g,
    ...
    normal_velocities(domain, gfs_domain_velocity(domain));
    gfs_domain_traverse_leaves(
        domain, (FttCellTraverseFunc)gfs_normal_divergence_2D, divn);
    gfs_poisson_coefficients(domain, fH, TRUE, TRUE, TRUE);
    gfs_free_surface_pressure(domain, &sim->approx_projection_params,
                              &sim->advection_params, p, divn, div, res,
                              sim->physical_params.g);
    gts_object_destroy(GTS_OBJECT(divn));
    gfs_correct_normal_velocities_weighted(
        domain, 2, p, g, sim->advection_params.dt / 2.,
...
static void ocean_read(GtsObject **object, GtsFile *fp) {
  ...
  sim->physical_params.g /=
      sim->physical_params.L /* *GFS_DOMAIN (sim)->lambda.z*/;
...
static void new_layer(GfsOcean *ocean) {
  ...
  GfsDomain *d =
      GFS_DOMAIN(gts_object_new(GTS_OBJECT_CLASS(gfs_domain_class())));
...
static void add_layer(GfsBox *box, GfsDomain *domain) {
  if (box->neighbor[FTT_FRONT] == NULL ||
      GFS_IS_BOUNDARY(box->neighbor[FTT_FRONT])) {
    GPtrArray *layer = GFS_OCEAN(domain)->layer;
    GtsObject *n;
    guint l = 0;

    gts_container_add(GTS_CONTAINER(g_ptr_array_index(layer, l++)),
                      GTS_CONTAINEE(box));
...
static void ocean_post_read(GfsDomain *domain, GtsFile *fp) {
  (*GFS_DOMAIN_CLASS(GTS_OBJECT_CLASS(gfs_ocean_class())->parent_class)
        ->post_read)(domain, fp);
...
static void compute_w(FttCell *c, GfsVariable *W) {
  ...
  while (c) {
    ...
    s->f[FTT_BACK].un = w;
    wf += (s->f[FTT_LEFT].v * s->f[FTT_LEFT].un -
           s->f[FTT_RIGHT].v * s->f[FTT_RIGHT].un +
           s->f[FTT_BOTTOM].v * s->f[FTT_BOTTOM].un -
           s->f[FTT_TOP].v * s->f[FTT_TOP].un);
    if (GFS_IS_MIXED(c))
      s->f[FTT_FRONT].un = w = GFS_STATE(c)->solid->s[FTT_FRONT] > 0.
                                   ? wf / GFS_STATE(c)->solid->s[FTT_FRONT]
                                   : 0.;
    else
...
static void compute_div(FttCell *c, GfsVariable *W) {
  ...
  while (c) {
    ...
    g_assert(FTT_CELL_IS_LEAF(c) && ftt_cell_level(c) == level);
    if (solid)
      wf += (solid->s[FTT_RIGHT] * s->f[FTT_RIGHT].un -
             solid->s[FTT_LEFT] * s->f[FTT_LEFT].un +
             solid->s[FTT_TOP] * s->f[FTT_TOP].un -
             solid->s[FTT_BOTTOM] * s->f[FTT_BOTTOM].un);
    else
      wf += (s->f[FTT_RIGHT].un - s->f[FTT_LEFT].un + s->f[FTT_TOP].un -
             s->f[FTT_BOTTOM].un);
...
static void face_interpolated_normal_velocity(const FttCellFace *face,
                                              GfsVariable **v) {
  gdouble u;
  ...
  case FTT_FINE_COARSE: {
    ...
    g_assert(w1 + w2);
    w1 = 2. * w1 / (w1 + w2);
    u = w1 * gfs_face_interpolated_value(face, i) +
        (1. - w1) * GFS_VALUEI(face->neighbor, i);
    break;
  }
  default:
  ...
  case FTT_FINE_COARSE:
    GFS_FACE_NORMAL_VELOCITY_RIGHT(face) +=
        u * GFS_FACE_FRACTION_LEFT(face) /
        (GFS_FACE_FRACTION_RIGHT(face) * FTT_CELLS_DIRECTION(face->d));
    break;
  default:
    g_assert_not_reached();
  }
...
static void depth_integrated_divergence(GfsDomain *domain, GfsVariable *div) {
  ...
  gfs_domain_cell_traverse_boundary(domain, FTT_FRONT, FTT_PRE_ORDER,
...
static void depth_integrated_coefficients(GfsDomain *domain) {
  ...
  gfs_domain_cell_traverse_boundary(
      domain, FTT_FRONT, FTT_POST_ORDER, FTT_TRAVERSE_NON_LEAFS, -1,
...
static void ocean_run(GfsSimulation *sim) {
  ...
  gts_container_foreach(GTS_CONTAINER(sim->events), (GtsFunc)gfs_event_init,
                        sim);
  ...
  while (sim->time.t < sim->time.end && sim->time.i < sim->time.iend) {
    ...
    gfs_correct_normal_velocities_weighted(
        ...
        sim->approx_projection_params.weighted);
    gfs_domain_cell_traverse_boundary(
        domain, FTT_FRONT, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1,
    ...
    gfs_free_surface_pressure(
        toplayer, &sim->approx_projection_params, &sim->advection_params, p,
        divn, div, res, sim->physical_params.g / GFS_OCEAN(domain)->layer->len);
    gts_object_destroy(GTS_OBJECT(divn));
 ...
 * @rho.
 */
void gfs_hydrostatic_pressure(GfsDomain *domain, GfsVariable *p,
                              GfsVariable *rho, gdouble g) {
  gpointer data[3];
...
static void gfs_source_hydrostatic_read(GtsObject **o, GtsFile *fp) {
  ...
  if (GTS_OBJECT_CLASS(gfs_source_hydrostatic_class())->parent_class->read)
    (*GTS_OBJECT_CLASS(gfs_source_hydrostatic_class())->parent_class->read)(o,
                                                                            fp);
  if (fp->type == GTS_ERROR)
    return;
  ...
  if (!(sh->ph = gfs_domain_get_or_add_variable(domain, fp->token->str,
...
static void gfs_source_hydrostatic_write(GtsObject *o, FILE *fp) {
  (*GTS_OBJECT_CLASS(gfs_source_hydrostatic_class())->parent_class->write)(o,
                                                                           fp);
  fprintf(fp, " %s %s", GFS_SOURCE_HYDROSTATIC(o)->rho->name,
          GFS_SOURCE_HYDROSTATIC(o)->ph->name);
}

static gdouble gfs_source_hydrostatic_mac_value(GfsSourceGeneric *s,
                                                FttCell *cell, GfsVariable *v) {
  return -gfs_center_gradient(cell, v->component,
                              GFS_SOURCE_HYDROSTATIC(s)->ph1->i) /
         ftt_cell_size(cell);
}
...
static gdouble gfs_source_hydrostatic_centered_value(GfsSourceGeneric *s,
  ...
  return -(gfs_center_gradient(cell, v->component, b->ph->i) +
           gfs_center_gradient(cell, v->component, b->ph1->i)) /
         (2. * ftt_cell_size(cell));
}
...
static gboolean gfs_source_hydrostatic_event(GfsEvent *event,
                                             GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_event_sum_class())->parent_class)
    ...
    if (s->not_first) {
      ...
      gfs_hydrostatic_pressure(GFS_DOMAIN(sim), s->ph1, s->rho,
                               sim->physical_params.g);
      gfs_domain_bc(GFS_DOMAIN(sim), FTT_TRAVERSE_LEAFS, -1, s->ph1);
      s->not_first = TRUE;
    }
...
static void gfs_source_hydrostatic_event_half(GfsEvent *event,
                                              GfsSimulation *sim) {
  GfsSourceHydrostatic *s = GFS_SOURCE_HYDROSTATIC(event);

  gfs_hydrostatic_pressure(GFS_DOMAIN(sim), s->ph, s->rho,
                           sim->physical_params.g);
...
static void gfs_source_friction_write(GtsObject *o, FILE *fp) {
  ...
  fprintf(fp, " %s %g", GFS_SOURCE_FRICTION(o)->h->name,
          GFS_SOURCE_FRICTION(o)->f);
}
...
static gboolean gfs_source_friction_event(GfsEvent *event, GfsSimulation *sim) {
  if ((*GFS_EVENT_CLASS(GTS_OBJECT_CLASS(gfs_event_sum_class())->parent_class)
    ...
    gfs_domain_cell_traverse(GFS_DOMAIN(sim), FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS,
                             -1, (FttCellTraverseFunc)save_velocity, event);
    return TRUE;
  }
  return FALSE;
}
...
static void flather(FttCellFace *f, GfsBc *b) {
  ...
  GFS_VALUE(f->cell, b->v) =
      2. * flather_value(f, b) - GFS_VALUE(f->neighbor, b->v);
}
