...
static gdouble transverse_term(FttCell *cell, gdouble *msize, FttComponent c,
...
void gfs_cell_advected_face_values(FttCell *                 cell,
  ...
  if (par->v->domain->scale_metric)
    for (c = 0; c < FTT_DIMENSION; c++)
      msize[c] =
          size * (*par->v->domain->scale_metric)(par->v->domain, cell, c);
  else
    for (c = 0; c < FTT_DIMENSION; c++)
      msize[c] = size;
...
static gdouble interpolate_1D1(const FttCell *cell, FttDirection dright,
                               FttDirection dup, gdouble x) {
  FttCell *       n;
  FttDirection    dleft;
  GfsStateVector *s;
...
static gdouble interpolate_2D1(const FttCell *cell, FttDirection dright,
                               FttDirection d1, FttDirection d2, gdouble x,
                               gdouble y) {
  FttCell *n1, *n2;
  gdouble x1 = 0., y1 = 1.;
  gdouble x2 = 1., y2 = 0.;
...
gdouble gfs_face_upwinded_value(const FttCellFace *face, GfsUpwinding upwinding,
  ...
  case FTT_FINE_FINE:
    return un > 0. ? GFS_STATE(face->cell)->f[face->d].v
                   : un < 0. ? GFS_STATE(face->neighbor)
  ...
  case FTT_FINE_COARSE:
    if (un > 0.)
...
#else  /* FTT_3D */
      gint *dp;
      static gint perpendicular[FTT_NEIGHBORS][FTT_CELLS][2] = {{{-1, -1},
                                                                 {2, 4},
                                                                 {-1, -1},
                                                                 {3, 4},
                                                                 {-1, -1},
      ...
      vcoarse = interpolate_2D1(face->neighbor, face->d, dp[0], dp[1], 1. / 4.,
                                1. / 4.);
#endif /* FTT_3D */
      if (un == 0.)
...
void gfs_face_advection_flux(const FttCellFace *       face,
  ...
  flux = gfs_domain_face_fraction(par->v->domain, face) *
         GFS_FACE_NORMAL_VELOCITY(face) * par->dt *
         gfs_face_upwinded_value(face, GFS_FACE_UPWINDING, NULL) /
...
void gfs_face_velocity_advection_flux(const FttCellFace *       face,
  ...
  flux = gfs_domain_face_fraction(par->v->domain, face) *
         GFS_FACE_NORMAL_VELOCITY(face) * par->dt / ftt_cell_size(face->cell);
#if 0
...
void gfs_face_velocity_convective_flux(const FttCellFace *       face,
  ...
  case FTT_FINE_COARSE:
    GFS_VALUE(face->neighbor, par->fv) +=
        u *
        (GFS_STATE(face->neighbor)->f[face->d].un +
         GFS_STATE(face->neighbor)->f[FTT_OPPOSITE_DIRECTION(face->d)].un) /
        FTT_CELLS;
    break;
  default:
...
void gfs_face_advected_normal_velocity(const FttCellFace *       face,
  ...
  case FTT_FINE_COARSE:
    GFS_FACE_NORMAL_VELOCITY_RIGHT(face) +=
        ...
        (gfs_domain_face_fraction_right(par->v->domain, face) *
...
void gfs_face_interpolated_normal_velocity(const FttCellFace *face,
  ...
  GFS_FACE_NORMAL_VELOCITY_LEFT(face) = u =
      gfs_face_interpolated_value(face, v[face->d / 2]->i);
  ...
  case FTT_FINE_COARSE:
    GFS_FACE_NORMAL_VELOCITY_RIGHT(face) +=
        ...
        (gfs_domain_face_fraction_right(v[0]->domain, face) *
         FTT_CELLS_DIRECTION(face->d));
    break;
  default:
    g_assert_not_reached();
...
void gfs_face_reset_normal_velocity(const FttCellFace *face) {
  ...
  GFS_FACE_NORMAL_VELOCITY_RIGHT(face) = GFS_FACE_NORMAL_VELOCITY_LEFT(face) =
      0.;
}
...
static void set_merged(FttCell *cell) {
  ...
  if (!gfs_cell_is_small(cell))
    ...
    for (i = 0; i < FTT_NEIGHBORS && abest < 1.; i++)
      if (neighbor.c[i] && !GFS_CELL_IS_BOUNDARY(neighbor.c[i]) &&
          solid->s[i] > 0.) {
        if (FTT_CELL_IS_LEAF(neighbor.c[i])) {
          ...
          guint           j, n = ftt_cell_children_direction(
                       neighbor.c[i], FTT_OPPOSITE_DIRECTION(i), &child);
    ...
    if (abest == 0.)
      g_warning("file %s: line %d (%s): cannot merge small cell: %g", __FILE__,
                __LINE__, G_STRFUNC, solid->a);
  }
}
    ...
    for (i = 0; i < FTT_NEIGHBORS; i++)
      if (neighbor.c[i]) {
        if (!FTT_CELL_IS_LEAF(neighbor.c[i])) {
          ...
          n = ftt_cell_children_direction(neighbor.c[i],
                                          FTT_OPPOSITE_DIRECTION(i), &child);
          ;
          for (j = 0; j < n; j++)
            if (GFS_IS_MIXED(child.c[j]) &&
 ...
 */
void gfs_domain_traverse_merged(GfsDomain *domain, GfsMergedTraverseFunc func,
                                gpointer data) {
  gpointer datum[2];
...
void gfs_advection_update(GSList *merged, const GfsAdvectionParams *par) {
  ...
  if (merged->next == NULL) { /* cell is not merged */
    ...
    GFS_VALUE(cell, par->v) += GFS_VALUE(cell, par->fv) /
                               gfs_domain_cell_fraction(par->v->domain, cell);
  } else if (par->average) {
    /* average value */
    GSList *i = merged;
    ...
    while (i) {
      ...
      if (a < GFS_SMALL) {
        GFS_VALUE(cell, par->v) +=
            GFS_VALUE(cell, par->fv) / (GFS_SMALL * f / a);
...
static gdouble none(FttCell *cell, FttComponent c, guint v) { return 0.; }
...
void gfs_advection_params_read(GfsAdvectionParams *par, GtsFile *fp) {
  ...
  if (flux) {
    if (!strcmp(flux, "gfs_face_advection_flux"))
      ...
      gts_file_variable_error(fp, var, "flux", "unknown flux parameter `%s'",
                              flux);
    g_free(flux);
  }
