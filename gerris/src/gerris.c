...
static void set_box_pid(GfsBox *box, gint *pid) { box->pid = *pid; }
...
int main(int argc, char *argv[]) {
  ...
  while (c != EOF) {
    ...
    static struct option long_options[] = {
        ...
        {NULL}};
    int option_index = 0;
    switch ((c = getopt_long(argc, argv, "hVs:ip:PD:I:mde:b:vB", long_options,
                             &option_index))) {
    ...
    case 'h': { /* help */
      gchar *usage =
          ...
          "  -p N   --partition=N partition the domain in 2^N subdomains and "
          "returns\n"
          "                       the corresponding simulation\n"
          "  -b N   --bubble=N    partition the domain in N subdomains and "
          "returns\n"
          "                       the corresponding simulation\n"
          "  -d     --data        when splitting or partitioning, output all "
          "data\n"
...
#endif /* HAVE_M4 */
          "  -eEV   --event=EV    Evaluates GfsEvent EV and returns the "
          "simulation\n"
    ...
    case 'V': { /* version */
      ...
      gfs_error(0,
                ...
                GFS_COMPILATION_FLAGS, mpi, pkgconfig, m4);
      return 0; /* succes */
      break;
    }
    case '?': /* wrong options */
  ...
  if (optind >= argc) { /* missing FILE */
    gfs_error(0, "gerris: missing FILE\n"
  ...
  if (macros) {
    gchar *awk =
        g_strconcat("awk -v prefix=", strstr(m4_options, "-P") ? "m4_" : "",
                    " -f ", GFS_DATA_DIR, "/m4.awk ", NULL);
    gchar *command;
  ...
  if (!(simulation = gfs_simulation_read(fp))) {
    gfs_error(-1,
              ...
              "%s:%d:%d: %s\n",
              argv[optind], argv[optind], fp->line, fp->pos, fp->error);
    return 1;
  }
  gts_file_destroy(fp);
  ...
  if (domain->pid >= 0) {
    ...
    if (size > 1 && domain->np < size) {
      if (npart != 0) {
        gfs_error(
            0,
            "gerris: manual partitioning is not a valid option in parallel\n");
        return 1;
      }
  ...
  if ((bubble && npart > 1) || (!bubble && npart > 0)) {
    ...
    if (gts_container_size(GTS_CONTAINER(simulation)) < np) {
      gfs_error(0,
                "gerris: the number of boxes in the domain to partition should "
                "be >= %d\n"
    ...
    if (bubble)
      partition = gts_graph_bubble_partition(
          GTS_GRAPH(simulation), npart, 100,
          verbose ? (GtsFunc)gts_graph_partition_print_stats : NULL, stderr);
    else
      partition = gts_graph_recursive_bisection(GTS_WGRAPH(simulation), npart,
    ...
    while (i) {
      if (gts_container_size(GTS_CONTAINER(i->data)) == 0) {
        ...
        if (!bubble)
          fprintf(stderr, "Try using the '-b' option\n"
                          "Try `gerris --help' for more information.\n");
        return 1;
      }
    ...
    for (i = 0; i < events->len; i++) {
      ...
      if (fp->type == GTS_ERROR) {
        gfs_error(-1,
                  "gerris: invalid event: '%s'\n"
                  "%d:%d: %s\n",
                  (char *)g_ptr_array_index(events, i), fp->line, fp->pos,
                  fp->error);
        return 1;
      }
      if (GFS_IS_ADAPT(object))
        gts_container_add(GTS_CONTAINER(simulation->adapts),
                          GTS_CONTAINEE(object));
      else if (GFS_IS_SOLID(object))
        gts_container_add(GTS_CONTAINER(simulation->solids),
                          GTS_CONTAINEE(object));
      gts_container_add(GTS_CONTAINER(simulation->events),
                        GTS_CONTAINEE(object));
