...
static double dd(const gchar *name, FttComponent c) {
  ...
  return gfs_dimensional_value(
      v, gfs_center_gradient(_cell, c, v->i) /
...
static double dd2(const gchar *name, FttComponent c) {
  ...
  if (f1.neighbor && f2.neighbor) {
    ...
    return gfs_dimensional_value(
        v, (g1.b + g2.b - (g1.a + g2.a) * GFS_VALUE(_cell, v)) /
...
static double dsd(const gchar *name, FttComponent c) {
  ...
  return gfs_dimensional_value(
      v, (&g.x)[c] / (_sim->physical_params.L * ftt_cell_size(_cell)));
}
...
static double flux(const gchar *name) {
  ...
  if ((_cell->flags & GFS_FLAG_DIRICHLET) == 0)
    ...
    flux = gfs_cell_dirichlet_gradient_flux(_cell, v->i, -1,
                                            GFS_STATE(_cell)->solid->fv);
  }
  return gfs_dimensional_value(
      v, flux * pow(_sim->physical_params.L * ftt_cell_size(_cell),
                    FTT_DIMENSION - 2.));
}

static gboolean overlaps(double x1, double y1, double x2, double y2) {
  double    h = ftt_cell_size(_cell) / 2.;
  FttVector p, min = {G_MAXDOUBLE, G_MAXDOUBLE},
               max = {-G_MAXDOUBLE, -G_MAXDOUBLE};
