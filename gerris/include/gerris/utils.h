...
#define GFS_GLOBAL(obj)    GTS_OBJECT_CAST(obj, GfsGlobal, gfs_global_class())
#define GFS_IS_GLOBAL(obj) (gts_object_is_from_class(obj, gfs_global_class()))
...
#define GFS_FUNCTION(obj)                                                      \
  GTS_OBJECT_CAST(obj, GfsFunction, gfs_function_class())
#define GFS_FUNCTION_CLASS(klass)                                              \
  GTS_OBJECT_CLASS_CAST(klass, GfsFunctionClass, gfs_function_class())
#define GFS_IS_FUNCTION(obj)                                                   \
  (gts_object_is_from_class(obj, gfs_function_class()))
...
#define GFS_IS_FUNCTION_SPATIAL(obj)                                           \
  (gts_object_is_from_class(obj, gfs_function_spatial_class()))
...
#define GFS_IS_FUNCTION_MAP(obj)                                               \
  (gts_object_is_from_class(obj, gfs_function_map_class()))

GfsFunctionClass *gfs_function_map_class(void);

/* GfsFunctionConstant: Header */

#define GFS_IS_FUNCTION_CONSTANT(obj)                                          \
  (gts_object_is_from_class(obj, gfs_function_constant_class()))
...
#define g_assert_not_reached()                                                 \
  G_STMT_START {                                                               \
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_ERROR,                                     \
          "file %s: line %d (%s): should not be reached", __FILE__, __LINE__,  \
          __PRETTY_FUNCTION__);                                                \
    abort();                                                                   \
  }                                                                            \
  G_STMT_END
...
#define g_assert(expr)                                                         \
  G_STMT_START {                                                               \
    if (expr) {                                                                \
      ...
      g_log(G_LOG_DOMAIN, G_LOG_LEVEL_ERROR,                                   \
            "file %s: line %d: assertion failed: (%s)", __FILE__, __LINE__,    \
            #expr);                                                            \
  }                                                                            \
  G_STMT_END
...
#define g_assert_not_reached()                                                 \
  G_STMT_START {                                                               \
    g_log(G_LOG_DOMAIN, G_LOG_LEVEL_ERROR,                                     \
          "file %s: line %d: should not be reached", __FILE__, __LINE__);      \
  }                                                                            \
  G_STMT_END
