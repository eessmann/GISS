...
struct _GfsDomain {
  ...
  gdouble (*viscous_metric_implicit)(const GfsDomain *domain, FttCell *cell,
                                     FttComponent component);

  gdouble (*viscous_metric_explicit)(const GfsDomain *domain, FttCell *cell,
                                     GfsVariable *v, GfsDiffusion *d);

  void (*advection_metric)(const GfsDomain *domain, FttCell *cell,
  ...
  GSList *projections; /**< list of GfsDomainProjection associated with this
                          domain */
...
#define GFS_DOMAIN(obj) GTS_OBJECT_CAST(obj, GfsDomain, gfs_domain_class())
#define GFS_DOMAIN_CLASS(klass)                                                \
  GTS_OBJECT_CLASS_CAST(klass, GfsDomainClass, gfs_domain_class())
#define GFS_IS_DOMAIN(obj) (gts_object_is_from_class(obj, gfs_domain_class()))

#define gfs_domain_variables_number(d) ((d)->allocated->len - 1)
#define gfs_domain_variables_size(d)                                           \
  (sizeof(GfsStateVector) + sizeof(gdouble) * (MAX((d)->allocated->len, 1) - 1))
...
#define gfs_domain_traverse_leaves(d, f, data)                                 \
  (gfs_domain_cell_traverse(d, FTT_PRE_ORDER, FTT_TRAVERSE_LEAFS, -1, f, data))
...
void gfs_domain_homogeneous_bc(GfsDomain *domain, FttTraverseFlags flags,
                               gint max_depth, GfsVariable *ov, GfsVariable *v);
...
typedef void (*FttCellCombineTraverseFunc)(FttCell *cell1, FttCell *cell2,
                                           gpointer data);
...
GfsDerivedVariable *
gfs_domain_add_derived_variable(GfsDomain *domain, GfsDerivedVariableInfo info);
 ...
 */
static inline gdouble gfs_domain_face_fraction(const GfsDomain *  domain,
 ...
 * orthogonal metric of @domain.
 */
static inline gdouble gfs_domain_face_fraction_right(const GfsDomain *  domain,
                                                     const FttCellFace *face) {
 ...
 * orthogonal metric of @domain.
 */
static inline gdouble gfs_domain_cell_fraction(const GfsDomain *domain,
                                               const FttCell *  cell) {
 ...
 */
static inline void gfs_domain_solid_metric(const GfsDomain *domain,
                                           const FttCell *cell, FttVector *m) {
 ...
 * center of the face.
 */
static inline gdouble gfs_domain_face_scale_metric(const GfsDomain *  domain,
                                                   const FttCellFace *face,
                                                   FttComponent       c) {
 ...
 */
static inline gdouble gfs_cell_volume(const FttCell *  cell,
                                      const GfsDomain *domain) {
  double v = ftt_cell_volume(cell) *
...
#define GFS_DOMAIN_PROJECTION(obj)                                             \
  GTS_OBJECT_CAST(obj, GfsDomainProjection, gfs_domain_projection_class())
...
typedef void (*GfsProjectionTraverseFunc)(FttCell *cell, FttCell *proj,
                                          gpointer data);
