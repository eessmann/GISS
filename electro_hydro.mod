...
static void gfs_source_electric_read (GtsObject ** o, GtsFile * fp)
{
  ...
  for (c = 0; c < FTT_DIMENSION; c++){
    ...
    GFS_SOURCE_ELECTRIC (*o)->fe[c]->units = -5;
  }
}
...
static gdouble gfs_source_electric_centered_value (GfsSourceGeneric * s,
						   ...
						   GfsVariable * v)
{ 
  return  gfs_dimensional_value (GFS_SOURCE_ELECTRIC (s)->fe[v->component], GFS_VALUE (cell, GFS_SOURCE_ELECTRIC (s)->fe[v->component]));
}
