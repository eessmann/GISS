...
plot -64./13.*1e-3*x+0.75 t 'analytical', \
     '< awk -f field.awk -v name="DRHO" end-8.txt | awk -f thermo.awk' t '8 layers', \
     '< awk -f field.awk -v name="DRHO" end-16.txt | awk -f thermo.awk' t '16 layers', \
     '< awk -f field.awk -v name="DRHO" end-32.txt | awk -f thermo.awk' t '32 layers', \
     '< awk -f field.awk -v name="DRHO" end-64.txt | awk -f thermo.awk' t '64 layers'
...
splot [-5:5][0:1.01]'< awk -v name="DRHO" -f field.awk end-64.txt' u 1:2:3
EOF
else
    exit 1
fi
